...
*/
    ...
    //3.5 fix: allow text selection if the target element
    if (bCancel && !(objSource && objSource.tagName.search(/input/i) == 0))
      objEvent.cancelAll();
  };
  ...
  Matrix_prototype._onPaint = function(bUpdateScroll, bNoMasks) {
    ...
    if (intModel == Matrix.PAGING_OFF || intModel == Matrix.PAGING_STEPPED) {
      ...
      if (intModel == Matrix.PAGING_PAGED) {
        ...
        if (intPanelLength > 1 && this._isValidPanel(intPanelLength -1))
