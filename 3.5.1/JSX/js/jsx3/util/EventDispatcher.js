...
/* @JSC :: end */
  ...
  /** @private @jsxobf-clobber */
  EventDispatcher_prototype._jsxeventreg = null;
...
/* @JSC :: end */
    ...
    for (var i = 0; i < strEventId.length; i++) {
      ...
      var queue = registry[strEventId[i]];
      if (queue == null) 
        queue = registry[strEventId[i]] = new jsx3.util.List();
      queue.add(toAdd);
    }
  };
  ...
  EventDispatcher_prototype.unsubscribe = function(strEventId, objHandler) {
    ...
    for (var i = 0; i < strEventId.length; i++) {
      ...
      if (queue != null) {
        for (var j = queue.iterator(); j.hasNext(); ) {
          if (j.next()[1] === objHandler)
            j.remove();
        }
      }
    }
  ...
  EventDispatcher_prototype.unsubscribeAll = function(strEventId) {
    delete this._getEventRegistry()[strEventId];
  };
  ...
  EventDispatcher_prototype.publish = function(objEvent) {
    ...
    var q1 = this._getEventRegistry()[strEventId];
    var q2 = this._getEventRegistry()[EventDispatcher.SUBJECT_ALL];
    if ((q1 == null || q1.size() == 0) && (q2 == null || q2.size() == 0)) return 0;
    
    // defensive copy: subscribers may unsubscribe themselves
    var queue = new jsx3.util.List();
    if (q1 != null) queue.addAll(q1);
    if (q2 != null) queue.addAll(q2);

    for (var i = queue.iterator(); i.hasNext(); ) {
      var item = i.next();
      var type = item[0];
      var target = item[1];
      var method = item[2];
