...
jsx3.Class.defineClass("jsx3.io.FileDialog", jsx3.gui.Dialog, null, function(FileDialog, FileDialog_prototype) {
  ...
  /** @private @jsxobf-clobber */
	FileDialog.TYPE_SAVE_FOLDER = FileDialog.MODE_SAVE | FileDialog.CHOOSE_FOLDER;
  ...
  /** @private @jsxobf-clobber */
	FileDialog.RESOURCE = "components/filesystem/opensave.xml";
   ...
   */
  FileDialog.deserialize = function(objParent, strName, bModal) {
		var dialog = objParent.loadAndCache(FileDialog.RESOURCE, false);
		dialog.setName(strName);
    if (bModal)
      dialog.setModal(jsx3.gui.Dialog.MODAL);
    objParent.paintChild(dialog);
    return dialog;
	};
	
	FileDialog_prototype.init = function(strName,vntWidth,vntHeight) {
		//call constructor for super class
		this.jsxsuper(strName,vntWidth,vntHeight,"File Dialog");
		
    /* @jsxobf-clobber */
		this._mode = FileDialog.MODE_OPEN;
    /* @jsxobf-clobber */
		this._current = null;
    /* @jsxobf-clobber */
		this._choose = FileDialog.CHOOSE_FILE;
	};

	FileDialog_prototype.openForOpen = function(strPath, strBase, intChoose) {
		this._mode = FileDialog.MODE_OPEN;
		this.openForX(strPath, strBase, intChoose);
	};
	
	FileDialog_prototype.openForSave = function(strPath, strBase, intChoose) {
		if (intChoose == FileDialog.CHOOSE_FILES)
			jsx3.ERROR.doLog("FLDI05", "cannot use file dialog to save to multiple files", 1);
		
		this._mode = FileDialog.MODE_SAVE;
		this.openForX(strPath, strBase, intChoose);
	};
	
  /** @private @jsxobf-clobber */
	FileDialog_prototype.openForX = function(strPath, strBase, intChoose) {
		if (intChoose == null)
			intChoose = FileDialog.CHOOSE_FILE;
		
		if (typeof(strPath) == 'string')
			this._current = new File(strPath);
		else if (strPath instanceof File && strPath.isDirectory())
			this._current = strPath;
		else 
			this._current = jsx3.ide.getSystemDirFile();

    /* @jsxobf-clobber */
		this._base = strBase instanceof File ? strBase : (strBase != null ? new File(strBase) : null);
    if (this._base == null)
      this.roots = File.getRoots();
    
    // prevent UI lock out
    if (this._base != null && ! this._current.isDescendantOf(this._base))
      this._base = this._current;
    
    this._choose = intChoose;
    /* @jsxobf-clobber */
		this._type = this._mode | intChoose;
		
		var bOpen = this._mode == FileDialog.MODE_OPEN;
		var bMultiple = bOpen && (intChoose == FileDialog.CHOOSE_FILES);
    ...
    this._getFileList().setSelectionModel(bMultiple ? Matrix.SELECTION_MULTI_ROW : Matrix.SELECTION_ROW);
		...
		switch (this._type) {
			case FileDialog.TYPE_OPEN_FILE: 
				strTitle = "Open File"; break;
			case FileDialog.TYPE_OPEN_FILES: 
				strTitle = "Open Files"; break;
			case FileDialog.TYPE_OPEN_FOLDER: 
				strTitle = "Choose Folder"; break;
			case FileDialog.TYPE_SAVE_FILE: 
				strTitle = "Save File"; break;
			case FileDialog.TYPE_SAVE_FOLDER: 
				strTitle = "Save To Folder"; break;
		}
		
		var title = this.getCaptionBar();
		if (title != null && strTitle)
			title.setText(strTitle, true);

		this._fixPath();
    
    this.setVisibility(jsx3.gui.Block.VISIBILITYVISIBLE, true);

    // timeout for slightly improved responsiveness
    jsx3.sleep(function() {
      if (this._mode == FileDialog.MODE_OPEN)
  ...
  /** @private @jsxobf-clobber */
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      var date = stat.mtime;
			var disabled = FileDialog.BINARY_EXT[file.getExtension()] || Number(0);

      var bFolder = file.isDirectory();
      var record = {
				jsxid: file.toURI().toString(),
				folder: bFolder ? 'true' : 'false',
        jsximg: file.isReadOnly() ? (bFolder ? "images/icon_7l.gif" : "images/icon_29l.gif") : (bFolder ? "images/icon_7.gif" : "images/icon_29.gif"),
        name: file.getName(),
...
if (jsx3.app.Browser.macosx) {
  ...
  /** @private @jsxobf-clobber */
    ...
    while (file != null && this._checkSecurity(file)) {
      ...
      record.setAttribute("jsximg", file != null ? "images/icon_7.gif" : "images/icon_103.gif");
  ...
  FileDialog_prototype._redrawPathSelect = function() {
    ...
    while (file != null && this._checkSecurity(file)) {
      ...
      file = file.exists() ? file.getParentFile() : null;
      record.setAttribute("jsximg", file != null ? "images/icon_7.gif" : "images/icon_102.gif");
    ...
    if (this.roots != null) {
      ...
      for (var i = 0; i < this.roots.length; i++) {
        ...
        if (! currentRoot.equals(root)) {
          ...
          node.setAttribute("jsximg", "images/icon_102.gif");
...
//		window.alert("doPathSelect " + strRecordId);
		...
		if (record != null) {
			this._current = new File(new URI(record.getAttribute("jsxid")));
			this._currentDidChange();
		}
	};
  ...
  /** @private */
	FileDialog_prototype.doFileSelect = function(strRecordId) {
		...
		for (var i = list.getSelectedNodes().iterator(); i.hasNext(); ) {
			...
			if (path) {
				var file = new File(new URI(path));
				if (names) names += ", ";
				names += file.getName();
			}
		}
  ...
  /** @private @jsxobf-clobber */
	FileDialog_prototype._fixPath = function() {
		...
		if (! this._current.isDirectory()) {
			// TODO: big error!
			jsx3.ERROR.doLog("FLDI01", this._current + " is not a folder, cannot recover", 1);
			this._current = null;
  ...
  /** @private */
	FileDialog_prototype.doNfCreate = function(folderName) {
		var newFolder = new File(this._current.toURI().resolve(URI.encode(folderName)));
  ...
  /** @private */
	FileDialog_prototype.doExecute = function(bConfirmed) {
		...
		var files = [];
		for (var i = list.getSelectedNodes().iterator(); i.hasNext(); )
			files.push(new File(new URI(i.next().getAttribute("jsxid"))));

		if (files.length == 0) {
			var fileName = this._getFileTextField().getValue();
			if (fileName) {
				files[0] = new File(this._current.toURI().resolve(URI.encode(fileName)));
			} else {
				files[0] = this._current;
			}
		}

    var bSave = (this._mode & FileDialog.MODE_SAVE) > 0;
    ...
    if (this._type == FileDialog.TYPE_SAVE_FILE && toExec != null) {
      if (!this._checkIsChild(toExec)) {
        this.alert(null, '"' + this._getFileTextField().getValue() + '" is not a valid file name.');
        return;
      }
    }
  ...
  /** @private @jsxobf-clobber */
    ...
    var parent = new File(objFile.getParentPath());
