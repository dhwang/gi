    ...
    if (this.MSXML_VERSION == null) {
      ...
      if (vers) {
        ...
        try {
          ...
          jsx3.MSXML_VERSION = 4;
        } catch (e1) {
          try {
            new ActiveXObject("MSXML2.FreeThreadedDOMDocument.3.0");
            jsx3.MSXML_VERSION = 3;
          } catch (e2) {
            throw new jsx3.Exception("Can not instantiate Microsoft XML control version 3 or 4.");
          }
        } 
      }
    }
  ...
  jsx3.makeCallback = function(fctBody, objThis, varArgs) {
    ...
    if (typeof(fctBody) == "string") {
      return function() {
        ...
        objThis[fctBody].apply(objThis, a);
      };
    } else {
      return function() {
        // make arguments to this function available
...
/* @JSC */ if (jsx3.CLASS_LOADER.FX) {
    // HACK: In Fx MacOS X some native calls cause a timeout to be invoked in the same stack
    if (jsx3.lang.getCaller() != null) {
//      jsx3.util.Logger.GLOBAL.logStack(3, "Stack weirdness ...");
      jsx3._SLEEP_TO = window.setTimeout(jsx3._sleepChunk, 0);
      return;
    }
/* @JSC */ }
...
/* @JSC :: end */
    ...
    var record = null;
    while (jsx3._SLEEP_QUEUE.length > 0 && (record == null || record[0] == null))
      record = jsx3._SLEEP_QUEUE.shift();
  ...
  jsx3.startup = function() {
    if (window.OpenAjax) {
      OpenAjax.registerLibrary("gi", "http://www.tibco.com/gi", jsx3.getVersion());
      OpenAjax.registerGlobals("gi", ["gi", "jsx3", "jsx", "tibco"]);
    }
  };
...
jsx3.Class.defineInterface("jsx3.net.URIResolver", null, function(URIResolver, URIResolver_prototype) {
  ...
  /** @private @jsxobf-clobber */
  URIResolver.getAbsPathUri = function() {
  ...
  /** @private @jsxobf-clobber */
  URIResolver.getHomePathUri = function() {
  ...
  URIResolver.DEFAULT.resolveURI = function(strURI) {
    ...
    if (scheme) {
      var pathURI = jsx3.net.URI.fromParts(null, null, null, null, path, uri.getQuery(), uri.getFragment());
      if (scheme == "jsx") {
        resolved = URIResolver.getAbsPathUri().resolve(jsx3.SYSTEM_ROOT + pathURI);
      } else if (scheme == "jsxapp") {
        var resolver = URIResolver.getResolver(uri);
        resolved = resolver.resolveURI(pathURI.toString().substring(1));
      } else if (scheme == "jsxaddin") {
        var host = uri.getHost();
        var addinKey = host.split("!", 2).join(":");
        var addin = jsx3.System.getLoadedAddinByKey(addinKey);
        resolved = addin != null ? addin.resolveURI(pathURI.toString().substring(1)) : uri;
      } else if (scheme == "jsxuser") {
        resolved = URIResolver.getHomePathUri().resolve(pathURI.toString().substring(1));
      } else {
        resolved = uri;
      }
    } else {
      if (path.indexOf(jsx3.SYSTEM_ROOT + "/") == 0) {
        resolved = URIResolver.getAbsPathUri().resolve(uri);
      } else if (path.indexOf(jsx3.APP_DIR_NAME + "/") == 0) {
        resolved = URIResolver.getHomePathUri().resolve(uri);
      } else if (path.indexOf("/") == 0) {
        resolved = uri;
      } else if (path.indexOf("GI_Builder/") == 0) {
        resolved = URIResolver.getAbsPathUri().resolve(uri);
      } else if (path.indexOf("..") >= 0) {
        var loc = jsx3.app.Browser.getLocation();
        resolved = loc.relativize(loc.resolve(uri));
      } else {
        // this would branch based on the resolver
        resolved = uri;
      }
    }
  ...
  URIResolver.DEFAULT.getUriPrefix = function() {
    return URIResolver.getAbsPathUri().toString();
  };
  ...
  URIResolver.JSX.getURI = function() {
    if (this._uri == null)
      this._uri = URIResolver.getAbsPathUri().resolve(jsx3.SYSTEM_ROOT + "/");
    return this._uri;
  };
  ...
  URIResolver.JSX.resolveURI = function(strURI) {
    ...
    if (!URIResolver.isAbsoluteURI(uri)) {
      return URIResolver.DEFAULT.resolveURI(this.getURI().resolve(uri.toString()));
    } else {
  ...
  URIResolver.JSX.getUriPrefix = function() {
    return URIResolver.getAbsPathUri() + jsx3.SYSTEM_ROOT + "/";
  };  
  ...
  URIResolver.USER.resolveURI = function(strURI) {
    ...
    if (!URIResolver.isAbsoluteURI(uri)) {
      return URIResolver.DEFAULT.resolveURI(URIResolver.getHomePathUri().resolve(uri));
    } else {
      return URIResolver.DEFAULT.resolveURI(uri);
    }
  };
  ...
  URIResolver.USER.getUriPrefix = function() {
    return URIResolver.getHomePathUri().toString();
  };  
  
  URIResolver.USER.relativizeURI = function(strURI, bRel) {
    var relative = URIResolver.getHomePathUri().relativize(strURI);
    if (relative.isAbsolute() || bRel)
      return relative;
    else
  ...
  /**
   * Returns the resolver explicitly referenced in the scheme and host parts of the URI <code>strURI</code>. This
   * method returns the the following values depending on the scheme of the URI:
   * <ul>
   ...
   *   <li>all other schemes and relative URIs &#8211; <code>URIResolver.DEFAULT</code>.</li>
   * </ul>
   *
   * @param strURI {String|jsx3.net.URI}
  ...
  URIResolver.getResolver = function(strURI) {
    ...
    if (scheme) {
      if (scheme == "jsx") {
        ...
        var appPath = uri.getHost().replace(/!/g, "/");
        var loadedApp = jsx3.System.getLoadedAppByPath(appPath);
        return loadedApp != null ? loadedApp : new jsx3.net.ServerURIResolver(appPath);
      } else if (scheme == "jsxaddin") {
        var host = uri.getHost();
        var addinKey = host.split("!", 2).join(":");
        return jsx3.System.getLoadedAddinByKey(addinKey);
      } else if (scheme == "jsxuser") {
 ...
 * @private
 */
jsx3.Class.defineClass("jsx3.net.ServerURIResolver", null, [jsx3.net.URIResolver], 
  ...
  ServerURIResolver_prototype.resolveURI = function(strURI) {
    ...
    if (! URIResolver.isAbsoluteURI(uri)) {
      return URIResolver.DEFAULT.resolveURI(this._uri.resolve(uri.toString()));
    } else {
      return URIResolver.DEFAULT.resolveURI(uri);
    }
  };
