...
/* @JSC */ }
  ...
  Server._showEnv = function(objEvent) {
    ...
    s.push('<b>Browser:</b> ', jsx3.CLASS_LOADER.getType() + " (" + jsx3.CLASS_LOADER.getTypes().join(", "), ')<br/>');
  ...
  Server_prototype._doLoad = function(objXML) {
    this._doLoadCaught(objXML);
  };

  /** @private @jsxobf-clobber */
  Server_prototype._doLoadCaught = function(objXML) {
    var child = null;
...
/* @JSC :: end */
  ...
  Server_prototype._onHelpKey = function(objEvent) {
    ...
    if (objJSX) this.invokeHelp(objJSX);
  };
  ...
  Server_prototype.invokeHelp = function(objJSX) {
    ...
    if (strId) this.publish({subject:Server.HELP, helpid:strId, model:objJSX});
