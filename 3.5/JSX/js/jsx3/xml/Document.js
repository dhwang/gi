...
// @jsxobf-clobber  _document _jsxloadlistener _jsx_sn_hash
...
/* @JSC */ if (jsx3.CLASS_LOADER.FX) {
  //reparsed documents throw errors on callback (don't get exposed until response); bypass by comparing domain to host; if on filesystem, both are blank
  /* @jsxobf-clobber */
  Document._IS_DOMAIN_CHANGED = (new jsx3.net.URI(window.location.href)).getHost() != document.domain;
/* @JSC */ }
...
/* @JSC */ if (jsx3.CLASS_LOADER.IE) {
  try {
    ...
    /* @jsxobf-clobber */
    Document._PROTO = jsx3.getXmlInstance();
  } catch (e) {;}
/* @JSC */ }
   ...
   * @param-package objDoc {Object} the native browser document object (optional).
   */
...
/* @JSC */ if (jsx3.CLASS_LOADER.IE) {
        ...
        this._document.validateOnParse = false;
        this._document.resolveExternals = false;
        this._document.preserveWhiteSpace = true;
        this._document.setProperty("ForcedResync", false); // Lets IE use cached version of file.
/* @JSC */ } else {
...
/* @JSC */ }
  ...
  Document_prototype.load = function(strURL, intTimeout) {
    ...
    var objURL = jsx3.net.URIResolver.DEFAULT.resolveURI(strURL);
    ...
    var bAsync = this.getAsync() ? true : false;

/* @JSC */ if (jsx3.CLASS_LOADER.FX) {
    if(Document._IS_DOMAIN_CHANGED) {
      this._loadCatchXDomain(objURL, intTimeout, bAsync);
      return this;
    }
/* @JSC */ }

    this._document.async = bAsync;

    if (this.getAsync()) {
      // timeout because the parsers throw certain errors synchronously, which meant client code checking for errors twice
      jsx3.sleep(function() {
        if (intTimeout > 0) {
          /* @jsxobf-clobber */
          this._jsxasynctimeout = window.setTimeout(
            jsx3.makeCallback(function() { this._onAsyncTimeout(); }, this), intTimeout);
        }
        this._loadCatch(objURL, intTimeout, true);
      ...
      this._loadCatch(objURL, null, false);
    }
...
/* @JSC */ if (jsx3.CLASS_LOADER.IE) {
  ...
  /* @jsxobf-clobber */
  Document_prototype._loadCatch = function(objURL, intTimeout, bAsync) {
    Document._log(6, jsx3._msg("xml.doc_load", objURL));

    try {
/* @JSC :: begin BENCH */
      var t1 = new jsx3.util.Timer(Document.jsxclass, objURL);
/* @JSC :: end */

      if (bAsync)
        this._document.onreadystatechange = jsx3.makeCallback(function() {
          if (this._document.readyState == 4) {
/* @JSC :: begin BENCH */
            t1.log("load async");
/* @JSC :: end */
            this._onAsyncReadyStateChange();
          }
        }, this);

      this._document.load(objURL.toString());

      // check if a parsing error occurred with the msxml parser; if so, throw a programmatic error
      if (!bAsync && !this._setErrorFromParser(this._document)) {
/* @JSC :: begin BENCH */
        t1.log("load sync");
/* @JSC :: end */
        // bind expected references to the root node as the jsx3.xml.Document class implements the methods of the
        // jsx3.xml.Entity class, meaning all methods that one can expect from the jsx3.xml.Entity class should apply
  ...
  Document_prototype.loadXML = function(strXML) {
    ...
    try {
/* @JSC :: begin BENCH */
      var t1 = new jsx3.util.Timer(Document.jsxclass, jsx3.util.strTruncate(strXML, 30), 5);
/* @JSC :: end */
...
/* @JSC :: end */
      ...
      if (bAsync)
        req.onload = jsx3.makeCallback(function() {
/* @JSC :: begin BENCH */
          t1.log("load async");
/* @JSC :: end */
          this._onAsyncLoad(req);
        }, this);

		  req.open("GET", objURL.toString(), bAsync);
		  req.overrideMimeType("text/xml");
		  req.send(null);

      if (!bAsync) {
        if (!(req.status == 0 || (req.status >= 200 && req.status < 300))) {
          this.setError(102, jsx3._msg("xml.doc_status", objURL, req.status));
        } else {
          this._document = req.responseXML;
          if (! this._document)
            this._document = (new DOMParser()).parseFromString(req.responseText, "text/xml");
...
/* @JSC :: end */
  ...
  /* @jsxobf-clobber */
  Document_prototype._onAsyncLoad = function(objReq) {
    if (!(objReq.status == 0 || (objReq.status >= 200 && objReq.status < 300))) {
      this.setError(103, jsx3._msg("xml.doc_status", this.getSourceURL(), objReq.status));
      this.publish({subject:Document.ON_ERROR});
    } else {
      this._asyncCleanup(objReq);
      this._document = objReq.responseXML;

      if (this._setErrorFromParser(this._document)) {
        this.publish({subject:Document.ON_ERROR});
      } else if (this._document == null || this._document.documentElement == null) {
        // TODO: any way to get more information about the parsing error?
        this.setError(104, jsx3._msg("xml.doc_bad"));
        this.publish({subject:Document.ON_ERROR});
      } else {
        this._initEntity(this._document.documentElement);
  ...
  Document_prototype._asyncCleanup = function(objReq) {
    window.clearTimeout(this._jsxasynctimeout);
    this._jsxasynctimeout = null;
    if (objReq)
      objReq.onload = null;
  };
...
/* @JSC */ } else if (jsx3.CLASS_LOADER.FX) {
  ...
  /* @jsxobf-clobber */
  Document._EP = netscape.security.PrivilegeManager.enablePrivilege;
  /* @jsxobf-clobber */
  Document._PERM = "UniversalBrowserRead";
  ...
  Document_prototype._loadCatch = function(objURL, intTimeout, bAsync) {
    ...
    try {
      // If this document is loaded from another domain or scheme, we'll just clone it before saving it to
      // side step any security constraints.
      if (objURL.getScheme() && objURL.getScheme() != "file") {
        this._jsxclonedoc = true;
        Document._EP(Document._PERM);
      }
...
/* @JSC :: end */
      ...
      if (bAsync) {
        this._jsxloadlistener = jsx3.makeCallback(function() {
/* @JSC :: begin BENCH */
          t1.log("load async");
/* @JSC :: end */
          this._onAsyncLoad();
        }, this);
        this._document.addEventListener('load', this._jsxloadlistener, false);
      }
...
/* @JSC :: end */
  ...
  /* @jsxobf-clobber */
  Document_prototype._cloneRestrictedDoc = function() {
    if (this._jsxclonedoc) {
      delete this._jsxclonedoc;

      try {
        Document._EP(Document._PERM);
      } catch (e) {;}

      // TODO: there has to be a more efficient way of doing this and removing the security restrictions
      var strXML = (new XMLSerializer()).serializeToString(this._document);
      this._document = (new DOMParser()).parseFromString(strXML, "text/xml");
    }
  };

  /* @jsxobf-clobber */
  Document_prototype._onAsyncLoad = function() {
    this._asyncCleanup();
    this._cloneRestrictedDoc();

    if (this._setErrorFromParser(this._document)) {
      this.publish({subject:Document.ON_ERROR});
    } else if (this._document.documentElement == null) {
      // TODO: any way to get more information about the parsing error?
      this.setError(105, jsx3._msg("xml.doc_bad"));
      this.publish({subject:Document.ON_ERROR});
    } else {
      this._initEntity(this._document.documentElement);
      this.publish({subject:Document.ON_RESPONSE});
  ...
  /** @private @jsxobf-clobber */
  Document_prototype._asyncCleanup = function() {
    window.clearTimeout(this._jsxasynctimeout);
    this._jsxasynctimeout = null;

    // Sometimes removeEventListener() requires this permission, but it can also throw errors unnecessarily.
    //TODO: validate the reason for the empty catch
    try {
      Document._EP(Document._PERM);
    } catch (e) {;}

    try {
      this._document.removeEventListener("load", this._jsxloadlistener, false);
    } catch(e) {;}
    delete this._jsxloadlistener;
  };


  /** @private @jsxobf-clobber */
  Document_prototype._onAsyncLoadXDomain = function() {
    if (this._request.readyState == 4) {
      var req = this._request;

      //housecleaning
      this._asyncCleanupXDomain();

      if (!(req.status == 0 || (req.status >= 200 && req.status < 300))) {
        this.setError(102, jsx3._msg("xml.doc_status", this.getSourceURL(), req.status));
        this.publish({subject:Document.ON_ERROR});
      } else {
        //get string of xml
        var strXML = req.responseText;

        //create native xml instance and wrap
        try {
          this._document = (new DOMParser()).parseFromString(strXML, "text/xml");
          if (! this._setErrorFromParser(this._document))
            this._initEntity(this._document.documentElement);
          //notify subscribers that the doc is now loaded; use sleep.  If done synchronously, will also throw a security error
          jsx3.sleep(function() {this.publish({subject:Document.ON_RESPONSE});}, null, this);
        } catch (e) {
          //notify subscribers that the doc did not load
          this._setErrorFromParser(this._document, e);
          this.publish({subject:Document.ON_ERROR});
        }
      }
    }
  ...
  /* @jsxobf-clobber */
  Document_prototype._asyncCleanupXDomain = function() {
    //stop timeout thread from firing; the response came back in time
    try {
      window.clearTimeout(this._jsxasynctimeout);
      this._jsxasynctimeout = null;

      //reset the function reference to free up memory
      this._request.onreadystatechange = new Function();

      //dereference
      delete this._request;
    } catch(ex) {
      Document._log(6, jsx3._msg("xml.unknown", jsx3.NativeError.wrap(ex)));
    }
  };

  /* @jsxobf-clobber */
  Document_prototype._onAsyncTimeoutXDomain = function() {
    this._asyncCleanupXDomain();
  ...
  /* @jsxobf-clobber */
  Document_prototype._loadCatchXDomain = function(objURL, intTimeout, bAsync) {
    Document._log(6, jsx3._msg("xml.doc_load", objURL));

    try {
      //subscribe here to the timeout event
      if (intTimeout > 0 && bAsync) {
        this._jsxasynctimeout = window.setTimeout(
          jsx3.makeCallback(function() { this._onAsyncTimeoutXDomain(); }, this), intTimeout);
      }
      ...
      if (bAsync) {
        ...
        if (!(req.status == 0 || (req.status >= 200 && req.status < 300))) {
          ...
          //get string of xml and resolve as xml object
          var strXML = req.responseText;
          try {
             this._document = (new DOMParser()).parseFromString(strXML, "text/xml");
             if (! this._setErrorFromParser(this._document))
...
/* @JSC */ if (jsx3.CLASS_LOADER.SAF) {
      ...
      if (docElement == null) {
        ...
        var objResult = (new XPathEvaluator()).evaluate("//x:parsererror//x:div", parser, nsResolver);
...
/* @JSC */ if (jsx3.CLASS_LOADER.IE) {
  ...
  /* @jsxobf-clobber */
  Document_prototype._onAsyncReadyStateChange = function() {
    this._asyncCleanup();

    // HACK: in IE the onreadystatechange callback can be called synchronously if a synchronous request is issued
    // after this call.
    jsx3.sleep(this._onAsyncReadyStateChangeSleep, null, this);
  };

  /* @jsxobf-clobber */
  Document_prototype._onAsyncReadyStateChangeSleep = function() {
    if (this._setErrorFromParser(this._document)) {
      this.publish({subject:Document.ON_ERROR});
    } else {
      this._initEntity(this._document.documentElement);
  ...
  /* @jsxobf-clobber */
  Document_prototype._onAsyncTimeout = function() {
    this._asyncCleanup();
    this.setError(111, jsx3._msg("xml.timeout"));
    this.publish({subject:Document.ON_TIMEOUT});
  };
  ...
  Document_prototype._asyncCleanup = function() {
    window.clearTimeout(this._jsxasynctimeout);
    this._jsxasynctimeout = null;
    this._document.onreadystatechange = new Function();
  };
...
/* @JSC */ if (! jsx3.CLASS_LOADER.IE) {
  ...
  Document_prototype._onAsyncTimeout = function() {
    this._asyncCleanup();
    this.setError(112, jsx3._msg("xml.timeout"));
    this.publish({subject:Document.ON_TIMEOUT});
  };
...
/* @JSC */ }
  ...
  Document_prototype.getAsync = function(strName) {
    return (this._jsxasync == null) ? false : this._jsxasync;
  };
