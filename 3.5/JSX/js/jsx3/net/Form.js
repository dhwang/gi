...
jsx3.Class.defineClass("jsx3.net.Form", null, [jsx3.util.EventDispatcher], function(Form, Form_prototype) {
  ...
  Form_prototype.initIFrame = function(strFormHtml) {
    ...
    if (! theForm)
      theForm = '<form method="get" action=""><input type="submit"/></form>';
    ...
    jsx3.html.insertAdjacentHTML(document.body, "beforeEnd", "<span id='" + this._id + 
        "_span' style='border:2px solid black;position:absolute;left:-50px;top:0px;width:10px;height:10px;overflow:hidden;'><iframe id='" + 
        this._id + "_frame' style='width:100%;height:100%;'></iframe></span>");

    /* @jsxobf-clobber */
    this._span = document.getElementById(this._id + "_span");
    /* @jsxobf-clobber */
    ...
    if (strFormHtml) {
      ...
      this._multipart = this._form.encoding &&
...
/* @JSC */ }
  ...
  Form_prototype.setField = function(strName, strValue, bConcat) {
    ...
    if (input == null || typeof(input) != "object") {
      jsx3.html.insertAdjacentHTML(form, "beforeEnd", "<div>" + strName + ": <input type='text' name='" + strName + "'/></div>");
...
/* @JSC */ } else {
  ...
  Form_prototype.send = function(intPollInterval, intTimeout) {
    ...
    var me = this;
    this._iframe.onload = function() { me.onIFrameLoad(); };
    ...
    /* @jsxobf-clobber */
    this._intervalId = window.setTimeout(function() {me.onIFrameTimeout();}, intPollInterval * intTimeout);
  };

  /* @jsxobf-clobber */
  Form_prototype.onIFrameLoad = function() {
  ...
  Form_prototype.getResponseText = function() {
    ...
    if (dcmt instanceof XMLDocument) {
      ...
      return dcmt.childNodes[0].innerHTML;
    } else {
