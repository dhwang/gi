...
jsx3.Class.defineClass("jsx3.gui.Splitter", jsx3.gui.Block, null, function(Splitter, Splitter_prototype) {
  ...
  jsx3.html.loadImages("jsx:///images/splitter/v.gif", "jsx:///images/splitter/h.gif");
...
/* NEW BOX MODEL ABSTRACTION **********************************************/
  ...
  Splitter_prototype.getClientDimensions = function(objChild) {
    ...
    var dblPct1 = Number(this.getSubcontainer1Pct().replace(/%/g,"")) / 100;
  ...
  Splitter_prototype.updateBoxProfile = function(objImplicit, objGUI, objQueue) {
    ...
    if (objGUI) {
      ...
      var maxLen = (oKids.length > 2) ? 2 : oKids.length;
      var child1Size = 0;
      for (var i = 0; i < maxLen; i++) {
        ...
        var myWidth = (objChildDimension.width != null) ? objChildDimension.width : objChildDimension.parentwidth;
        var myHeight = (objChildDimension.height != null) ? objChildDimension.height : objChildDimension.parentheight;

        child1Size = (this.getOrientation() == Splitter.ORIENTATIONH) ? myWidth : myHeight;
      }
    }
  };
  ...
  Splitter_prototype.paint = function() {
    ...
    b1.setStyles(this.paintBackgroundColor() + this.paintVisibility() + this.paintDisplay() + this.paintWrap() + this.paintCSSOverride());
    ...
    b1a.setStyles(this.paintBackgroundColor() + ((this.getOrientation() == Splitter.ORIENTATIONH)?this.paintHSplitImage():this.paintVSplitImage()));
  ...
  /**
   * renders layyout-flow prop
   * @return {String} e.g., layout-flow:horizontal;
   * @private
   */
  Splitter_prototype.paintWrap = function() {
    return "";//(this.getOrientation() == Splitter.ORIENTATIONH) ? "layout-flow:horizontal;" : "layout-flow:;";
  };
  ...
  Splitter_prototype.doBeginMove = function(objEvent, objGUI) {
    ...
    Event.subscribe(Event.MOUSEUP,this,"doResizeEnd");
  };
   ...
   * @private
   */
   Splitter_prototype.doResizeEnd = function(objEvent) {
     objEvent = objEvent.event;
     var objGUI = this.getRendered(objEvent).childNodes[0];
     Event.unsubscribe(Event.MOUSEUP,this,"doResizeEnd");
    ...
    objGUI.style.backgroundColor = (this.getBackgroundColor()) ? this.getBackgroundColor() : Splitter.DEFAULTBGCOLOR;

    //process resize according to the orientation of the splitter
    if (this.getOrientation() == Splitter.ORIENTATIONH) {
      var val1 = parseInt(objGUI.style.left);
      var intTotal = objGUI.parentNode.offsetWidth;
      if (val1 < this.getSubcontainer1Min()) val1 = this.getSubcontainer1Min();
      if (val1 > intTotal - 8) val1 = intTotal - 8;
      var intPct = (val1 / intTotal) * 100;
      intPct = parseInt(intPct * 100) / 100;
      var strPct1 = intPct + '%';
    } else {
      var val1 = parseInt(objGUI.style.top);
      var intTotal = objGUI.parentNode.offsetHeight;
    ...
    //3.2: call to update the box profile
    //set the value (when using new box model, old method is skipped)
    this.setSubcontainer1Pct(strPct1,true);
...
/* @JSC :: end */
  ...
  Splitter_prototype.paintVSplitImage = function() {
    ...
    return (strURL.search(/background-image\s*:\s*url\(/) != -1) ?
      strURL : "background-image:url(" + jsx3.resolveURI(strURL) + ");background-repeat:no-repeat;background-position:center center;";
  };
