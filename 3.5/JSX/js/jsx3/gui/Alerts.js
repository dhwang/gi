...
jsx3.Class.defineInterface("jsx3.gui.Alerts", null, function(Alerts, Alerts_prototype) {
  ...
  Alerts_prototype.alert = function(strTitle, strMessage, fctOnOk, strOk, objParams) {
    ...
    var dialog = this.getAlertsParent().loadAndCache('xml/components/dialog_alert.xml', false, null, resolver);
    ...
    if (fctOnOk != null) {
      ...
      ok.setEvent("this." + onExecute + "(this.getAncestorOfType(jsx3.gui.Dialog));", Interactive.EXECUTE);
    }
  ...
  Alerts_prototype.prompt = function(strTitle, strMessage, fctOnOk, fctOnCancel, strOk, strCancel, objParams) {
    var resolver = jsx3.net.URIResolver.JSX;
    var dialog = this.getAlertsParent().loadAndCache('xml/components/dialog_prompt.xml', false, null, resolver);
    var ok = dialog.getDescendantOfName('ok');
    ...
    if (fctOnOk != null) {
      var onExecute = "_onExecute";
      ok._onExecute = fctOnOk;
      ok.setEvent("var d = this.getAncestorOfType(jsx3.gui.Dialog); this." + onExecute + "(d, d.getDescendantOfName('value').getValue());", Interactive.EXECUTE);
    }
    if (fctOnCancel != null) {
      var onExecute = "_onExecute";
      cancel._onExecute = fctOnCancel;
      cancel.setEvent("this." + onExecute + "(this.getAncestorOfType(jsx3.gui.Dialog));", Interactive.EXECUTE);
    }
  ...
  Alerts_prototype.confirm = function(strTitle, strMessage, fctOnOk, fctOnCancel, strOk, strCancel, intBtnDefault,
    ...
    var resolver = jsx3.net.URIResolver.JSX;
    var dialog = this.getAlertsParent().loadAndCache('xml/components/dialog_confirm.xml', false, null, resolver);
    var ok = dialog.getDescendantOfName('ok');
    var cancel = dialog.getDescendantOfName('cancel');
    ...
    if (fctOnNo != null || strNo != null || intBtnDefault == 3) {
      ...
      if (fctOnNo) {
        ...
        no.setEvent("this." + onExecute + "(this.getAncestorOfType(jsx3.gui.Dialog));", Interactive.EXECUTE);
      }
   ...
   * @param objParams {object} may include fields 'width', 'height', 'noTitle', and 'nonModal'.
   * @protected
   */
