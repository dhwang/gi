...
Copyright 2006-2007 TIBCO Software, Inc
 ...
 (0.8 ) updated for selenium 0.8.2
        - see the release notes under gitak/gi for list of changed and new features.
        - added include command, which is modified from include command v2.1 by Rober Zimmermann
 */
...
/* Fire a left mouse button mouse event in a browser-compatible manner
          evt.detail = 0;
          evt.screenX = screenX;
          evt.screenY = screenY;
          evt.ctrlKey = false;
...
function triggerLeftMouseEvent(element, eventType, canBubble, objPos) {
    ...
    if (element && element.fireEvent) {
        ...
        if (selenium.browserbot && selenium.browserbot.controlKeyDown)
            evt.ctrlKey = selenium.browserbot.controlKeyDown;

        evt.shiftKey = false;
        if (selenium.browserbot && selenium.browserbot.shiftKeyDown)
            evt.shiftKey = selenium.browserbot.shiftKeyDown;

        evt.altKey = false;
        if (selenium.browserbot && selenium.browserbot.altKeyDown)
        ...
        // Mac only?
        var metaKey = false;
        if (selenium.browserbot && selenium.browserbot.metaKeyDown)
...
*/
   ...
   try {
     ...
     if (jsxElement && jsxElement.onmouseout) {
      ...
      LOG.debug("Exception caught in clickJsxElement! message=" + e.message);
   }
...
Selenium.prototype.doDragJsxTo = function(locator, posval) {
   ...
   */
    ...
    var element = this.browserbot.findElement(locator);
    var jsxprop = locator.split('=')[1];
    var jsxObj = this.browserbot.findByJsxName(jsxprop);
    if (jsxObj == null) // not found? try with text getter
      jsxObj = this.browserbot.findByJsxText(jsxprop);
    var jsxPos = jsxObj.getAbsolutePosition();
    jsxPos.T += 2;
    jsxPos.L += 2; // add 2 pixel to be within the rendered element
    triggerLeftMouseEvent(element, 'mousedown', true, jsxPos);
...
Selenium.prototype.doDragJsxToJsx = function(locator, locator2) {
   ...
   */
    ...
    var element = this.browserbot.findElement(locator);
    var elementTo = this.browserbot.findElement(locator2);
    LOG.debug("element from = " + getOuterHTML(element));
    LOG.debug("element to = " + getOuterHTML(elementTo));

    var jsxPos = jsx3.html.getRelativePosition(null, element);
    jsxPos.T +=2;
    jsxPos.L +=2;
    var jsxPosTo = jsx3.html.getRelativePosition(null, elementTo);

    LOG.debug('drag from pos.t=' + jsxPos.T + ",pos.l=" + jsxPos.L );
    LOG.debug('drag to pos.t=' + jsxPosTo.T + ",pos.l=" + jsxPosTo.L );

    //triggerLeftMouseEvent(element, 'click', true, jsxPos);
    triggerLeftMouseEvent(element, 'mousedown', true, jsxPos);
    triggerLeftMouseEvent(element, 'mousemove', true, jsxPos);

    var posVal = jsxPos;
    posVal.L =  jsxPos.L + Math.round((jsxPosTo.L - jsxPos.L)/2);
    posVal.T = jsxPos.T + Math.round((jsxPosTo.T - jsxPos.T)/2);
    triggerLeftMouseEvent(element, 'mousemove', true, posVal);
    triggerLeftMouseEvent(element, 'mousemove', true, jsxPosTo);
    triggerLeftMouseEvent(element, 'mouseup', true, jsxPosTo);
    //triggerLeftMouseEvent(elementTo, 'mouseup', true, jsxPosTo);
 ...
 */
  ...
  // getJSXByName() does not return the same object as findDescendants(), which access the assoc array directly
   if (this.jsxNamespace != null && this.getCurrentWindow()[this.jsxNamespace]) {
...
IncludeCommand.prepareUrl = function(includeUri, location) {
    ...
    if (document.URL.match(/^chrome:/)) {
        // special case of selenium IDE
        // takes the baseUrl parameter as base url
        baseUrl = uc.getQueryParameter("baseURL");
        // adds a special seleniumRoot variable if exists
        // TODO look for selenium-methods to get this array
        //      this array beeing global may change in any next selenium release
        if (storedVars["seleniumRoot"]) {
            baseUrl += storedVars["seleniumRoot"];
        }
    }
