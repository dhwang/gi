...
//
    ...
    post: function () {
        ...
        // move numTestTotal and suite to the front. --gitak
    ...
    _saveToFile: function (fileName, form) {
        ...
        // TODO - merge the fix for SEL-470 , from rev 1802  07-06-2007 --gitak
        // DGF get CSS
        var styles = "";
        try {
            var styleSheetPath = window.location.pathname.replace(/[^\/\\]+$/, "selenium-test.css");
        ...
        // TODO -- fail tests rows added. Need to add passed tests also -- gitak
        scriptFile.WriteLine("</table><table id=\"faile_tests\">");
        for (var i = 0;  i < this.metrics.failedTests.length; i++) {
...
objectExtend(HtmlRunnerTestLoop.prototype, {
    ...
    nextCommand : function() {
        this.cmdStartTime = new Date(); // move here because commandStarted pause per set interval before next command
    ...
    commandComplete : function(result) {
        ...
        if (com == 'open'  || (com.search(/^waitFor/) != -1)){
            var elapse = this.cmdEndTime - this.cmdStartTime;
            this.currentRow.setMessage('used ' + elapse +' ms');
        }
    },
    ...
    _recordFailure : function(errorMsg) {
        ...
        // TODO - shouldnt refer to headerRow directly.
        var command = this.currentCommand;
        var strFailedTest = getText(this.htmlTestCase.headerRow.trElement) + " | " + command.command + " | " + command.target + " | "
                + command.value + " | " + errorMsg;
        LOG.debug(strFailedTest);
