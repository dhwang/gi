...
jsx3.Class.defineClass("jsx3.ide.mapper.Mapper", jsx3.gui.Block, null, function(Mapper, Mapper_prototype) {
  ...
  var ADDIN = jsx3.ide.mapper.ADDIN;
  ...
  Mapper.ns = {};
  Mapper.ns.wsdl = "http://schemas.xmlsoap.org/wsdl/";
  Mapper.ns.schema1999 = "http://www.w3.org/1999/XMLSchema";
  Mapper.ns.schema2001 = "http://www.w3.org/2001/XMLSchema";
  Mapper.ns.schemaInstance1999 = "http://www.w3.org/1999/XMLSchema-instance";
  Mapper.ns.schemaInstance2001 = "http://www.w3.org/2001/XMLSchema-instance";
  Mapper.ns.soap = "http://schemas.xmlsoap.org/wsdl/soap/";
  Mapper.ns.soapencoding = "http://schemas.xmlsoap.org/soap/encoding/";
  Mapper.ns.soapenvelope = "http://schemas.xmlsoap.org/soap/envelope/";
  //Mapper.ns.wsdl2 = "http://www.w3.org/2005/05/wsdl/";
  ...
  Mapper_prototype.showLast = function() {
    ...
    if (this._showtimeout == null) {
      ...
      this._showtimeout = window.setTimeout(function() {
        ...
        jsx3.ide.scrollDownToLastDomChild(me.getOut());
      }, 0);
    }
  };
 ...
 /**
  ...
  /**
   * Returns true if @strType is a Schema-defined Simple Type (as defined by <code>jsx3.ide.mapper.Mapper.ns.schema2001</code>)
  ...
  /**
   * recursively traverses up the WSDL/Schema tree to find a namespace declaration on an ancestor node matching the prefix for <code>jsx3.ide.mapper.Mapper.ns.schema2001</code>
   * @param objNode {jsx3.xml.Entity} XML Entity (Element)
  ...
  Mapper_prototype._isSimpleNode = function(objNode,strType) {
    for (var i=objNode.getAttributes().iterator(); i.hasNext(); ) {
      var objAtt = i.next();
      if (objAtt.getBaseName() == strType && objAtt.getValue() == Mapper.ns.schema2001) return true;
    }
 ...
 /**
   * Loads the WSDL and tries to load all referenced WSDL imports and Schema imports to create a single combined XML document
   ...
   */
  Mapper_prototype.openWSDL = function(strBaseURL) {
    //resolve the URI
    ...
    if (! objDoc.hasError()) {
      ...
      if (strMyURI == Mapper.ns.wsdl) {
        Mapper._LOG.trace("Document Type: WSDL (" + Mapper.ns.wsdl + ")",{instance:this});
        ...
        //get wsdl:imports and append to the master
        ...
        for (var i=objImportNodes.iterator(); i.hasNext(); ) {
          ...
          if (objError.code == 0) {
            //to support improper implementations of the standard, transfer any XSL element straight over
                ...
                if (oTNode.getNodeType() == jsx3.xml.Entity.TYPEELEMENT) {
                  if (oTNode.getBaseName() != "import") {
                    ...
                    if (oClone != null && oClone instanceof jsx3.xml.Entity) {
                      objDoc.insertBefore(oClone,objImportNode);
                    } else {
                      Mapper._LOG.error("Import Failure. Could not resolve:\n" + oTNode,{instance:this});
                    }
                  } else {
                    Mapper._LOG.error("Nested imports are not allowed. If the following import is necessary, it must be referenced within the starting document, " + strBaseURL + ", NOT within, " + strImportURL + ".\n         " + oTNode,{instance:this});
                  }
                }
              }
            }
        ...
        //TO DO: validate that this doesn't break a WSDL that doesn't use types -- shouldn't be an issue due to absolute addressing
        var objMyType = objDoc.selectSingleNode("/wsdl:definitions/wsdl:types");
        if (!objMyType) {
          //the section does not exist in the wsdl. load here
          var objNewType =  objDoc.getRootNode().createNode(jsx3.xml.Entity.TYPEELEMENT,"wsdl:types",Mapper.ns.wsdl);
          objDoc.getRootNode().appendChild(objNewType);
        }
        //if the contained schema references any imports, load them here
        this.loadSchemaImports(objBaseURI,objDoc,"//xsd:schema/xsd:import[@schemaLocation]","/wsdl:definitions/wsdl:types");

        //set input type as wsdl
        this.setInputType(Mapper.TYPE_WSDL);
      } else if (strMyURI == Mapper.ns.schema2001 || strMyURI == Mapper.ns.schema1999) {
        Mapper._LOG.trace("Document Type: Schema (" + strMyURI + ")",{instance:this});
        if (strMyURI == Mapper.ns.schema1999) Mapper._LOG.error("Outdated Schema Version: '" + Mapper.ns.schema1999 + "'. Must update to version, '" + Mapper.ns.schema2001 + "'. The parse will continue, but may fail.",{instance:this});
        ...
        //if the contained schema references any imports, load them here
        this.loadSchemaImports(objBaseURI,objDoc,"/xsd:schema/xsd:import[@schemaLocation] | /xsd:schema/xsd:include[@schemaLocation]","/xsd:schema");
      } else {
 ...
 /**
   * Loads any referenced Schemas (as imports/includes)
   * @param objBaseURI {jsx3.net.URI} URI object
   * @param objDoc {jsx3.xml.Document} Source document to use to create the rules tree (a schema or WSDL document type for now)
   * @param strImportQuery {String} XSL Query to apply to objDoc to find all possible schema imports/includes
   * @param strParentQuery {String} XSL Query to apply to objDoc to find the parent node to bind all imported schema docs into
   */
  Mapper_prototype.loadSchemaImports = function(objBaseURI,objDoc,strImportQuery,strParentQuery) {
    //get the parent to which all imported XSDs will be appended
    var objParent = objDoc.selectSingleNode(strParentQuery);

    //get imports/includes
    var objImportNodes = objDoc.selectNodes(strImportQuery);
    for (var i=objImportNodes.iterator(); i.hasNext(); ) {
      //get import for this iteration
      var objImportNode = i.next();

      //get true URL for import (may be relative to path for the WSDL)
      var strImportURL = objImportNode.getAttribute("schemaLocation");
      if (!this.isAbsoluteURL(strImportURL)) strImportURL = objBaseURI.resolve(strImportURL).toString();
      ...
      var objError = objAppend.getError();
      if (objError.code == 0) {
        objParent.appendChild(objAppend.getRootNode());
      } else {
        //update the splitter, so the error log is given 20% to show users any parsing errors supplemental to the following alert
        this.getChild(0).setSubcontainer1Pct("80%",true);
        Mapper._LOG.error("The Schema import/include located at '" + objBaseURI.getPath() + "' could not be parsed and added to the WSDL. Please make sure the document exists and that it is a valid schema document.",{instance:this});
      }
    }
  };
 ...
 /**
  ...
  Mapper_prototype.parseWSDLDelay = function(strURL,bDeep) {
    //open the WSDL
    var objWSDL = this.openWSDL(strURL);
 ...
 /**
  ...
  Mapper_prototype.parseDocumentsDelay = function(strOutURL,strInURL) {
    ...
    o.type = "I";
    o.jsxopen = "1";
    var nodeArray = [this.getRulesTree().insertRecord(o,strId,false)];
    ...
    for (var i=0;i<2;i++) {
      if (arguments[i]) {
        ...
        var objWSDL = this.openWSDL(arguments[i]);
        //check if the parse was successful
        if (this.getInputType()) {
          //set reference to the WSDL for this iteration
          this.addSourceDocument(objWSDL,arguments[i]);
          ...
          if (this.getInputType() == Mapper.TYPE_SCHEMA) {
            ...
            this.parseGlobals(objWSDL,nodeArray[i],arguments[i]);
          } else if (this.getInputType() == Mapper.TYPE_XML) {
            //parse the xml document
            this.parseXML(objWSDL,nodeArray[i],i==0,arguments[i]);
          }
        }
      }
    }
      ...
      if (strURL != Mapper.NEW_FILE_PATH) {
        ...
        if (objSources.size() == 0) {
          ...
          for (var i=objSources.iterator(); i.hasNext(); ) {
            ...
            this.addSourceDocument(this.openWSDL(strWSDLURL),strWSDLURL);
          }
        }
 ...
 /**
  ...
  Mapper_prototype.parseServices = function() {
    ...
    if (objMap[Mapper.ns.soap]) {
      var strMyQuery = "/" +  objMap[Mapper.ns.wsdl] + ":definitions/" +
                              objMap[Mapper.ns.wsdl] + ":service/" +
                              objMap[Mapper.ns.wsdl] + ":port/" +
                              objMap[Mapper.ns.soap] + ":address";
      ...
      if (objSOAPServices.size() > 0) {
        ...
        for (var i=objSOAPServices.iterator(); i.hasNext(); ) {
          ...
          var strMyQuery = "/" + objMap[Mapper.ns.wsdl] + ":definitions/" + objMap[Mapper.ns.wsdl] + ":binding[@name='" + strBindingName + "']";
      ...
      Mapper._LOG.error("The WSDL located at '" + strWSDLURL + "' does not contain a valid reference to a supported version of SOAP. At this time only the following namespace(s) are supported: '" + Mapper.ns.soap + "'.",{instance:this});
    }
 ...
 /**
  ...
  Mapper_prototype.getBindingStyle = function(objBinding) {
    ...
    //query the WSDL for the available SOAP services (endpoints)
    var strMyQuery = ".//" +  objMap[Mapper.ns.soap] + ":*[@style='document']";
 ...
 /**
  ...
  Mapper_prototype.parseOperations = function(objBinding,strCDFParentId,strLocation) {
    ...
    var objMap = this.getNSMap();
    var objOperations = objBinding.selectNodes(objMap[Mapper.ns.wsdl] + ":operation",objMap);
    ...
    for (var i=objOperations.iterator(); i.hasNext(); ) {
      ...
      var objSOAPOperation = objOperation.selectSingleNode(objMap[Mapper.ns.soap] + ":operation",objMap);
 ...
 /**
  ...
  Mapper_prototype.parseMessages = function(objOpNode,objMap) {
    ...
    var strMyQuery = "/" +  objMap[Mapper.ns.wsdl] + ":definitions/" +
                            objMap[Mapper.ns.wsdl] + ":portType/" +
                            objMap[Mapper.ns.wsdl] + ":operation[@name='" + strOpName + "']";
 ...
 /**
  ...
  Mapper_prototype.setMessageEncoding = function(objProfile,MESSAGETYPE,strOperationName,objMap) {
    ...
    var strMyQuery = "/" +  objMap[Mapper.ns.wsdl] + ":definitions/" +
                            objMap[Mapper.ns.wsdl] + ":binding/" +
                            objMap[Mapper.ns.wsdl] + ":operation[@name='" + strOperationName + "']/" +
                            objMap[Mapper.ns.wsdl] + ":" + MESSAGETYPE + "/" +
                            objMap[Mapper.ns.soap] + ":body"
  ...
  Mapper_prototype.getMEPElement = function(objOperation,TYPE,objMap) {
    var objMsg;
    if ((objMsg = objOperation.selectSingleNode(objMap[Mapper.ns.wsdl] + ":" + TYPE,objMap)) != null)
 ...
 /**
  ...
  Mapper_prototype.parseMessage = function(MESSAGETYPE,strOperationName,strOperationId,objOperation,strDescription,objMap) {
    ...
    if ((strMessageName = this.getMEPElement(objOperation,MESSAGETYPE,objMap)) != null) {
      ...
      var strMyQuery = "/" +  objMap[Mapper.ns.wsdl] + ":definitions/" + objMap[Mapper.ns.wsdl] + ":message[@name='" + strMessageName + "']";
 ...
 /**
  ...
  Mapper_prototype.parseParts = function(objMessageWSDL,strCDFParentId,bRPC,objMap) {
    ...
    var strQuery = objMap[Mapper.ns.wsdl] + ":part";
    ...
    for (var i=objParts.iterator(); i.hasNext(); ) {
      ...
      var strElement;
      if (!bRPC && (strElement = objPart.getAttribute("element")) != null && strElement != "") {
        //get matching element being referenced (is a global element as direct child of schema)
        ...
        if (objElementNode != null) {
          ...
          Mapper._LOG.trace("Adding         -Part: " + strElement,{instance:this});
          this.parseComplex(strCDFParentId,objElementNode,true);
        } else {
        ...
        if (strType != null && strType != "") {
          ...
          if (this.isSimpleNode(objPart,strType)) {
            ...
            Mapper._LOG.trace("Adding         -Part: " + o.jsxtext,{instance:this});
            ...
            if (objElementNode != null) {
              //the input for a message is defined by a complex element by the given name
              Mapper._LOG.trace("Adding         -Part: " + strType,{instance:this});
  ...
  Mapper_prototype.findParsableObjects = function(objTree,objWSDL,objElementNode,strQuery,strNodeId,bAttribute,parentNoPass) {
    ...
    for (var i=objElements.iterator(); i.hasNext(); ) {
      ...
      if (!this.isDescendant(objElement,objPrevElement) && (parentNoPass == null || (parentNoPass != null && !this.isDescendantArray(objElement,parentNoPass)))) {
        ...
        if (objElement.getAttribute("type") == null) {
...
/* DO DRILL ***************************************************/
  ...
  Mapper_prototype.doDrill = function(strNodeId,bOpen,objTree,objToggledNode,objElementNode) {
    ...
    if (bOpen) {
      ...
      if (objElementNode || (objToggledNode && objToggledNode.selectNodes("record").size() == 0)) {
        ...
        var objElementNode = (!objElementNode) ? objWSDL.selectSingleNode(myPath,objMap) : objElementNode;
        ...
        if (myType != null && myType != "" && this.isSimpleNode(objElementNode,myType)) {
          ...
          var objElementNode = objWSDL.selectSingleNode(mQuery,objMap);

          //if simple, convert here and exit early
          if (objElementNode == null) {
            //**'SIMPLE TYPE'**
            var objElementNode = objWSDL.selectSingleNode("//" + objMap[this.getSchemaNS()] + ":schema" + myTSQ + "//" + objMap[this.getSchemaNS()] + ":simpleType[@name='" + this.getBaseName(myType) + "']",objMap);
        ...
        if (objExtensionNode != null) {
          ...
          var myType = objExtensionNode.getAttribute("base");
          if (myType != null && myType != "" && !this.isSimpleNode(objExtensionNode,myType)) {
            ...
            var objExtensionNode = objExtensionNode.selectSingleNode("//" + objMap[this.getSchemaNS()] + ":schema" + myTSQ + "/*[@name='" + this.getBaseName(myType) + "']",objMap);
            ...
            if (objExtensionNode != null) {
              ...
              //scan this 'group' tag for child elements/groups
              //3.3 rem: var strQuery = ".//" + objMap[this.getSchemaNS()] + ":group | .//" + objMap[this.getSchemaNS()] + ":attributeGroup | .//" + objMap[this.getSchemaNS()] + ":element";
              //3.3 rem: this.findParsableObjects(objTree,objWSDL,objExtensionNode,strQuery,strNodeId,false);
            }
          }
        }
        ...
        var strQuery = ".//" + objMap[this.getSchemaNS()] + ":attribute";
 ...
 /**
  ...
  Mapper_prototype.parseComplex = function(strCDFParentId,objSchemaNode,bFirst,bDrill,bAttribute) {
    ...
    if ((strMyNodeName = objSchemaNode.getAttribute("name")) == null || strMyNodeName == "") {
      ...
      if (strRefBase == "arrayType" && strRefURI == Mapper.ns.soapencoding) {
        ...
        objRuleParent.setAttribute("ttns",Mapper.ns.soapencoding);
      ...
      if (strMyNodeName != null && strMyNodeName != "" && !this.isSimpleNode(objSchemaNode,strMyNodeName)) {
        ...
        var myTSQ = this.getTargetedSchemaQuery(strMyNodeName,objSchemaNode,objMap);
 ...
 /**
  ...
  Mapper_prototype.setFormDefaults = function(objRNode,objSNode) {
    ...
    var strEFD = objSNode.getAttribute("elementFormDefault");
  ...
  Mapper_prototype.generateCode = function(objMnu) {
    ...
    var strSrc = jsx3.ide.SERVER.getBaseDirectory().relativePathTo(objFile);
    var includes = jsx3.ide.SERVER.getSettings().getMapsInArrayByField('includes','src',strSrc);
    if (includes.length) {
   ...
   * @private
   */
  Mapper_prototype.onRuleDrop = function(objRulesTree,objSourceTree,strDraggedId,strRuleId)  {
    ...
    if (objSourceTree.getName() == "jsxdom" && (objNode = objRulesTree.getRecordNode(strRuleId)) != null && (objDragged = jsx3.GO(strDraggedId)) != null) {
    ...
    } else if (objSourceTree.getName() == "jsx_schema_rulestree" && strDraggedId != strRuleId) {
      ...
      if (strDstType == "I" || strDstType == "O" || strDstType == "F" || strDstType == "C" || strDstType == "E" || strDstType == "A" || strDstType == "D") {
         if (strSrcType == "A" || strSrcType == "C" || strSrcType == "E" || strSrcType == "D") {
           ...
           if (strDstType == "E" || strDstType == "A" || strDstType == "D") objDst.setAttribute("type","C");

           //perform the actual adoption and redraw the new parent
           objRulesTree.adoptRecord(objRulesTree.getId(),strDraggedId,strRuleId,false);
           objRulesTree.redrawRecord(strRuleId);
   ...
   * @param TYPE {String} one of: mappings, restrictions
   * @private
   */
  ...
  Mapper_prototype.doRemoveBranch = function() {
    ...
    if (objTree.getRecord(strParentId)) objTree.setValue(strParentId);
   ...
   * @param MAPTYPE {String} one of: CDF Attribute,
   * @private
   */
...
/*
          var mxs = objContent.getDescendantsOfType(jsx3.gui.Matrix);
          for (var i=0;i<mxs.length;i++) {
            mxs[i].clearBoxProfile(true);
            mxs[i].repaint();
          }
*/
      ...
      if (objNode) {
        ...
        if (objSearchFrom) {
          objSearchFrom.findDescendants(function(objJSX) {
            if (objJSX.isInstanceOf("jsx3.gui.TextBox") || objJSX.isInstanceOf("jsx3.gui.Select")) {
  ...
  Mapper_prototype.readMappings = function(objTree,objNode,strAttName) {
    ...
    var objRNode = objNode.selectSingleNode(strAttName);
      if (objRNode) {
        //load all restrictions into the map grid editor
        objMtx.setXMLId(objTree.getXMLId());
        objMtx.setRenderingContext(objRNode.getAttribute("jsxid"),true);
      } else {
        objMtx.setRenderingContext("jsxnull",true);
      }
      objMtx.repaintData();
  };
  ...
  Mapper_prototype.browseURL = function(strJSXId) {
    ...
    //shows the open dialog, so user can choose a file path; persists ref to the path in the CDF for the rules tree
    ...
    dialog.onExecute = function(objFile) {
      var objJSX = me.getDescendantOfName(strJSXId);
      objJSX.setValue(jsx3.net.URI.decode(jsx3.ide.SERVER.getBaseDirectory().relativePathTo(objFile)));
      jsx3.ide.setCurrentDirectory(objFile.getParentFile());
    };
    dialog.openForOpen(jsx3.ide.getCurrentDirectory(), null);
  };
  ...
  Mapper_prototype.insertRow = function(TYPE,objRecord) {
    var objTree = this.getRulesTree();
    var objNode = objTree.getRecordNode(objTree.getValue());
    this.bindComplexRule(objTree,objNode,TYPE,objRecord?objRecord.name:"",objRecord?objRecord.value:"");
    this.readMappings(objTree,objNode,TYPE);
    if (objNode.getParent().getAttribute("jsxid") != "jsxroot") {
      objTree.redrawRecord(objNode.getParent().getAttribute("jsxid"),jsx3.xml.CDF.UPDATE);
    } else {
      this.getRulesTree().repaint();
    }
    ...
    //return false to cancel the autorow event for the matrix isntance that called this method (stops the append)
    return false;
  };
    ...
    if (objParent) {
      ...
      this.getAncestorOfType(jsx3.gui.Dialog).confirm(
        ...
        function(d){
          ...
          var objChildren = objParent.selectNodes("record");
          for (var i=objChildren.size()-1;i>=0;i--)
            objParent.removeChild(objChildren.item(i));
          objTree.redrawRecord(objParent.getAttribute("jsxid"),jsx3.xml.CDF.UPDATE);
