...
jsx3.ide.onDomChange = function(bNoEvent) {
  ...
  if (objTree != null && jsx3.ide.ComponentEditor && jsx3.ide.getActiveEditor() instanceof jsx3.ide.ComponentEditor) {
    ...
    if (selectedIds.length > 0 && newValues.length == 1)
      jsx3.ide.triggerDomTreeSelect(objTree, null);
...
//  jsx3.ide.LOG.warn("doRecycle obj:" + objJSX);
  ...
  if (objJSX.length == 1) {
    ...
    var childCount = objJSXParent.getChildren().length;

    jsx3.ide.setDomValue(childCount > 1 ? objJSXParent.getChild(Math.max(0, intIndex - 1)) : objJSXParent);
  } else {
...
//  jsx3.log("setDomValue " + objJSX.getName());
  if (objTree == null) objTree = jsx3.IDE.getJSXByName("jsxdom");
  if (!objJSXs) objJSXs = [];
