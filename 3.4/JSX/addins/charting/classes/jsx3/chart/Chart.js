...
jsx3.require("jsx3.chart.ChartComponent", "jsx3.xml.Cacheable", "jsx3.chart.Series");
 ...
 * </ul>
 */
jsx3.Class.defineClass("jsx3.chart.Chart", jsx3.chart.ChartComponent, [jsx3.xml.Cacheable, jsx3.xml.CDF], function(Chart, Chart_prototype) {
  ...
  Chart.QUADRANTS = {top: 1, right: 1, bottom: 1, left: 1};
  ...
  Chart_prototype.setTitlePlacement = function( titlePlacement ) {
    if (Chart.QUADRANTS[titlePlacement]) {
  ...
  Chart_prototype.setLegendPlacement = function( legendPlacement ) {
    if (Chart.QUADRANTS[legendPlacement]) {
  ...
  Chart_prototype.getDataCanvas = function() {
    return this.fetchTransient("trans_dataCanvas");
  };
  ...
  Chart_prototype.getDataProvider = function() {
    return this.fetchTransient("trans_dp");
  };
   ...
   * @jsxobf-clobber
   */
  Chart_prototype.updateDataProvider = function(xml) {
    if (xml != null) {
      this.storeTransient("trans_dp", xml.selectNodes("/data/record").toArray());
    } else {
      this.clearTransient("trans_dp");
    }
  };
  ...
  Chart_prototype.getCanvasClass = function() {
    return vector.Canvas.jsxclass;
  };
   ...
   * @private
   */
  Chart_prototype.updateView = function() {
    this.jsxsuper();
    ...
    if (isNaN(width) || isNaN(height)) {
      var objImplicit = this.getParent().getClientDimensions(this);
      if (width.toString().indexOf("%") >= 0)
        width = Math.round(objImplicit.parentwidth * parseFloat(width) / 100);
      if (height.toString().indexOf("%") >= 0)
        height = Math.round(objImplicit.parentheight * parseFloat(height) / 100);
    ...
    if (chartTitle != null && chartTitle.getDisplay() != jsx3.gui.Block.DISPLAYNONE) {
      ...
      contentCanvas.appendChild(chartTitle.getRenderRoot());
    ...
    this.storeTransient("trans_dataCanvas", dataCanvas);
    ...
    if (legend != null && legend.getDisplay() != jsx3.gui.Block.DISPLAYNONE) {
      ...
      legend.updateView();
      contentCanvas.appendChild(legend.getRenderRoot());
  ...
  Chart_prototype.toString = function() {
  ...
  /** 
   * by default allow size editing and moving of charts in GI
   */
  Chart_prototype.getMaskProperties = function() {
    return this.getRelativePosition() == jsx3.gui.Block.ABSOLUTE ?
      chart.ChartComponent.MASK_PROPS_ALLEDIT : chart.ChartComponent.MASK_PROPS_ALLEDITREL;
  };

  Chart_prototype.onDestroy = function(objParent) {
    this.jsxsuper(objParent);
    this.onDestroyCached(objParent);
  };

  Chart_prototype.createBoxProfile = function(objImplicit) {
  ...
  Chart_prototype.updateBoxProfile = function(objImplicit, objGUI, objQueue) {
    if (objImplicit.width == null && objImplicit.height == null) {
      ...
      if (width.toString().indexOf("%") >= 0 || height.toString().indexOf("%") >= 0) {
        ...
        var root = this.getRenderRoot();
        if (objGUI != null && (width != root.getWidth() || height != root.getHeight()) && width > 0 && height > 0)
          objQueue.addRepaint(this);
      }
    }
  };
