...
jsx3.Class.defineClass("jsx3.chart.GridLines", jsx3.chart.ChartComponent, null, function(GridLines, GridLines_prototype) {
  ...
  GridLines_prototype.updateView = function() {
    ...
    this.updateViewDirection(xLines, w, h, this.getXAxis(), this.fillV, 
      this.strokeMajorV, this.strokeMinorV, false);
    // render the horizontal lines
    this.updateViewDirection(yLines, w, h, this.getYAxis(), this.fillH, 
   ...
   * @jsxobf-clobber
   */
  GridLines_prototype.updateViewDirection = function(group, w, h, axis, fills, majorStrokes, minorStrokes, vert) {
    group.setDimensions(0, 0, w, h);
    
    var numFills = this.getArrayLength(fills);
    ...
    if (axis != null) {
      ...
      if (ticks.length == 0 || numFills == 1) {
        this.attachRectangle(group, 0, 0, w, h, this.getFillFromArray(fills, 0));
      } 
      
      if (ticks.length > 0) {
        if (numFills > 1)
          this.attachFills(group, axis, ticks, fills, null, vert);
        this.attachTicks(group, axis, ticks, majorStrokes, null, vert);
        this.attachMinorTicks(group, axis, ticks, minorStrokes, vert);
      }
    } else {
   ...
   * @jsxobf-clobber
   */
  GridLines_prototype.attachFills = function( group, axis, ticks, fills, shapes, vert ) {
    if (shapes == null)
      shapes = new Array(this.getArrayLength(fills));
    ...
    for (var i = 0; i <= ticks.length; i++) {
      ...
      if (shape == null) {
        ...
        shape.setFill(this.getFillFromArray(fills, i));
      ...
      if (i == ticks.length) {
        ...
        if (ticks[i-1] < maxLength)
          this.addRectangle(shape, ticks[i-1], 0, maxLength, crossLength, vert);
      } else if (i == 0) {
        // do first fill
        if (ticks[i] > 0)
          this.addRectangle(shape, 0, 0, ticks[i], crossLength, vert);
      } else {
        // do middle fill
        this.addRectangle(shape, ticks[i-1], 0, ticks[i], crossLength, vert);
      }
    }
  };
   ...
   * @private
   */
  GridLines_prototype.addRectangle = function( shape, x1, y1, x2, y2, vert ) {
    if (vert)
   ...
   * @jsxobf-clobber
   */
  GridLines_prototype.attachTicks = function( group, axis, ticks, strokes, shapes, vert ) {
    if (shapes == null)
      shapes = new Array(this.getArrayLength(strokes));
    ...
    for (var i = 0; i < ticks.length; i++) {
      ...
      if (shape == null) {
        ...
        shape.setStroke(this.getStrokeFromArray(strokes, i));
        group.appendChild(shape);
      }
   ...
   * @jsxobf-clobber
   */
  GridLines_prototype.attachMinorTicks = function( group, axis, ticks, strokes, vert ) {
    var numShapes = this.getArrayLength(strokes);
    if (numShapes == 0) return;
    
    var shapes = new Array(numShapes);
    
    for (var i = 0; i < ticks.length; i++) {
      var minorTicks = axis.getMinorTicks(ticks, i);
      this.attachTicks(group, axis, minorTicks, strokes, shapes, vert);
    }
  };
   ...
   * @jsxobf-clobber
   */
  GridLines_prototype.attachRectangle = function(group, l, t, w, h, fill, zIndex) {
    if (fill != null) {
  ...
  GridLines_prototype.toString = function() {
    return "[GridLines '" + this.getName() + "']";
  };
   ...
   * @private
   */
  GridLines_prototype.getFillFromArray = function( a, index ) {
   ...
   * @private
   */
  GridLines_prototype.getStrokeFromArray = function( a, index ) {
    if (a == null) return null;
   ...
   * @private
   */
  GridLines_prototype.getArrayLength = function( a ) {
    if (a == null) return 0;
