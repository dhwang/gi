...
jsx3.Class.defineClass("jsx3.chart.LineSeries", jsx3.chart.Series, null, function(LineSeries, LineSeries_prototype) {
   ...
   * @jsxobf-clobber
   */
  LineSeries.FORMS = {segment: 1, step: 1, reverseStep: 1, horizontal: 1, vertical: 1};
   ...
   * @final @jsxobf-final
   */
  LineSeries.DEFAULT_POINT_RADIUS = 4;
   ...
   * @jsxobf-clobber
   */
  LineSeries.LegendRenderer = function(series) {
    this.series = series;
  };

  LineSeries.LegendRenderer.prototype.render = function(x1, y1, x2, y2, fill, stroke) {
    var w = x2 - x1; 
    var h = y2 - y1;
    ...
    if (renderer != null) {
      ...
      var pointFill = this.series.getDisplayedPointFill();
      var pointStroke = this.series.getDisplayedPointStroke();
  ...
  LineSeries_prototype.getPointRadius = function() {
    return this.pointRadius != null ? this.pointRadius : LineSeries.DEFAULT_POINT_RADIUS;
  };
  ...
  LineSeries_prototype.getPointRenderer = function() {
    return this.getReferenceField("pointRenderer");
  };
  ...
  LineSeries_prototype.setPointRenderer = function( pointRenderer ) {
    this.setReferenceField("pointRenderer", pointRenderer);
  };
   ...
   * @private
   */
  LineSeries_prototype.getDisplayedPointFill = function() {
    var fill = this.fetchTransient("trans_pointFill");
    if (fill == null)
      this.storePointFillAndStroke();
   ...
   * @private
   */
  LineSeries_prototype.getDisplayedPointStroke = function() {
   ...
   * @jsxobf-clobber
   */
  LineSeries_prototype.storePointFillAndStroke = function() {
  ...
  LineSeries_prototype.updateView = function() {
    ...
    var points = this.getShownPoints();
    for (var i = 0; i < points.length; i++) {
      ...
      this.paintPoint(i, continuous, point[0], point[1], point[2], point[3], lx, ly);
   ...
   * @jsxobf-clobber
   */
  LineSeries_prototype.paintPoint = function(i, continuous, record, x, y, index, lx, ly) {
    var root = this.getRenderRoot();
    ...
    if (renderer != null) {
      ...
      var thisFill = colorFunct != null ? colorFunct.call(null, record, index) : this.getDisplayedPointFill();
      
      var point = renderer.render(x - radius, y - radius, x + radius, y + radius, 
          thisFill, this.getDisplayedPointStroke());
      ...
      if (funct != null)
        point.setToolTip(funct.call(null, this, record));
    }
    
    // figure out if this is the start of a new line segment
    var startSegment = i == 0 ||
   ...
   * @jsxobf-clobber
   */
  LineSeries_prototype.getShownPoints = function() {
  ...
  LineSeries_prototype.addPoint = function(record, x, y, index) {
    this.getShownPoints().push([record, x, y, index]);
  };
  ...
  LineSeries_prototype.getLegendRenderer = function() {
    return new LineSeries.LegendRenderer(this);
  };
  ...
  LineSeries_prototype.toString = function() {
    return "[LineSeries '" + this.getName() + "']";
  };
