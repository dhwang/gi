...
jsx3.Class.defineClass("jsx3.chart.LineChart", jsx3.chart.CartesianChart, null, function(LineChart, LineChart_prototype) {
  ...
  LineChart.ALLOWED_TYPES = {overlay: 1, stacked: 1, stacked100: 1};
  ...
  LineChart_prototype.setType = function( type ) {
    if (LineChart.ALLOWED_TYPES[type]) {
      this.type = type;
    } else {
   ...
   * @private
   */
  LineChart_prototype.updateView = function() {
    this.jsxsuper();
    this.drawSeries();
  };
   ...
   * @private
   */
  LineChart_prototype.drawSeries = function() {
  ...
  LineChart_prototype.toString = function() {
    return "[LineChart '" + this.getName() + "']";
  };
