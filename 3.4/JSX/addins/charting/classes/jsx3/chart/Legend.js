...
jsx3.Class.defineClass("jsx3.chart.Legend", jsx3.chart.ChartComponent, null, function(Legend, Legend_prototype) {
  ...
  Legend.ZINDEX_BG = 1;
  /** @private @jsxobf-clobber @jsxobf-final */
  Legend.ZINDEX_TITLE = 3;
  /** @private @jsxobf-clobber @jsxobf-final */
  Legend.ZINDEX_CONTENT = 2;
  ...
  Legend_prototype.updateView = function() {
    ...
    var titleHeight = (title != null && title.getDisplay() != jsx3.gui.Block.DISPLAYNONE) ?
        title.getPreferredHeight() + Legend.TITLE_SPACE : 0;
    
    // figure out main padding and margins
    var margins = this.getMarginDimensions();
    ...
    contentsView.setZIndex(Legend.ZINDEX_CONTENT);
    ...
    if (this.backgroundFill || this.backgroundStroke) {
      ...
      if (numEntries > 0 || (title != null && title.getDisplay() != jsx3.gui.Block.DISPLAYNONE)) {
        ...
        bgRect.setZIndex(Legend.ZINDEX_BG);
    ...
    if (title != null && title.getDisplay() != jsx3.gui.Block.DISPLAYNONE) {
      ...
      title.setZIndex(Legend.ZINDEX_TITLE);
    ...
    var labelL = l + padding[3] + this.boxHeight + Legend.BOX_SPACE;
    var labelW = insideW - this.boxHeight - Legend.BOX_SPACE;
    ...
    if (entryType == Legend.SHOW_SERIES && numEntries > 0) {
      ...
      for (var i = 0; i < series.length; i++) {
        ...
        var box = this.attachTextBox(contentsView, series[i].getSeriesName(), this.labelClass, this.labelStyle,
      ...
      for (var i = 0; i < dp.length; i++) {
        ...
        var box = this.attachTextBox(contentsView, name, this.labelClass, this.labelStyle,
            labelL, Math.round(top + this.boxHeight/2), labelW);
        top += this.lineHeight;
   ...
   * @jsxobf-clobber
   */
  Legend_prototype.attachTextBox = function( group, text, cssClass, cssStyle, left, top, width) {
  ...
  /**
   * control repaint behavior
   * @private
   */
  Legend_prototype.onResize = function() {
  ...
  Legend_prototype.toString = function() {
    return "[Legend '" + this.getName() + "']";
  };
...
/* @JSC :: end */
  ...
  Legend_prototype._ebClick = function(objEvent, objGUI) {
    ...
    this.doEvent(jsx3.gui.Interactive.SELECT, this.getEventContext(objEvent, strSeriesId, intRecordIndex));
  };
    
  Legend_prototype._ebDoubleClick = function(objEvent, objGUI) {
    var strSeriesId = objGUI.getAttribute("seriesId");
    var intRecordIndex = objGUI.getAttribute("recordIndex");    
    this.doEvent(jsx3.gui.Interactive.EXECUTE, this.getEventContext(objEvent, strSeriesId, intRecordIndex));
  };
  ...
  Legend_prototype.doSpyOver = function(objEvent, objGUI) {
    ...
    this.jsxsupermix(objEvent, objGUI, this.getEventContext(objEvent, strSeriesId, intRecordIndex));
  };
  ...
  Legend_prototype._ebMouseUp = function(objEvent, objGUI) {
    ...
    if (objEvent.rightButton() && (strMenu = this.getMenu()) != null) {
      ...
      if (objMenu != null) {
        var objContext = this.getEventContext(objEvent, strSeriesId, intRecordIndex);
  ...
  Legend_prototype.getEventContext = function(objEvent, strSeriesId, intRecordIndex) {
