...
jsx3.Class.defineClass("jsx3.gui.TimePicker", jsx3.gui.Block, [jsx3.gui.Form], function(TimePicker, TimePicker_prototype) {
  ...
  TimePicker_prototype.getDate = function(objDate) {
    ...
    objDate.setHours(this.jsxhours || Number(0));
    objDate.setMinutes(this.jsxminutes || Number(0));
  ...
  TimePicker_prototype.createBoxProfile = function(objImplicit) {
    ...
    //create outer box
    objImplicit.boxtype = (bRelative) ? "relativebox" : "box";
  ...
  TimePicker_prototype.paint = function() {
    ...
    b1.setAttributes('id="' + this.getId() + '" class="jsx30timepicker" ' + 'label="' + this.getName() + '" ' +
        this.paintTip() + this.renderHandlers(null, 0) + this.renderAttributes(null, true));
    ...
    bSpin.setAttributes(' unselectable="on" class="spinner"');

    //up/down spinner images
    var imageHTML = '<img unselectable="on" style="top:0px;left:0px;position:absolute;'+strImageCursor+'" width="11" height="8" src="'+ TimePicker.SPINNER_UP + '"' +
                    this.renderHandler(Event.CLICK, "doSpinUp", 3) +
                    this.renderHandler(Event.MOUSEDOWN, "doSpinMD", 3)+'/>' +
                    '<img unselectable="on" style="top:8px;left:0px;position:absolute;'+strImageCursor+'" width="11" height="8" src="'+ TimePicker.SPINNER_DOWN + '"' +
  ...
  TimePicker.FIELDS = {
      hour: {
          _increment: function(picker, value) {
            ...
            value++;
            value = picker.is24Hour() ? value % 24 : (value - 1) % 12 + 1;
            return value;
          },
          _decrement: function(picker, value) {
            if (isNaN(value)) value = picker.is24Hour() ? 24 : 13;
            value--;
            value = picker.is24Hour() ? jsx3.util.numMod(value, 24) : jsx3.util.numMod((value - 1), 12) + 1;
            return value;
          },
          ...
          _set: function(picker, value) {
            if (value == null || value === "") {
              picker.jsxhours = null;
            } else if (isNaN(value)) {
              picker.jsxhours = 0;
            } else if (picker.is24Hour() || value == null) {
              picker.jsxhours = Number(value);
            } else {
              ...
              if (ampmField != null && ampmField.value != null && ampmField.value.toLowerCase() == ampm[1].toLowerCase())
                picker.jsxhours = Number(value) + 12;
              else
                picker.jsxhours = Number(value);
            }
          },
          ...
          _next: function(picker) { return picker.getFieldByType("minute"); },
          _previous: function(picker) { return null; },
      ...
      minute: {
          ...
          _decrement: function(picker, value) {
            ...
            value--;
            return jsx3.util.numMod(value, 60);
          },
          _format: function(picker, value) {
            if (value == null) return "";
            return TimePicker._PAD2.format(value);
          },
          _set: function(picker, value) {
            if (isNaN(value)) {
              picker.jsxminutes = 0;
            } else {
              picker.jsxminutes = Number(value);
            }
          },
          _get: function(picker) { return picker.jsxminutes; },
          _next: function(picker) { return picker.getFieldByType(picker.jsxshowsecs ? "second" : "ampm"); },
          _previous: function(picker) { return picker.getFieldByType("hour"); },
          _keydown: function(picker, input, keycode, shift) {
            if (!(keycode >= Event.KEY_0 && keycode <= Event.KEY_9) || shift) return true;
            var val1 = input.value;
            jsx3.sleep(function() {
      ...
      second: {
          ...
          _set: function(picker, value) {
            if (isNaN(value)) {
              ...
              picker.jsxseconds = Number(value);
            }
          },
          _get: function(picker) { return picker.jsxseconds; },
          _next: function(picker) { return picker.getFieldByType(picker.jsxshowmillis ? "milli" : "ampm"); },
          _previous: function(picker) { return picker.getFieldByType("minute"); },
          _keydown: function(picker, input, keycode, shift) {
      ...
      milli:  {
          ...
          _decrement: function(picker, value) {
            ...
            value--;
            return jsx3.util.numMod(value, 1000);
          },
          _format: function(picker, value) {
            if (value == null) return "";
            return TimePicker._PAD3.format(value);
          },
          _set: function(picker, value) {
            if (isNaN(value)) {
              picker.jsxmillis = 0;
            } else {
              picker.jsxmillis = Number(value);
            }
          },
          _get: function(picker) { return picker.jsxmillis; },
          _next: function(picker) { return picker.getFieldByType("ampm"); },
          _previous: function(picker) { return picker.getFieldByType("second"); },
          _keydown: function(picker, input, keycode, shift) {
            if (!(keycode >= Event.KEY_0 && keycode <= Event.KEY_9) || shift) return true;
            var val1 = input.value;
            jsx3.sleep(function() {
      ...
      ampm: {
          ...
          _format: function(picker, value) { return value; },
          _set: function(picker, value) {
            var ampm = picker._getLocaleProp("time.ampm");
            var hourValue = Number(picker.getFieldByType("hour").value);
            if (! isNaN(hourValue)) {
              if (value != null && value.toLowerCase() == ampm[1].toLowerCase())
                picker.jsxhours = hourValue + 12;
              else
                picker.jsxhours = hourValue;
            }
          },
          _get: function(picker) { return picker._getLocaleProp("time.ampm")[picker.jsxhours < 12 ? 0 : 1]; },
          _next: function(picker) { return null; },
  ...
  TimePicker_prototype.doKeyDownField = function(objEvent, objGUI) {
    ...
    if (keyCode == Event.KEY_ARROW_UP || keyCode == Event.KEY_ARROW_DOWN) {
      ...
      var value = field[inc](this, objGUI.value);
      if (this.doEvent(Interactive.CHANGE, {objEVENT:objEvent, strFIELD:fieldName}) !== false) {
        objGUI.value = field._format(this, value);
        field._set(this, value);
      }
...
//      jsx3.log(keyCode + " " + String.fromCharCode(keyCode));
  ...
  TimePicker_prototype.getFieldByType = function(strType) {
    ...
    if (objGUI != null) {
      var inputs = objGUI.childNodes[0].childNodes;
      for (var i = 0; i < inputs.length; i++)
        if (inputs[i].getAttribute("jsxfield") == strType)
          return inputs[i];
    }
    return null;
  };
