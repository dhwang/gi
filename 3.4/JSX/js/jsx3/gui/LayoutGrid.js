...
/* @JSC :: end */
   ...
   * @package
   */
  LayoutGrid_prototype.getClientDimensions = function(objChild) {
    ...
    if (rows.length == 0 || cols.length == 0)
      return {boxtype:"box", width:"100%", height:"100%"};

    var colIndex = intIndex % cols.length;
    var rowIndex = Math.floor(intIndex / cols.length);

    var h = rowIndex < rows.length ? rows[rowIndex] : 0;
    var w = cols[colIndex];

    var l = 0, t = 0;
    for (var i = 0; i < rowIndex && i < rows.length; i++)
      t += rows[i];
    for (var i = 0; i < colIndex && i < cols.length; i++)
      l += cols[i];
  ...
  LayoutGrid_prototype.updateBoxProfile = function(objImplicit, objGUI, objQueue) {
    ...
    var rows = this._getTrueSizeArray(true);
    var cols = this._getTrueSizeArray(false);

    var intTop = 0, intLeft = 0;
    var c = 0, numKids = objKids.length;

    for (var i = 0; i < rows.length; i++) {
      intLeft = 0;
      for (var j = 0; j < cols.length && c < numKids; j++) {
        var objChild = objKids[c++];

        objQueue.add(objChild, {boxtype:"box", left:intLeft, top:intTop, width:"100%", height:"100%",
            parentwidth:cols[j], parentheight:rows[i]}, (objGUI) ? objGUI.childNodes[c-1] : null, true);
        intLeft += cols[j];
      }
      intTop += rows[i];
    }
    for (var i = c; c < numKids; c++) {
      var objChild = objKids[c];
      objQueue.add(objChild, {boxtype:"box", left:0, top:0, width:0, height:0, parentwidth:0, parentheight:0},
          (objGUI) ? objGUI.childNodes[c] : null, true);
    }
  };
  ...
  LayoutGrid_prototype.createBoxProfile = function(objImplicit) {
    ...
    //create the box
    if(objImplicit.boxtype == null) objImplicit.boxtype = (bRelative || objImplicit.left == null || objImplicit.top == null) ? "relativebox" : "box";
  ...
  LayoutGrid_prototype.paint = function() {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      for (var j = 0; j < cols.length && c < numKids; j++) {
        var objChild = objKids[c++];
        objChild.syncBoxProfileSync({left:intLeft, top:intTop, width:"100%", height:"100%",
            parentwidth:cols[j], parentheight:rows[i], boxtype:"box"});
        intLeft += cols[j];
      }
      intTop += rows[i];
    }
