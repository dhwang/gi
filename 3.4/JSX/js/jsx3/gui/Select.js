...
/* @JSC :: end */
  ...
  jsx3.html.loadImages(Select.ARROWICON, Select.OVERIMAGE, Select.SELECTEDIMAGE);
...
/* @JSC :: end */
  ...
  Select_prototype.init = function(strName,vntLeft,vntTop,vntWidth,vntHeight,strSelectedValue) {
    ...
    if (strSelectedValue != null) this.jsxvalue = strSelectedValue;
  };
  ...
  Select_prototype.getXSL = function() {
    return this._getSharedXSL(Select.DEFAULTXSLURL);
  };
  ...
  Select_prototype.paintBackgroundColor = function() {
    var bgc = this.getEnabled() != jsx3.gui.Form.STATEDISABLED ?
              this.getBackgroundColor() || Select.DEFAULTBACKGROUNDCOLOR :
  ...
  Select_prototype._ebKeyDownSelect = function(objEvent, objGUI) {
    ...
    if ((objEvent.spaceKey() || objEvent.enterKey()) && srcElm.getAttribute("jsxid") != null) {
      ...
      if (bMod) return;
      if (srcElm == srcElm.parentNode.lastChild) {
        Select.doFocus(srcElm.parentNode.firstChild.nextSibling); // beware the space at #0
      } else {
        Select.doFocus(srcElm.nextSibling);
      }
    } else if (objEvent.upArrow()) {
      if (bMod) return;
      if (srcElm == srcElm.parentNode.firstChild.nextSibling) {
        Select.doFocus(srcElm.parentNode.lastChild);
      } else {
        Select.doFocus(srcElm.previousSibling);
      }
    } else if (objEvent.tabKey()) {
      if (objEvent.hasModifier(true)) return;
      this._doSelectRecord(objEvent, srcElm.getAttribute("jsxid"));
      jsx3.html[objEvent.shiftKey() ? "focusPrevious" : "focusNext"](this.getRendered(objEvent), objEvent);
      this.hide(false);
      return;
    } else {
      return;
  ...
  Select_prototype._ebKeyDownCombo = function(objEvent, objGUI) {
    ...
    if (!bMod && (objEvent.downArrow() || objEvent.enterKey())) {
      ...
      if (!objEvent.enterKey() && hw != null && hw.getRendered(objEvent).childNodes[0].getAttribute("jsxselid") == this.getId()) {
        Select.doFocus(hw.getRendered(objEvent).childNodes[0].childNodes[1]);
      } else {
        var filter = objEvent.enterKey() ? "" : this.getText();
        this._show(filter);
      }
      objEvent.cancelAll();
    } else if (!bMod && objEvent.tabKey()) {
      var hw = jsx3.gui.Heavyweight.GO(Select._HW_ID);
      if (hw != null && hw.getRendered(objEvent).childNodes[0].getAttribute("jsxselid") == this.getId()) {
        var firstDiv = hw.getRendered(objEvent).childNodes[0].childNodes[1];
        if (firstDiv) {
          var recordId = firstDiv.getAttribute("jsxid");
          if (recordId != null)
            this._doSelectRecord(objEvent, recordId);
        }
        this.hide(true);
        objEvent.cancelAll();
      }
    } else if (objEvent.tabKey() && objEvent.shiftKey() && !objEvent.hasModifier(true)) {
      jsx3.html.focusPrevious(this.getRendered(objEvent), objEvent);
  ...
  /**
   * if implemented by the XSLT, the focus event for an option will call this method
   * @param objGUI {HTMLElement} on-screen HTML element to focus (add a bg image to)
   * @private
   */
  Select.doFocus = function(objGUI) {
    var attr = objGUI.getAttribute("jsxid");
    if (attr != null) {
      objGUI.focus();
  ...
  /**
   * if implemented by the XSLT, the blur event for an option will call this method
   * @param objGUI {HTMLElement} on-screen HTML element to blur (remove the bg image from)
   * @private
   */
  Select.doBlur = function(objGUI) {
  ...
  Select_prototype._doBlurCombo = function(objEvent, objGUI) {
    ...
    var objNode = this.getXML().selectSingleNode('//record[@jsxtext="' + strText.replace(/"/g,'&quot') +
        '" or (not(@jsxtext) and @jsxid="' + strText.replace(/"/g,'&quot') + '")]');
    if (objNode != null) {
...
/* @JSC :: end */
  ...
  Select_prototype._show = function(strFilter) {
    ...
    if (objGUI != null) {
      ...
      var spacer = '<div style="height:1px;width:'+(intTrueWidth-4)+'px;position:relative;left:0px;top:0px;' +
          'padding:0px;margin:0px 0px -1px 0px;overflow:hidden;">&#160;</div>';

      //generate DHTML for child option group
      var strHTML = '<div jsxselid="' + this.getId() + '"' + this.renderHandler(Event.KEYDOWN, "_doKeyDownItem") +
          ...
          'min-width:' + (intTrueWidth-4) + 'px;' + this.paintFontName() + this.paintFontSize() + this.paintFontWeight() +
...
// removed to let the select be as wide as it wants
      objHW.addXRule(objGUI,"W","W",-2);
      objHW.addXRule(objGUI,"E","E",-6);
      ...
      if (TYPE == Select.TYPESELECT || strFilter == null) {
        ...
        jsx3.sleep(function() {
          ...
          try {
            Select.doFocus(objHW.getRendered(objGUI).childNodes[0].childNodes[1]);
...
//    jsx3.log("_checkLoseFocus " + objEvent.event.getType() + " " + jsx3.html.getOuterHTML(focusedElm));
    ...
    if (objGUI) {
      ...
      if (this.getType() != Select.TYPESELECT) {
        ...
        b1c.recalculate({parentwidth:b1a.getClientWidth()-1,parentheight:b1a.getClientHeight()},
            (objGUI)?jsx3.html.selectSingleElm(objGUI, 0, 0, 0):null, objQueue);
      } else {
  ...
  Select_prototype.paint = function() {
    ...
    b1.setStyles(this.paintColor() + this.paintBackgroundColor() + 'background-image:url(' + Select.ARROWICON +
        ');background-repeat:no-repeat;background-position:right 0px;' + this.paintZIndex() + this.paintVisibility() +
        this.paintDisplay() + this.paintCSSOverride());
    ...
    if(this.getType() == Select.TYPESELECT) {
      var b1b = b1a.getChildProfile(0);
      b1b.setAttributes(' jsxtype="Text" class="jsx30select_text" unselectable="on" ');
      b1b.setStyles(this.paintFontName() + this.paintFontSize() + this.paintFontWeight() + this.paintColor() + this.paintTextAlign());
      var strHTML = b1.paint().join(b1a.paint().join(b1b.paint().join(this.paintText())));
    } else {
      ...
      b1c.setAttributes(this.paintIndex() + this.paintMaxLength() + this.paintEnabled() + ' class="jsx30combo_text" value="' +
          this.paintText().replace(/"/g,'&quot') + '" jsxtype="Text" ' +
  ...
  Select_prototype._ebFocus = function(objEvent, objGUI) {
    this._getInputElement(objGUI).focus();
  };
...
/* @JSC :: end */
  ...
  Select_prototype._setText = function(strText) {
    var objGUI;
    if (objGUI = this._getInputElement())
      objGUI[((this.getType() == Select.TYPECOMBO) ? "value" : "innerHTML")] = strText;
  };
  ...
  /** @package */
  Select_prototype.onDestroy = function(objParent) {
