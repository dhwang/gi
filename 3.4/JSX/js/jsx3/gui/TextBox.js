...
/* @JSC :: end */
  ...
  /** @private */
  TextBox_prototype._ebChange = function(objEvent, objGUI) {
    var veto = this.doEvent(Interactive.CHANGE, {objEVENT:objEvent, strPREVIOUS:this.jsxvalue, strVALUE:objGUI.value});
    if (veto === false) {
      objGUI.value = this.jsxvalue;
    } else {
  ...
  /**
   * overrides the setOverflow() function from the superclass; sets the overflow property for the object
   *            that defines how its on-screen view will behave when its contents are larger than its specified width
   *            and/or height; applies only if type is jsx3.gui.TextBox.TYPETEXTAREA;
   *            returns a reference to self to facilitate method chaining
   * @param OVERFLOW {String} one of: jsx3.gui.TextBox.OVERFLOWNORMAL, jsx3.gui.TextBox.OVERFLOWAUTO, jsx3.gui.TextBox.OVERFLOWSCROLL
   * @return {jsx3.gui.TextBox} this object
   */
  TextBox_prototype.setOverflow = function(OVERFLOW) {
  ...
  /**
   * called by paint method to convert the value for the text area to an escaped string that when repainted on-screen will appear visually identical to the format in memory
   * @return {String} value for object
   * @private
   * @jsxobf-clobber
   */
  TextBox_prototype.getScreenValue = function() {
    var strValue = null;
    var objGUI = this.getRendered();
    if (objGUI != null)
    ...
    if (strValue == null)
      strValue = (this.jsxvalue != null) ? this.jsxvalue : this.getDefaultValue();

    strValue = strValue.toString().replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
    return strValue;
  };
  ...
  TextBox_prototype.setValue = function(strValue) {
    ...
    var objGUI = this.getRendered();
    if (objGUI != null) objGUI.value = strValue != null ? strValue : "";
    return this;
  };
  ...
  TextBox_prototype.createBoxProfile = function(objImplicit) {
    ...
    if(objImplicit.boxtype == null) objImplicit.boxtype = (bRelative) ? "inline" : "box";
  ...
  TextBox_prototype.paint = function() {
    ...
    if (strType == TextBox.TYPETEXT || strType == TextBox.TYPEPASSWORD) {
      b1.setAttributes(this.paintType() + ' id="' + strId + '"' + this.paintLabel() + this.paintEnabled() + this.paintMaxLength() + this.paintIndex() + this.paintTip() + strImplementedEvents +
          ' value="' + this.getValue().toString().replace(/"/g,'&quot;') + '" class="' + this.paintClassName() + '" ' + strProps);
      b1.setStyles(this.paintFontName() + this.paintFontSize() + this.paintFontWeight() + this.paintVisibility() + this.paintDisplay() + this.paintZIndex() + this.paintBackgroundColor() + this.paintBackground() + this.paintColor() + this.paintTextAlign() + this.paintCSSOverride());
      var strContent = "";
    } else {
      b1.setAttributes(' id="' + strId + '"' + this.paintLabel() + this.paintEnabled() + this.paintIndex() + this.paintTip() + strImplementedEvents + ' class="' + this.paintClassName() + '" ' + this.renderAttributes() + this.paintWrap());
      b1.setStyles(this.paintFontName() + this.paintFontSize() + this.paintFontWeight() + this.paintVisibility() + this.paintDisplay() + this.paintZIndex() + this.paintOverflow() + this.paintBackgroundColor() + this.paintBackground() + this.paintColor() + this.paintTextAlign() + this.paintCSSOverride());
      var strContent = this.getScreenValue();
    }
...
/* @JSC :: end */
  ...
  TextBox_prototype.emUpdateDisplay = function(objTdDim, objPaneDim) {
    if (this.getType() == TextBox.TYPETEXTAREA) {
      ...
      var width = isNaN(w) ? objTdDim.W : Math.min(parseInt(w), objTdDim.W);
      var height = isNaN(h) ? objTdDim.H : Math.min(parseInt(h), objPaneDim.H - objTdDim.T);
