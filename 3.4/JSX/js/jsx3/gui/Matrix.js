...
jsx3.Class.defineClass("jsx3.gui.Matrix", jsx3.gui.Block, [jsx3.gui.Form, jsx3.xml.Cacheable, jsx3.xml.CDF], function(Matrix, Matrix_prototype) {
 ...
 /** @private @jsxobf-clobber */
  ...
  /**
   * {jsx3.util.MessageFormat} Default XSLT structure that will be bound as a child of the &lt;tr&gt; element. When processed during the transformation,
   *          This element calls out to the named template defined by <code>Matrix.DEFAULT_CELL_TEMPLATE</code>.
   ...
   * @jsxobf-clobber
   */
  Matrix.DEFAULT_CALL_CELL_TEMPLATE = new jsx3.util.MessageFormat(
  ...
  /**
   * {jsx3.util.MessageFormat} Default <code>xsl:when</code> that is used to call an individual cell value template for a granular, cell-based update.
   * <br/>Wildcards are as follows:
   ...
   * @jsxobf-clobber
   */
  Matrix.DEFAULT_UPDATE_CELL_VALUE_TEMPLATE = new jsx3.util.MessageFormat(
  ...
  /**
   * {jsx3.util.MessageFormat} Default XSLT structure that is called by the XSLT represented by <code>Matrix.DEFAULT_CALL_TEMPLATE</code>. Renders the TD/DIV
   * <br/>Wildcards are as follows:
...
/* @JSC :: end */
  ...
  /**
   * {int} 250. number of milliseconds between the time a new panel is added and the reaper checks for content exceeding <code>Matrix.DEFAULT_PANEL_POOL_COUNT</code>
   */
...
/* @JSC :: end */
  ...
  Matrix_prototype.updateBoxProfile = function(objImplicit, objGUI, objQueue) {
    ...
    b1b.recalculate({parentwidth:intViewPaneWidth,parentheight:intHeaderHeight},(objGUI && objGUI.childNodes[0])?jsx3.html.selectSingleElm(objGUI, 0, 0, 0):null, objQueue);
    ...
    if (!bUpdateWidths) {
      ...
      if (this.getRenderingModel() == "hierarchical") {
    ...
    for (var i = 0; i < objRows.length; i++) {
      ...
      for (var j=0;j<objRow.childNodes.length;j++) {
        ...
        if (j==0 && this.getRenderNavigators(1) != 0) {
          ...
          var intAdjWidth = intCurWidth - parseInt(objTable.style.left) - 1;
          objTable.style.width = Math.max(0, intAdjWidth) + "px";
          var objFirstRow = this._getFirstRow(objTable);
          //'32' is the summed width of the toggler and the icon cells
          //objFirstRow.childNodes[2].style.width = Math.max(0, intAdjWidth - (parseInt(objFirstRow.childNodes[0].style.width) + parseInt(objFirstRow.childNodes[1].style.width))) + "px";
        }
      }
    }
  };
  ...
  Matrix_prototype.paint = function() {
    ...
    b1.setAttributes('id="' + sId + '" class="jsx30matrix" ' + this.renderHandler("mouseup", "_onMouseUp"));
    ...
    b1f.setAttributes(this.renderHandler("scroll", "_ebScrollV") + this.renderHandler("mouseover", "_ebMouseOverVScroll") +
                      ...
                      this.renderHandler("mousedown", "_ebMouseDownV") + ' unselectable="on" class="jsx30matrix_scrollv"');
    ...
    //configure hscroller
    var b1g = b1.getChildProfile(3);
    b1g.setAttributes(this.renderHandler("scroll", "_ebScrollH") + this.renderHandler("mousedown", "_ebMouseDownH") + ' unselectable="on" class="jsx30matrix_scrollh"');
    ...
    if ((bBeforePaint && sync) || (!bBeforePaint && !sync)) {
      ...
      for (var i = 0; i < objDChildren.length; i++) {
        ...
        for (var j = 0; j < objMasks.length; j++) {
          ...
          if (this._initEditMask(objMask, objChild))
            if (objMask.emGetType() == Matrix.EditMask.NORMAL || objMask.emGetType() == Matrix.EditMask.DIALOG)
              masksToPaint.push(objMask);
        }
      }
      return '<div id="' + this.getId() + '_masks" class="jsx30matrix_masks">' + this.paintChildren(masksToPaint) + "</div>";
    }
...
//        strDataRows += this._paintMasks(true);
      ...
      if (objMasks) objMasks.parentNode.removeChild(objMasks);
  ...
  Matrix_prototype.repaintHead = function() {
    ...
    var strHTML = this._paintHead();
      jsx3.html.setOuterHTML(objHeader, strHTML);
    }
  };
  ...
  Matrix_prototype._doStartReorder = function(objEvent,objGUI) {
    ...
    if (objEvent.leftButton() && intDisplayIndex >= this.getFixedColumnIndex(0) && this.getCanReorder() != jsx3.Boolean.FALSE) {
      ...
      var objNode = objGUI.cloneNode(true);
      var strGhost = '<table id="' + this.getId() + '_ghost" cellspacing="0" cellpadding="0" class="jsx30matrix_ghost" style="width:' + parseInt(objGUI.offsetWidth) + 'px;"><tr>' + jsx3.html.getOuterHTML(objNode) + '</tr></table>';
      var objDiv = this.getRendered(objEvent).childNodes[0].childNodes[0];
      jsx3.html.insertAdjacentHTML(objDiv,"BeforeEnd",strGhost);
      ...
      Interactive._beginMoveConstrained(objEvent,objGhost,function(x,y) {return [x,0];});
...
// ***********************************************+
  ...
  Matrix_prototype._onDoneReorder = function(objEvent) {
    ...
    objGhost.parentNode.removeChild(objGhost);
    ...
    if (intGhostLeft == this._jsxstartleft) {
      ...
      if (this.getCanReorder() != jsx3.Boolean.FALSE) {
        ...
        this._reorderColumns(objEvent,objMovedChild,objDisplayedChildren[objDisplayedChildren.length-1]);
      }
    }
  };
...
// ***********************************************+
   ...
   * @jsxobf-clobber
   */
  Matrix_prototype._reorderColumns = function(objEvent,objMoveChild,objPrecedeChild) {
    ...
    var intFIndex = this.getFixedColumnIndex(0);
    ...
    var bSuccess = this.insertBefore(objMoveChild,objPrecedeChild,true);
...
// ***********************************************+
   ...
   * @jsxobf-clobber
   */
  Matrix_prototype._handleFocusEvent = function(objEvent,objGUI) {
...
// ***********************************************+
    ...
    if (!bAuto) {
      if (this._getCtrlKeys().ctrl) {
        ...
        if (strAnchorId) {
          ...
          if (objAnchorRow)
            this._doSelectRange(objEvent, objAnchorRow, objTargetRow, intIndex);
        } else {
  ...
  /**
   * Gets the rowIndex for an HTML TR element
   * @param objNode {HTMLTableRowElement}
   * @return {int} zero-based; -1 if cannot be resolved
   * @private
   * @jsxobf-clobber
   */
  Matrix_prototype._getRowIndex = function(objNode) {
    if (objNode.rowIndex != null && !isNaN(objNode.rowIndex) && objNode.rowIndex > -1) return objNode.rowIndex;
    var objParent = objNode.parentNode;
    if (objParent) {
      var intLen = objParent.childNodes.length;
      for (var i = 0;i<intLen;i++)
        if (objParent.childNodes[i] == objNode) return i;
    }
    return -1;
  };
   ...
   * @jsxobf-clobber
   */
  Matrix_prototype._doSelectRange = function(objEvent, objAnchorRow, objTargetRow,intCellIndex) {
    //exit if null
    ...
    var intAnchorIndex = this._getRowIndex(objAnchorRow);
    var intTargetIndex = this._getRowIndex(objTargetRow);
    ...
    if (this.getRenderingModel() != "hierarchical") {
      //will be true if chunked paging is used and range is constained to the same panel
      var bSame = false;

      //is chunked?
      var intPagingModel = this.getPagingModel(Matrix.PAGING_OFF);
      if (intPagingModel == Matrix.PAGING_CHUNKED) {
        //get the panel(s) that own the rows -- it might be the same panel
        var objAnchorPanel = objAnchorRow.parentNode;
        var objTargetPanel = objTargetRow.parentNode;
        var bBody = false;
        if (objAnchorPanel.tagName.toLowerCase() != "table") {
          bBody = true;
          objAnchorPanel = objAnchorPanel.parentNode;
          objTargetPanel = objTargetPanel.parentNode;
        }

        //if the rows aren't in the same panel, handle here
        if (objAnchorPanel != objTargetPanel) {
          //deselect all selected records
          for (var i=0;i<intIdLen;i++)
            this._doDeselect(false, strIds[i],this._getRowById(strIds[i]));

          //loop through all panels (TABLES) in the viewpane, finding the index for panels containing the target and anchor rows
          var objVP = objAnchorPanel.parentNode;
          var objIndex = {};
          for (var i=0;i<objVP.childNodes.length;i++) {
            if (objAnchorPanel == objVP.childNodes[i])
              objIndex.anchor = i;
            else if (objTargetPanel == objVP.childNodes[i])
              objIndex.target = i;
          }

          //determine the starting row in the starting panel and the ending row in the ending panel
          if (objIndex.anchor < objIndex.target) {
            var intPanelStart = objIndex.anchor;
            var intPanelEnd = objIndex.target;
            intRangeStart = intAnchorIndex;
            intRangeEnd = intTargetIndex;
          } else {
            var intPanelStart = objIndex.target;
            var intPanelEnd = objIndex.anchor;
            intRangeStart = intTargetIndex;
            intRangeEnd = intAnchorIndex;
          }

          //nested loop to select the range of rows
          var intRowsPerPanel = this.getRowsPerPanel(Matrix.DEFAULT_ROWS_PER_PANEL);
          for (var j=intPanelStart;j<=intPanelEnd;j++) {
            var curPanel = objVP.childNodes[j];
            var curRangeStart = (j == intPanelStart) ? intRangeStart : 0;
            var curRangeEnd = (j == intPanelEnd) ? intRangeEnd : intRowsPerPanel-1;

            var objPanel = (bBody) ? curPanel.childNodes[0]: curPanel;
            for (var i=curRangeStart;i<=curRangeEnd;i++) {
              var strCdfId = objPanel.childNodes[i].getAttribute("jsxid");
              this._doSelect(false, strCdfId, objPanel.childNodes[i], true, intCellIndex);
            }
          }
        } else {
          bSame = true;
      ...
      //the rows are in the same panel. select the range
      if (bSame || intPagingModel == Matrix.PAGING_OFF || intPagingModel == Matrix.PAGING_2PASS) {
        //optimization: deselect all selected records that fall outside the range
        for (var i=0;i<intIdLen;i++) {
          var objTR = this._getRowById(strIds[i]);
          if (bSame || this._getRowIndex(objTR) < intRangeStart || this._getRowIndex(objTR) > intRangeEnd) {
            //jsx3.log("about to deselect: " + strIds[i]);
        ...
        var objPanel = objAnchorRow.parentNode;
      ...
      //get first common ancestor node for target and anchor rows. (this will at least optimize the range selection where possible)
      var objContextNode = this.getRecordNode(this.getRenderingContext("jsxroot"));
      var objAnchorNode = this.getRecordNode(strAnchorId);
      var objTargetNode = this.getRecordNode(strTargetId);
      var objAnchorAncestors = jsx3.util.List.wrap(this._getAncestorNodes(objAnchorNode,objContextNode,[]));
      var objTargetAncestors = jsx3.util.List.wrap(this._getAncestorNodes(objTargetNode,objContextNode,[]));
      var objCommonAncestors = objAnchorAncestors.filter(function(objItem) {
        for (var i=0;i<objTargetAncestors.size();i++)
          if (objItem.equals(objTargetAncestors.get(i)))
            return true;
        return false; });
      var objFirstCommonAncestor = objCommonAncestors.get(0);

      //if a common ancestor was found, resolve to the TR in the view; otherwise, just get the first TR in the view
      //get the onscreen TR represented by the first common ancestor node that was just found in the model
      //get the first row in the first panel
      var objReference = objFirstCommonAncestor ?
                         this._getRowById(objFirstCommonAncestor.getAttribute("jsxid")) :
                         this._getFirstRow(this._getFirstPanel());

      //loop to populate an array of all TR elements to be selected in the VIEW
      var rangeArray = [];
      var intBookend = 0;
      while (objReference && intBookend < 2) {
        if (objReference == objAnchorRow)
          intBookend++;
        if (objReference == objTargetRow)
          intBookend++;
        if (intBookend)
          rangeArray.push(objReference);
        objReference = (intBookend < 2) ? this._getHierarchicalNavToRow("S",objReference) : null;
      }

      //range array should now contain all TRs that should be selected
      for (var i=0;i<rangeArray.length;i++) {
        var strCdfId = rangeArray[i].getAttribute("jsxid");
        this._doSelect(false, strCdfId, rangeArray[i], true, intCellIndex);
      }
    }
...
// ***********************************************+
  ...
  Matrix_prototype._handleEditEvent = function(objEvent, objTD, objColumn, bInsert) {
    ...
    if (objMask != null) {
      ...
      var tdDim = jsx3.html.getRelativePosition(viewPane, objTD);
      var paneDim = jsx3.html.getRelativePosition(rend, rend);
    ...
    Matrix_prototype._isAutoRowSessionDirty = function() {
      if (this._jsxautorowsession) {
        for (var p in this._jsxautorowsession) {
          if (this._jsxautorowbaseline[p] != this._jsxautorowsession[p])
            return true;
        }
      }
      return false;
  ...
  Matrix_prototype._cleanUpAutoRowSession = function(objEvent,bCleanUpEditSession) {
    ...
    if (jis != null) {
      ...
      if (eventRet !== false) {
        ...
        var objNode = this.insertRecord(jis,null,true);
  ...
  Matrix_prototype.commitAutoRowSession = function(objEvent, intCellIndex) {
    ...
    if (!isNaN(intCellIndex)) {
      ...
      if (objTR && objTR.childNodes[intCellIndex-0])
        objTR.childNodes[intCellIndex-0].focus();
    }
  };
  ...
  Matrix_prototype._cleanUpEditSession = function(objEvent, bKeepOpen) {
    ...
    if (es != null) {
      ...
      if (objEvent && bAutoCellWasEdited) {
        ...
        bAutoRowWasExited = jsx3.html.findElementUp(objEvent.srcElement(), function(x) { return x == objAutoRow;},true) == null;
      }
  ...
  /** @private @jsxobf-clobber */
  Matrix_prototype._initEditMask = function(objMask, objColumn) {
    ...
    if (objMask.instanceOf(jsx3.gui.Form)) {
      ...
      Matrix.DialogMask.jsxclass.mixin(objMask, true);
...
// ***********************************************+
  ...
  Matrix_prototype._showHideContainer = function(objEvent,objGUI,bOpen) {
    ...
    if (objContent.style.display == "none" || bOpen) {
      ...
      if (this.getRenderNavigators(1) != 0) objGUI.style.backgroundImage = "url(" + this.getUriResolver().resolveURI(this.getIconMinus(Matrix.ICON_MINUS)) + ")";
      ...
      if (this.getRenderNavigators(1) != 0) objGUI.style.backgroundImage = "url(" + this.getUriResolver().resolveURI(this.getIconMinus(Matrix.ICON_PLUS)) + ")";
    }
  ...
  Matrix_prototype.revealRecord = function(strRecordId) {
    ...
    if(recordNode) {
      if (this.getRenderingModel() == "hierarchical") {
        //make sure to toggle display to reveal nested records
        var i = recordNode.selectNodeIterator("ancestor::record[@jsxid!='jsxroot']");
        while (i.hasNext())
          this.toggleItem(i.next().getAttribute('jsxid'), true);
      }
      //make sure the scroller is synchronized with the content
  ...
  Matrix_prototype._getDragIcon = function(objTR,opacity) {
    //resolve the table
    var objTable = objTR;
    while (objTable.tagName.toLowerCase() != "table") objTable = objTable.parentNode;

    //clone to generate HTML for the custom drag icon
    var objClonedTable = objTable.cloneNode(false);
    var objClonedRow = objTR.cloneNode(true);
    objClonedTable.appendChild(objClonedRow);
    objClonedTable.style.width = "300px";
    objClonedRow.id = "";
    objClonedTable.id = "";
    return  "<div id='JSX' class='jsx30matrix_dragicon' style='" +
                  jsx3.html.getCSSOpacity(opacity) + "'>" +
                  jsx3.html.getOuterHTML(objClonedTable) + "</div>";
  };
  ...
  Matrix_prototype._ebMouseOver = function(objEvent, objGUI) {
    ...
    if (jsx3.EventHelp.isDragging() && this.getCanDrop() == 1 && jsx3.EventHelp.getDragIds()[0] != null) {
      ...
      if (!(bContinue===false)) {
        ...
        var intIndent = (this.getRenderingModel() == "hierarchical"&& this.getRenderNavigators(1) != 0) ?
          parseInt(objContext.row.childNodes[0].childNodes[0].childNodes[0].style.left) :
          4;

        if (this.getRenderingModel() != "hierarchical" || objP.H/3 > objEvent.getOffsetY()) {
      ...
      Matrix.SPYTIMEOUT = window.setTimeout(function(){
        ...
        if (me.getParent() != null)
          me._doSpyDelay(objEvent, intLeft, intTop, strRECORDID, objColumn, objSource);
   ...
   * @param intLeft {int} left position of the spyglass
   * @param intTop {int} top position of the spyglass
   ...
   * @jsxobf-clobber
   */
  Matrix_prototype._doSpyDelay = function(objEvent, intLeft, intTop, strRecordId, objColumn, objSource) {
    ...
    if (eventResult) {
      ...
      Interactive.hideSpy();
      this.showSpy(eventResult, intLeft,intTop);
    }
  };
  ...
  Matrix_prototype._getRowCellContext = function(objGUI) {
    if (objGUI.getAttribute("jsxtype") == "record") return {row:objGUI,cell:null};

    //gets the context TD and TR elements
    while (objGUI.parentNode.getAttribute("jsxtype") != "record") {
      objGUI = objGUI.parentNode;
      if (objGUI.tagName.toLowerCase() == "body" || objGUI.id == this.getId()) return null;
    }

    return {row:objGUI.parentNode,cell:objGUI};
  };
...
//    jsx3.log("wheel: " + objEvent);
            ...
            for (var i=0;i<strIds.length;i++) {
              ...
              if (!(this == objSource && objContext && this._isDescendantOrSelf(this.getRecordNode(objContext.row.getAttribute("jsxid")),this.getRecordNode(strIds[i])))) {
                ...
                if (bInsertBefore) {
                  ...
                  var objRecord = this.adoptRecord(objSource,strIds[i],(this.getRenderingModel() == "hierarchical" && objContext)?objContext.row.getAttribute("jsxid"):null,objContext != null);
                }
              }
            }
    ...
    if (bAuto && (intKeyCode == Event.KEY_ENTER || intKeyCode == Event.KEY_ARROW_DOWN || intKeyCode == Event.KEY_ARROW_UP ||
      ...
      if (intKeyCode == Event.KEY_ENTER) {
        ...
        if (objTR && objTR.childNodes[intIndex])
          objTR.childNodes[intIndex].focus();
        ...
        if (objPanel) {
          ...
          if (objRow) objRow.childNodes[intIndex].focus();
        }
      ...
      if (intKeyCode == Event.KEY_ARROW_UP || (intKeyCode == Event.KEY_ENTER && objEvent.shiftKey())) {
        ...
        if (this.getRenderingModel() == "hierarchical" && intIndex == 0 && this.getSuppressVScroller() != 1) {
        ...
        //go to previous control (escape the LV)
        this.getRendered(objEvent).lastChild.focus();
        bCaptured = true;
      }
    }
  ...
  Matrix_prototype._getNavToRow = function(DIRECTION,objReference,bFetch) {
    ...
    if (DIRECTION == "N" && objReference.previousSibling && objReference.previousSibling.tagName.toLowerCase() != "colgroup") {
      ...
      var objPanel;
      var intMaxPossiblePanels = (this.getRenderingModel() == "hierarchical") ? this._getRowCount() : this._getPanelArray().length;
      if ((DIRECTION == "N" && intCurrentPanelIndex == 0) || (DIRECTION == "S" && intCurrentPanelIndex == intMaxPossiblePanels - 1)) {
        ...
        var intRowCount = (this.getRenderingModel() == "hierarchical") ? 1 : this.getRowsPerPanel(Matrix.DEFAULT_ROWS_PER_PANEL);
  ...
  Matrix_prototype._scrollIntoView = function(objReference) {
    var objP = jsx3.html.getRelativePosition(this._getViewPane(),objReference);

    var intScrollTop = this.getScrollTop();
    var intRefTop = objP.T;
    var intVPHeight = parseInt(this.getBoxProfile(true).getChildProfile(1).getClientHeight() - Box.getScrollSize() + 1);
    //jsx3.log(this.getId() + ":" + intScrollTop + " < " + intRefTop  + " < " + (intScrollTop + (intVPHeight -  (Box.getScrollSize() + 1))));
    if (!(intRefTop > intScrollTop && (intRefTop + objP.H) < (intScrollTop + (intVPHeight - (Box.getScrollSize() + 1))))) {
      ...
      var intBottomOffset = Math.abs(intRefTop - (intScrollTop + intVPHeight));
      ...
      if (intBottomOffset == 0) intBottomOffset = objP.H;
        ...
        this.setScrollTop(intRefTop - (Box.getScrollSize() + 1));
      }
    }
    ...
    if (this.getScaleWidth() != 1) {
      ...
      var intVPWidth = parseInt(this.getBoxProfile(true).getChildProfile(1).getClientWidth() - Box.getScrollSize() + 1);
      //jsx3.log(intScrollLeft + " < " + (intRefLeft + objP.W)  + " < " + (intScrollLeft + (intVPWidth -  (Box.getScrollSize() + 1))));
      if (!(intRefLeft > intScrollLeft && (intRefLeft + objP.W) < (intScrollLeft + (intVPWidth -  (Box.getScrollSize() + 1))))) {
        //the given object is not visible within the viewpane. determine if top or bottom is closest
        ...
        if (intRightOffset < intLeftOffset) {
          ...
          this.setScrollLeft(intRefLeft - (Box.getScrollSize() + 1));
        }
      }
    }
  };
...
// ***********************************************+
  ...
  Matrix_prototype._configurePanelArray = function() {
    ...
    var intPanelCount = this._getRowCount(true) / this.getRowsPerPanel(Matrix.DEFAULT_ROWS_PER_PANEL);
    if (intPanelCount > parseInt(intPanelCount)) intPanelCount = parseInt(intPanelCount) + 1;
  ...
  Matrix_prototype._onPaint = function(bUpdateScroll, bNoMasks) {
    ...
    if (intModel == Matrix.PAGING_OFF || intModel == Matrix.PAGING_STEPPED) {
      ...
      if (this.getRenderingModel() == "hierarchical") myToken.contextnodes = this._getViewPane().childNodes;
      this._getPanelArray()[0] = myToken;
      ...
      if (intModel == Matrix.PAGING_PAGED) {
        //paint the first second and last panels
        if (this._isValidPanel(this._getPanelArray().length -1))
          this._getPaintQueue().unshift({index:intPanelLength -1});

        if (this._isValidPanel(0))
      ...
      if (! bNoMasks) {
        ...
        if (strMaskHTML) {
          ...
          if (viewPane.lastChild)
            jsx3.html.insertAdjacentHTML(viewPane.lastChild, "BeforeEnd", strMaskHTML);
          else
  ...
  Matrix_prototype._getAutoRow = function() {
    return (this.getPagingModel() == Matrix.PAGING_PAGED || this.getRenderingModel() == "hierarchical") ? 0 : this.getAutoRow();
  };
  ...
  Matrix_prototype._reapRange = function(objArray, intStart, intEnd, intDifference,intDirection) {
    ...
    for (var i=intStart;((intDirection==-1 && i>intEnd) || (intDirection==1 && i<intEnd));i+=intDirection) {
      if (objArray[i] != null) {
        ...
        if (objGUI) objGUI.parentNode.removeChild(objGUI);
      }
  ...
  Matrix_prototype._getFirstPanel = function() {
    ...
    for (var i=0;i<objKids.length;i++)
      if ((objKids[i].tagName.toLowerCase() == "table" && objKids[i].getAttribute("jsxautorow") != "true") ||
        (this.getRenderingModel() == "hierarchical" && objKids[i].getAttribute("jsxtype") == "structure"))
        return (this.getRenderingModel() == "hierarchical") ? objKids[i].firstChild : objKids[i];
  };
  ...
  Matrix_prototype._paintPanel = function(strHTML,objViewPane,intPanelIndex) {
    ...
    if (objViewPane) {
      ...
      //insert
      jsx3.html.insertAdjacentHTML(objViewPane,"BeforeEnd",strHTML);
   ...
   * @param objRows {Array} Optional array of rows (TR) to be reformatted
   * @private
   * @jsxobf-clobber
   */
  Matrix_prototype._applyFormatHandlers = function(objToken, bSync) {
    /* @jsxobf-clobber */
    this._jsx2pass = []; // clear out any queue for efficiency purposes
    ...
    var bHier = this.getRenderingModel() == "hierarchical" && this.getRenderNavigators(1) != 0;
      ...
      for (var k = 0; k < h.length; k++) {
        ...
        if (objCell) {
          if (bSync) {
            ...
            this._2passqueue(objFormatHandler, objCell, jsxid, objColumn, rowindex, objToken);
          }
        }
      }
    }
  ...
  /**
   * Called to run the 2-pass formatter for a given Column Cell
   * @param objFormatter {function}
   ...
   * @param myToken {Object} Map describing the painted state of a given panel (token, index, painted)
   * @private
   * @jsxobf-clobber
   */
  Matrix_prototype._2passqueue = function(objFormatter, objDiv, strCDFKey, objMatrixColumn, intRowNumber, myToken) {
    var twoP = this._jsx2pass;
    if (twoP.length == 0)
      jsx3.sleep(this._2passqueueChunk, "Matrix2pass" + this.getId(), this);

    twoP[twoP.length] = [objFormatter, objDiv, strCDFKey, objMatrixColumn, intRowNumber, myToken];
  };
  ...
  Matrix_prototype._2passqueueChunk = function() {
    ...
    while (this._jsx2pass.length > 0 && (t2 - t1 < Matrix._2PASS_INTERVAL)) {
  ...
  Matrix_prototype._getXSL = function(bShell) {
    var baseDoc = jsx3.getSharedCache().getOrOpenDocument(Matrix.DEFAULTXSLURL, null, jsx3.xml.XslDocument.jsxclass);
    ...
    if (objXSL == null) {
      ...
      var intRenderingModel = this.getRenderingModel("deep");
      var objChildren = this._getDisplayedChildren();
      ...
      for (var i = 0; i < objChildren.length; i++) {
        ...
        var strColSpan = (i==0 && intRenderingModel == "hierarchical") ? ' colspan="{$jsx_colspan}" jsxcolspan="{$jsx_colspan}" ' : '';
        b1a.setAttributes(strFocusHandlers + strColSpan + ' jsxtype="cell" class="jsx30matrixcolumn_cell" unselectable="on" id="{$jsx_id}_jsx_{$jsx_cdfkey}_jsx_' + i + '"');
        ...
        //configure the DIV
        b1b.setAttributes('class="jsx30matrixcolumn_cell_value" unselectable="on"');
        ...
        var strUpdateValueTemplate = Matrix.DEFAULT_UPDATE_CELL_VALUE_TEMPLATE.format(objColumn.getId() + "_value");
        var objXSLSnippet = new jsx3.xml.Document();
        objXSLSnippet.loadXML(strUpdateValueTemplate);
        if (!objXSLSnippet.hasError()) {
          objChoose.appendChild(objXSLSnippet.getRootNode());
        }
        ...
        if (intRenderingModel == "hierarchical" && i==0 && this.getRenderNavigators(1) != 0) {
          ...
          objNode.setAttribute("name", objColumn.getId() + "_value");
        } else {
          //either deep or shallow. no need for the intermediary template to create the hierarchical controllers
          var strCallValueTemplate = Matrix.DEFAULT_CALL_VALUE_TEMPLATE.format(objColumn.getId() + "_value");
        }

        var b1c = "";
        var objMask = objColumn.getEditMask();
        if (objMask != null && this._initEditMask(objMask) && objMask.emGetType() == Matrix.EditMask.FORMAT)
          b1c = objMask.emPaintTemplate().replace(/\{0\}/g, "@" + objColumn.getPath());
        var strCallValueTemplate = b1a.paint().join(b1b.paint().join(strCallValueTemplate));

        //factor out the width; and replace with xslt variable (param)
        var re = /width:\d*px;/;
        var strWidthProperty = strCallValueTemplate.match(re);
        //create the cell template and merge with existing snippets (creates: xsl:template/td/div/xsl:call-template)
        var strCellTemplate = Matrix.DEFAULT_CELL_TEMPLATE.format(objColumn.getId(),strCallValueTemplate,"##JSX_REPLACE##"); //replace wildcard with wildcard to support a double-replace (happens further down)
        var o = (strWidthProperty + "").split(":");
        var ii = parseInt(o[1]);
        ...
        strCellTemplate = strCellTemplate.replace(strWidthProperty,"{$jsx_first_row_width_style}").replace("##JSX_REPLACE##","'"+ii+"'").replace("##JSX_REPLACE_ALL##",iii);

        objXSLSnippet = new jsx3.xml.Document();
        objXSLSnippet.loadXML(strCellTemplate);
        if (!objXSLSnippet.hasError()) {
          objXSL.getRootNode().appendChild(objXSLSnippet.getRootNode());
        }
        ...
        if (!objXSLSnippet.hasError()) {
          ...
          objXSLSnippet.getRootNode().setAttribute("name", objColumn.getId() + "_value");
          objXSL.getRootNode().appendChild(objXSLSnippet.getRootNode());
        }
        ...
        if (b1c != "") {
          objXSLSnippet = new jsx3.xml.Document();
          objXSLSnippet.loadXML(b1c);
          if (!objXSLSnippet.hasError()) {
            //set name on template so the value caller can locate it
            var objXRoot = objXSL.selectSingleNode("//xsl:template[@name='" + objColumn.getId() + "_value']");
            objXRoot.appendChild(objXSLSnippet.getRootNode());
          } else {
            LOG.error("Could not create valid XSLT for the edit mask owned by the column, '" + objColumn.getId() + "':\n\t" + objXSLSnippet.getError());
          }
        }
        ...
        var strCallCellTemplate = Matrix.DEFAULT_CALL_CELL_TEMPLATE.format(objColumn.getId());
        objXSLSnippet = new jsx3.xml.Document();
        objXSLSnippet.loadXML(strCallCellTemplate);
        if (!objXSLSnippet.hasError()) {
          if (i==0)
            objTR.insertBefore(objXSLSnippet.getRootNode(),objTRWhen.getParent());
          else
            objTRWhen.appendChild(objXSLSnippet.getRootNode());
        }
      }
    }
  ...
  Matrix_prototype.doTransform = function(objParams) {
    ...
    //rendering model
    objParams.jsx_rendering_model = this.getRenderingModel("deep");
    ...
    if (objParams.jsx_use_categories && this.getRenderingModel() != "hierarchical") delete objParams.jsx_use_categories;
    ...
    /*TO DO: IMPROVE HOW to check for empty content. The jsx_mode provides for situations where no TR is a problem and other times when it isn't (as is the case with the cellvaue mode)*/
  ...
  Matrix_prototype.getSortedIds = function() {
    var result = this.doTransform({jsx_mode:"sort",jsx_return_at_all_costs:true});
   ...
   * @param intType {int}  one of Matrix: .SELECTION_UNSELECTABLE, .SELECTION_ROW, .SELECTION_MULTI_ROW
   */
  ...
  /**
   * Sets the on-screen row height. If row height is null, the default row height will be used (<code>Matrix.DEFAULT_ROW_HEIGHT</code>).
   ...
   * the value defined by the constant, <code>Matrix.DEFAULT_PANEL_POOL_COUNT</code>, will be used.
   * @param-private strDefault {String} The default value to use if null
   * @return {int}
   */
  ...
  /**
   * Returns how data should be painted on-screen.  If no value is specified, <code>Matrix.PAGING_OFF</code> will be applied. Note that the rendering model limits the available paging models:
  ...
  Matrix_prototype.getPagingModel = function(strDefault) {
    ...
    if (this.getRenderingModel() == "hierarchical" && this.jsxpagingmodel != Matrix.PAGING_STEPPED) {
  ...
  /**
   * Sets how data should be painted on-screen.  If no value is specified, <code>Matrix.PAGING_OFF</code> will be applied.
  ...
  Matrix_prototype.setScrollTop = function(intScrollTop, objGUI) {
    ...
    if (objGUI && objGUI.childNodes[2]) {
      ...
      if(objGUI.childNodes[2].scrollTop == intScrollTop == 0)
  ...
  /**
   * Returns an array of selected values (or empty array) if the selection model is <code>Matrix.SELECTION_MULTI_ROW</code>. Returns a string (or null)
...
// ***********************************************+
  ...
  Matrix_prototype.redrawCell = function(strRecordId,objColumn,bSuppressTriggers) {
    ...
    if (objCell) {
      ...
      if (this.getRenderingModel() == "hierarchical" && objColumn.getChildIndex() == 0  && this.getRenderNavigators(1) != 0) {
        while (objDiv && objDiv.tagName.toLowerCase() != "tr") objDiv = objDiv.childNodes[0];
        if (objDiv) {
          objDiv = objDiv.lastChild; //this is actually a TD, although the variable name will be objDiv
        } else {
  ...
  Matrix_prototype.redrawRecord = function(strRecordId,intAction,bRecurse) {
    ...
    if (strRecordId != null && intAction == jsx3.xml.CDF.UPDATE) {
      if (this.getRenderingModel() == "hierarchical") {
        ...
        jsx3.html.setOuterHTML(objMyGUI, strHTML);
      ...
      if (this.getRenderingModel() == "hierarchical") {
        ...
        if (objGUI) {
          ...
          if (objPanel.childNodes.length == 1) {
            ...
            objPanel.parentNode.removeChild(objPanel);
          } else {
      ...
      if (this.getPagingModel(Matrix.PAGING_OFF) != Matrix.PAGING_PAGED) {
        ...
        if (this.getRenderingModel() == "hierarchical") {
          ...
          jsx3.html.insertAdjacentHTML(objSiblingContext,"BeforeBegin",strHTML);
      ...
      if (this.getPagingModel(Matrix.PAGING_OFF) != Matrix.PAGING_PAGED) {
        if (this.getRenderingModel() == "hierarchical") {
          ...
          if (this._hasNoEntityContent(objContent)) {
            ...
            var strHTML = this._drawStructure(strRecordId,objContent.getAttribute("jsxcontextindex"));
            jsx3.html.insertAdjacentHTML(objContent,"BeforeEnd",strHTML);
  ...
  Matrix_prototype._insertFlattenedDescendants = function(objRecord) {
    ...
    if (this.getRenderingModel("deep") == "deep") {
...
// ***********************************************+
  ...
  Matrix_prototype.onDestroy = function(objParent) {
...
/**
 * A class that defines the methods required for an object to be used by <code>Matrix.Column</code> instances to
  ...
  ColumnFormat._formatLookup = function(objDiv, strCDFKey, objMatrix, objMatrixColumn, intRowNumber, objServer) {
    ...
    if (objMask != null && typeof(objMask.getRecordNode) == "function") {
      ...
      if (objNode) {
        ...
        objDiv.innerHTML = lookupNode != null ? lookupNode.getAttribute("jsxtext") :
            (strValue != null ? jsx3.util.strEscapeHTML(strValue) : "");
      }
    }
  };
...
jsx3.Class.defineInterface("jsx3.gui.Matrix.BlockMask", jsx3.gui.Matrix.EditMask, function(BlockMask, BlockMask_prototype) {
  ...
  BlockMask_prototype.emBeginEdit = function(strValue, objTdDim, objPaneDim, objMatrix, objColumn, strRecordId, objTD) {
    this.setLeft(objTdDim.L, true);
    this.setTop(objTdDim.T, true);

    if (isNaN(this.getWidth()))
      this.setWidth(objTdDim.W, true);
    if (isNaN(this.getHeight()))
      this.setHeight(objTdDim.H, true);
...
jsx3.Class.defineInterface("jsx3.gui.Matrix.DialogMask", jsx3.gui.Matrix.BlockMask,
  ...
  DialogMask_prototype.emBeginEdit = function(strValue, objTdDim, objPaneDim, objMatrix, objColumn, strRecordId, objTD) {
    ...
    if (this.getLeft() == 0 && this.getTop() == 0) {
      var offsetPos = jsx3.html.getRelativePosition(this.getRendered(objTD).parentNode, objTD);
      this.setLeft(offsetPos.L, true);
      this.setTop(offsetPos.T + offsetPos.H, true);
    }
