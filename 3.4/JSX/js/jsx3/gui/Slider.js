...
jsx3.Class.defineClass("jsx3.gui.Slider", jsx3.gui.Block, [jsx3.gui.Form], function(Slider, Slider_prototype) {
  ...
  Slider_prototype.paint = function() {
    ...
    var strEvents = this.renderHandlers(Slider.BRIDGE_EVENTS, 0);
    var strAttributes = this.renderAttributes(null, true);

    //paint the outer-most box
    var b1 = this.getBoxProfile(true);
    b1.setAttributes('id="' + this.getId() + '" class="jsx30slider" ' + 'label="' + this.getName() + '" ' +
                      this.paintIndex() + this.paintTip() + strEvents + strAttributes);
    ...
    b1ba.setAttributes(' class="handle' + (bEnabled ? '' : '_disabled') + '" unselectable="on"');
