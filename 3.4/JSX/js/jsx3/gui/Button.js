...
//      objEvent.releaseCapture(objGUI);
  ...
  Button_prototype.paint = function() {
    ...
    if((myKeyBinding = this.getKeyBinding()) != null) {
      ...
      this._jsxhotkey.setEnabled(this.getEnabled() != jsx3.gui.Form.STATEDISABLED);
    }
    ...
    b1.setAttributes('id="' + this.getId() + '"' + this.paintLabel() + this.paintTip() + strEvents + ' class="' + this.paintClassName() + '" unselectable="on" ' + strAttributes);
    b1.setStyles(this.paintVisibility() + this.paintDisplay() + this.paintCursor() + this.paintFontSize() + this.paintFontName() + this.paintFontWeight() + this.paintColor() + this.paintBackgroundColor() + this.paintZIndex() + this.paintTextAlign() + this.paintCSSOverride());

    //render the label box
    var b1a = b1.getChildProfile(0);
    b1a.setAttributes(this.paintIndex() + ' class="jsx30button_text" unselectable="on" ');
   ...
   * @jsxobf-clobber
   */
  Button_prototype.paintCursor = function() {
