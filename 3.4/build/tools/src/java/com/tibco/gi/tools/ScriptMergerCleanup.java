...
public class ScriptMergerCleanup {
  ...
  private static final Properties XML_PROPS = new Properties();

  static {
    XML_PROPS.put("method", "xml");
    XML_PROPS.put("version", "1.0");
    XML_PROPS.put("encoding", "UTF-8");
    XML_PROPS.put("indent", "yes");
    XML_PROPS.put("omit-xml-declaration", "no");
    XML_PROPS.put("standalone", "no");
  }
  ...
  public void cleanUpScripts(File dir, File config, File log) throws IOException {
    ...
    if (config != null) {
      ...
      try {
        ...
        serializeToXML(configDoc, config);
  ...
  public void addScriptToConfig(File config, File script, String scripttarget) throws IOException {
    ...
    srcNode.appendChild(configDoc.createTextNode(config.getParentFile().toURI().relativize(script.toURI()).toString()));
    record.appendChild(srcNode);

    serializeToXML(configDoc, config);
  }

  private void serializeToXML(Document configDoc, File configFile) throws IOException {
    Serializer serializer = SerializerFactory.getSerializer(XML_PROPS);
    FileOutputStream out = new FileOutputStream(configFile);
    serializer.setOutputStream(out);
    serializer.asDOMSerializer().serialize(configDoc);
    out.close();
  }
}
