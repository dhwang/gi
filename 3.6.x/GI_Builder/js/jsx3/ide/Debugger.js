...
jsx3.Class.defineClass("jsx3.ide.Debugger", null, null, function(Debugger, Debugger_prototype) {
...
/** CONSTANTS *****************************************/
  jsx3.ide.DEBUG_LEFT = 50;
  jsx3.ide.DEBUG_TOP = 50;
  jsx3.ide.DEBUG_WIDTH = 590;
  jsx3.ide.DEBUG_HEIGHT = 590;
  jsx3.ide.DEBUG_ARGS;
...
/** DEBUG **********************************/
   ...
   */
  jsx3.ide.debug = function(objArgs) {
    if(typeof(objArgs) == "string") {
      ...
      var objExpression = jsx3.ide.createExpressionObject(objArgs);
    } else {
      //globally reference the locally-scoped arguments array to make accessible from within the debugger
      jsx3.ide.DEBUG_ARGS = arguments.callee.caller.arguments;


      //call function that will parse the function (which is now a string) into a parsable expression object;
      var objExpression = jsx3.ide.createExpressionObject(arguments.callee.caller.toString().replace(/\}\s*else/g,"}\r\nelse"));
    }
    //pass the expression object to the step-through engine
    return jsx3.ide.doDebug(objExpression);
  };
  ...
  jsx3.ide.doDebug = function(_ide_objExpression) {
    ...
    do {
      ...
      if(!_ide_curExpressionObject["bIterated"]) {
        ...
        if(_ide_b_modal_return != 4) _ide_b_modal_return = window.showModalDialog("GI_Builder/components/debugger/debugger.html",[_ide_objExpression,_ide_curExpressionObject,-1,_ideobjResult,0,window],"dialogHeight: " + jsx3.ide.DEBUG_HEIGHT + "px; dialogWidth: " + jsx3.ide.DEBUG_WIDTH + "px; dialogTop: " + jsx3.ide.DEBUG_TOP + "px; dialogLeft: " + jsx3.ide.DEBUG_LEFT + "px; edge: Raised; help: No; resizable: No; status: No;");
...
jsx3.ide.createExpressionObject = function(strFunction) {
  ...
  if(f1 == 0) {
    ...
    if(strCondition.search(matchArray[0]) > -1) {
      ...
      eO["inputParameters"] = jsx3.ide.getDebugParameters(strFunction);
      eO["precondition"] = "_ide_debugswitch = 0";
      eO["condition"] = "_ide_debugswitch == 0";
      eO["postcondition"] = "_ide_debugswitch = 1";
    ...
    for(var i=0;i<maxLen;i++) {
      ...
      if(intDepth == -1) {
        if(strSubExpression != "") {
          ...
          eO["expressionArray"][eO["expressionArray"].length] = jsx3.ide.createExpressionObject(strSubExpression);
          strSubExpression = "";
...
jsx3.ide.getDebugParameters = function(strFunction) {
  ...
  if(strArgNameArray.length > 0) {
    ...
    for(var i=0;i<objArgNameArray.length;i++) {
      str += "var " + objArgNameArray[i] + " = jsx3.ide.DEBUG_ARGS[" + i + "];";
    }
  }
  return str;
};
