...
jsx3.Class.defineClass("jsx3.ide.Preferences", jsx3.app.Settings, null, function(Preferences, Preferences_prototype) {
  ...
  Preferences_prototype.init = function(objFile, objSettings) {
    ...
    if (!objSettings) {      
      ...
      if (objFile && objFile.exists()) {
        objXML.load(objFile.toURI());
  
        if (objXML.hasError()) {
          jsx3.ide.LOG.warn("Error reading preferences XML " + objFile.toURI() + ": " + objXML.hasError());
          
          objXML.loadXML(objFile.read());
          if (objXML.hasError()) {
            jsx3.ide.LOG.error("Error reading preferences file " + objFile + ": " + objXML.hasError());
            objXML = null;
          }
        }
      } else {
