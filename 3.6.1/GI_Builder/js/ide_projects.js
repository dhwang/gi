...
jsx3.ide.getProjectSettings = function(bRW) {
  ...
  if (bRW) {
    ...
    if (!jsx3.ide._PROJECT_PREFS[strPath]) {
      var objFile = new jsx3.io.File(objServer.getDirectory(), jsx3.CONFIG_FILE);
...
jsx3.ide.getInstalledProjects = function() {
	...
	for (var i = 0; i < dirs.length; i++) {
      ...
      if (jsx3.ide.isFileToIgnore(file)) continue;
			...
			if (file.isDirectory()) {
				var configFile = new jsx3.io.File(file, jsx3.CONFIG_FILE);
...
jsx3.ide.doNewProject = function(objDialog, strName, bConfirmed) {
  ...
  // close down existing project
	...
	if (! templateFolder.isDirectory()) {
		jsx3.ide.LOG.error('Project template folder should exist: ' + templateFolder);
		return false;
	}
...
jsx3.ide.checkProjectExists = function(strPath, bAlert) {
	var configFile = jsx3.ide.getHomeRelativeFile(strPath + "/" + jsx3.CONFIG_FILE);
	if (! configFile.isFile()) {
		if (bAlert) {
		  jsx3.IDE.alert(null, 
...
jsx3.ide.getRecentFiles = function() {
	var settings = jsx3.ide.getIDESettings();
	var project = jsx3.ide.SERVER;
	return settings.get('projects', project.getRelPath(), 'recentFiles') || [];
};
...
jsx3.ide.checkProjectJsxVersion = function() {
  try {
    //TODO: why does fx 3 bomb out here??  even though jsx3.ide.SERVER is declared in a prior function, it is sometimes lost
    ...
    jsx3.ide.LOG.warn("Error during project version check: " + jsx3.NativeError.wrap(e).getMessage());
  }
};
