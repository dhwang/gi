     ...
     if (! objDoc.hasError()) {
       ...
       if (strMyURI == Mapper._ns.wsdl) {
         ...
         for (var i=objImportNodes.iterator(); i.hasNext(); ) {
           ...
           //3.7: move Import loading to external method to allow recursion...now need to deal with possibility of infinite loop
 ...
 /**
  ...
  /**
   ...
   Mapper_prototype._openJSON = function(strBaseURL) {
     ...
     var objReq = new jsx3.net.Request();
     objReq.open("get",strBaseURL,false);
     objReq.send();
          ...
          */
            ...
            if (strType != "C" && objNode.selectNodes(".//restrictions/record[@name='enumeration']").size() == 0) {
              ...
              for (var j = objNodes.iterator(); j.hasNext(); ) {
                ...
                if (objEnums.size() > 0) {
                  // We need to relativize the component path to the project so that jsx3.IDE does not become
                  // the URI resolver for this DOM branch.
                  var strPath = jsx3.ide.SERVER.relativizeURI(Mapper.SELECT_PROJECTION, true);
                  var objChild = objJSX.load(strPath,false);
                  objChild.setPersistence(jsx3.app.Model.PERSISTEMBED);
                  ...
                  //update the name of the control
                  // See comment above about relatizing the component path.
                  var strPath = jsx3.ide.SERVER.relativizeURI(Mapper.TEXTBOX_PROJECTION, true);
                  var objChild = objJSX.load(strPath,false);
                  objChild.setPersistence(jsx3.app.Model.PERSISTEMBED);
                  objChild.getDescendantsOfType(jsx3.gui.TextBox)[0].setName(strName);
                }
