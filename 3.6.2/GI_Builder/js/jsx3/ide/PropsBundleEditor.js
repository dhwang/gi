...
jsx3.require("jsx3.ide.PropertiesEditor");
...
jsx3.Class.defineClass("jsx3.ide.PropsBundleEditor", jsx3.ide.PropertiesEditor, null,
  ...
  PropsBundleEditor_prototype.getUntitledPath = function() {
    return "templates/untitled_ljss.xml";
  };

  PropsBundleEditor_prototype.getTabPath = function() {
    return "components/containers/tab_propsbundleeditor.xml";
  };

  PropsBundleEditor_prototype.getMatrix = function() {
    return this.getTab().getDescendantOfName("jsxpropsbundleeditor");
  };

  PropsBundleEditor_prototype.loadMatrixData = function(objXML) {
    var objMatrix = this.getMatrix();
    objMatrix.getServer().getCache().setDocument(objMatrix.getXMLId(), this._convertToCDF(objXML));

    var defaultCol = objMatrix.getChild("default");
  ...
  PropsBundleEditor_prototype._addColumn = function(locKey, objMatrix, defaultCol) {
    ...
    if (defaultCol == null) defaultCol = objMatrix.getChild("default");
  ...
  PropsBundleEditor_prototype.getFileType = function() {
		return "ljss";
	};
  ...
  PropsBundleEditor_prototype.addLocaleColumn = function() {
    ...
    jsx3.IDE.prompt("Add Locale", "Enter the key of the new locale. Example: en_US.",
        function(d, val) {
          d.doClose();
          val = jsx3.util.strTrim(val);

          if (objMatrix.getChild(val)) {
            jsx3.ide.LOG.error("Locale " + val + " already exists in properties bundle.");
