...
jsx3.ide.disagreeLicenseAgreement = function(objDialog) {
  ...
  window.location.href = "http://gi.tibco.com/links/devnet/";
};

jsx3.ide.newVersionCheck = function(bForce) {
  if (jsx3.ide._newVchecking) return;
  ...
  doc.setAsync(true);
  doc.subscribe("*", jsx3.ide, function(objEvent) {
    jsx3.ide._newVchecking = false;
    ...
    if (objEvent.subject == jsx3.xml.Document.ON_RESPONSE && !objEvent.target.hasError()) {
        ...
        if (ignoreVersion != newVersion) {
          if (jsx3.util.compareVersions(newVersion, myVersion) > 0)
            jsx3.IDE.confirm(
              ...
              function() { window.open("http://gi.tibco.com/links/devnet/", "jsxide_newversion"); }, null,
              "Download", "Later", 1,
      ...
      jsx3.ide.LOG.debug("Version check error: " + objEvent.target.getError());

      if (bForce)
        jsx3.IDE.alert("Version Check Failed", "The version check failed. Check your network connection and try again.",
            null, null, {nonModal:1});
    }
  });

  doc.load("http://gi.tibco.com/versioncheck/gi.xml?ts=" + new Date().getTime());
  /* @jsxobf-clobber */
  jsx3.ide._newVchecking = true;
};
