    ...
    if (bCreateApps) {
      var configs = (new jsx3.io.File(objDir, "JSXAPPS")).find(function(x) { 
        if (! jsx3.ide.isFileToIgnore(x)) {
          ...
          else if (x.getName() == jsx3.CONFIG_FILE)
...
jsx3.ide._getUserDomainPrefs = function() {
  if (!jsx3.ide._USER_DOMAIN_PREFS) {
    var objFile = new jsx3.io.File(jsx3.io.File.getUserHome(), ".tibcogi.xml");
    jsx3.ide._USER_DOMAIN_PREFS = new jsx3.ide.Preferences(objFile);
  }
  return jsx3.ide._USER_DOMAIN_PREFS;
};
...
jsx3.ide.getCurrentUserHome = function(bCheckMem) {
  if (bCheckMem) {
    var prefs = jsx3.ide._getUserDomainPrefs();
