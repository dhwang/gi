...
/* CONSTANTS ************************************************************/
  ...
  Service._convertXML = function(objNode) {
    ...
    if(myNS == Service.json_namespace + "array" || myNS == Service.json_namespace + "array/literal") {
      ...
      for (var i = objNode.getChildIterator(); i.hasNext(); ) {
        var objKid = i.next();
        var kNS = objKid.getNamespaceURI();
        a.push((kNS.indexOf("literal") > -1) ? Service._convertXML(objKid) : "{" + Service._convertXML(objKid) + "}");
      }
      strMid = a.join(',');
      strEnd = '';
    }
    ...
    if (objRulesXML != null) {
      ...
      if (!this.getMode()) {
        ...
        if (this._isJSON("O")) {
          ...
          objReq.open("get", strStaticResponseUrl, false);
          objReq.send();
        ...
        if (objInputNode) {
          ...
          for (var i=objResponseRules.iterator(); i.hasNext(); ) {
            ...
            if (objNodeChild != null) {
              ...
              //3.6.2: added final step to reset the complete document in the cache, so setXMLBind will fire for all interested parties
