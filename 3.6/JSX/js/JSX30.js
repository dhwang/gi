    ...
    jsx3.lstr = function(objBundle, strKey) {
      if (!objBundle.__) {
        ...
        if (indexDash > 0) {
            ...
            if (trueLocale) localeBundles.push(trueLocale);
        }
...
//          alert("_super this:" + this + " name:" + strName + " fct:" + fct + " args:" + arrArgs);
    ...
    defClass(jsx3.util, "JobGraph", jsx3.util.Graph, function(JobGraph, JobGraph_prototype) {
       ...
       */
      JobGraph_prototype.addJob = function(objJob, preReqs) {
        if (! this.add(objJob))
          throw new Error(jsx3._msg("job.dup", objJob.id()));
        ...
        if (preReqs instanceof Array) {
          ...
          for (var i = 1; i < arguments.length; i++)
            this.addPrereq(objJob, arguments[i]);
        }
...
//        jsx3._logIt("Add job " + objJob.toString() + " with pres [" + objJob.up() + "] tried [" + preReqs + "]");
      ...
      JobGraph_prototype.insertJob = function(newJob, referenceJob) {
        if (! this.add(newJob))
          throw new Error(jsx3._msg("job.dup", newJob.id()));
...
//        jsx3._logIt("... ran job " + objJob.id() + " with result " + intCmd);

        if (typeof(intCmd) == "undefined" || intCmd == Job.DONE ||
            (intCmd == Job.WAIT && objJob.state() == Job.FINISHED)) {
...
//        jsx3._logIt("_onFinish " + objJob.id() + " (running:" + this._finishSP + ")");

        delete this._runningJobs[objJob.id()];
      ...
      JobGraph_prototype._finishThread = function() {
        if (this._finishSP) return;
        
        // Set the semaphore to true so that the loop cannot cause this method to be called again in an ever-growing stack.
        this._finishSP = true;

//        try {
        while (this._finishingJobsQueue.length > 0) {
          var objJob = this._finishingJobsQueue.shift();

          var dependentJobs = objJob.down().concat();
          this.del(objJob);
          this._doneCt++;

          this._publish({subject:JobGraph.RAN_JOB, job:objJob});
          
//          jsx3._logIt("Finished " + objJob.toString() + " -> " + dependentJobs);

          for (var i = 0; i < dependentJobs.length; i++)
            this._runIfReady(dependentJobs[i]);
        }
//          } catch (e) {
...
//        }
        ...
        if (this.size() == 0) {
          this._publish({subject:JobGraph.DONE});
        } else {
...
/*            if (inc.type == "jss") {
              ...
              allJsxJss[allJsxJss.length] = job.id();
              this.graph().add(job);
          ...
          for (var i = 0; i < addinIncludes.length; i++) {
            ...
            if (this.passesLoad(include.onLoad || include.load) && this._passesBrowser(include.browser)) {
              ...
              if (include.type == 'script') {
                ...
                var job = new ClassLoader.PBJob(rsrcJobId, src, jsx3.System.JSS, jsx3.getSystemCache(), jsx3.System.getLocale());
                jobManager.addJob(job);
                jobManager.addPrereq(jobPrefix + ".jss", job);
              }
            }
...
/* @JSC :: end */
    ...
    defClass(ClassLoader, "XmlJob", Job, function(XmlJob, XmlJob_prototype) {
      ...
      XmlJob_prototype.init = function(strId, strSrc, objCache, strCacheId, objClass) {
        ...
        /* @jsxobf-clobber */        
        this._cacheid = strCacheId || this.id();
        /* @jsxobf-clobber */        
...
//          jsx3._logIt("Error loading JSS properties file " + this._src + ": " + doc.getError());
    ...
    defClass(ClassLoader, "ClassJob", Job, function(ClassJob, ClassJob_prototype) {
      ...
      ClassJob_prototype.run = function() {
        if (jsx3.lang.Class && jsx3.lang.Class.forName(this.id()))
          return Job.DONE;
              ...
              '<div style="padding: 4px;">'+
                ...
                '<div style="padding:6px 0px 0px 0px;font-size:10px;font-family:Verdana,sans-serif;">' + getMsgStr("nosup") +
                   '<div style="color:#000033;padding:8px 0px 0px 0px;margin:0px 0px 0px -2px;">'+
                     '<span style="font-size:8px;">&gt; </span><a style="color:#000033;" href="http://power.tibco.com/gi/builderlink/sysreqs35/" target="_blank">' + getMsgStr("b_req") + '</a>'+
