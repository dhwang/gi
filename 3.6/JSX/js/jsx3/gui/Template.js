  ...
  Template._filterTemplate = function(objXML,objClass) {
    ...
    while (objStyleNodes.hasNext()) {
      ...
      strStyle = strStyle.replace(regStyle, function($0, $1, $2) {
        var strName = $1;
        var strValue = $2;
        if (!bProtected && strName.search(regBox) == 0) {
          if(strName== "right") {
            strName = "left";
...
/* @JSC */ }
   ...
   * copy the JavaScript code generated by this method and replace the <code>jsx3.gui.Template.compile</code>
   * method call within your GUI class with the new code.
   * The generated code reflects the exact structures injected by the template class when <code>jsx3.gui.Template.compile</code> is called.
  ...
  Template.precompile = function(strClassName,strPrototypeName,strQualifiedName) {
    ...
    if(objClass) {
      ...
      return (a.join(";\n\n") + ";").replace(/function anonymous/gi,"function");
    }
  };
  ...
  Template.compile = function(Markup,objClass,bCache) {
    ...
    if(!objDoc.hasError()) {
      ...
      LOG.error("The markup that was provided cannot be parsed as a valid XML document (" + objDoc.getError().code + ") :" + objDoc.getError().description);
  ...
  Template_prototype._parseMarkup = function() {
    ...
    //TODO: need to enable a try/catch, so developers who author errant JavaScript will know which method caused the issue; until then, they can use precompile
    //      possible that this is only caused by select and test statements in their DM, allowing for useful reporting of the issue in the catch block
    //createBoxProfile
    functionBuilder.create.push("return " + _jsxboxprofile + ";");
    var strCreateBox = functionBuilder.create.join("\n");
    var dynFn = this._class.getConstructor().prototype._createBoxProfile = new Function("_jsxdrawspace","_jsxpaintprofile",strCreateBox);

    //paint
    functionBuilder.paint.push("return _output.join('');");
    var strPaint = functionBuilder.paint.join("\n");
    dynFn = this._class.getConstructor().prototype._paint = new Function("_jsxpaintprofile","_jsxboxprofile",strPaint);
    ...
    dynFn = this._class.getConstructor().prototype._updateBoxProfileImpl = new Function("_jsxdrawspace","_jsxpaintprofile","_jsxupdatequeue","_jsxobjgui","_jsxforcecascade",functionBuilder.update.join("\n"));
    ...
    dynFn = this._class.getConstructor().prototype._getClientDimensions = new Function("_jsxdrawspace","_jsxpaintprofile","_targetchild","_jsxboxprofile",strDimensionsBox);

    //paintChild
    Template._optimizeCompiledCode(functionBuilder.paintchild);
    var strPaintChild = functionBuilder.paintchild.join("\n");
    dynFn = this._class.getConstructor().prototype._paintChild = new Function("_targetchild","_jsxpaintprofile",strPaintChild);
  };
  ...
  Template.addResolver(null,"$classname", new Function('var cn = this.jsxclassname;return this.getClass().getConstructor().DEFAULTCLASSNAME + (cn ? " " + cn : "");'),{type:"attribute",name:"class",trigger:"jsxclassname"});
...
/* @JSC */ if (jsx3.CLASS_LOADER.IE) {
  Template.addResolver("jsx3.gui.Form","$disabled_opacity",new Function('if(this.getEnabled && this.getEnabled() == 0) {return jsx3.html.getCSSOpacity(.4).split(":")[1];}'),{type:"css",name:"filter",triggers:["jsxenabled"]});
/* @JSC */ } else if (jsx3.CLASS_LOADER.FX) {
  Template.addResolver("jsx3.gui.Form","$disabled_opacity",new Function('if(this.getEnabled && this.getEnabled() == 0) {return jsx3.html.getCSSOpacity(.4).split(":")[1];}'),{type:"css",name:"-moz-opacity",triggers:["jsxenabled"]});
/* @JSC */ } else {
  Template.addResolver("jsx3.gui.Form","$disabled_opacity",new Function('if(this.getEnabled && this.getEnabled() == 0) {return jsx3.html.getCSSOpacity(.4).split(":")[1];}'),{type:"css",name:"opacity",triggers:["jsxenabled"]});
/* @JSC */ }
  for(var p in jsx3.gui.Interactive.BRIDGE_EVENTS_MAP)
...
jsx3.Class.defineClass("jsx3.gui.Template.Box", jsx3.lang.Object, null, function(Box, Box_prototype) {
   ...
   * </ul>
   * @returns {jsx3.gui.Template.Box} This instance
   * @package
   */
  ...
  Box_prototype.getRendered = function(objRoot) {
     return jsx3.html.DOM.selectSingleElm(objRoot,this.getNodePath());
  };
  ...
  Box_prototype.recalculate = function(objDrawspace, objGUI, bCascade) {
    ...
    for (var p in objDrawspace) {
      if (bCascade || this.implicit[p] != objDrawspace[p]) {
        ...
        if (p == "border")
          ...
          if(objGUI && objGUI.childNodes.length && objGUI.childNodes[0].nodeType == 3)
            objGUI.childNodes[0].nodeValue = this.implicit[p];
          else if(objGUI && objGUI.childNodes.length)
            jsx3.html.insertAdjacentHTML(objGUI.childNodes[0],"beforebegin",this.implicit[p]);
          else if(objGUI)
    ...
    if (bUpdate && !this._protected) {
      ...
      if (objGUI) {
        ...
        if (this.implicit.position == "absolute") {
          ...
          if (objDrawspace.position == "relative") {
            ...
            if(this.implicit.tagname == "inlinebox" && objStyle.display != "none")
              jsx3.html.DOM.setStyles(objGUI,this._getCSSFix());
          }
          if(objStyle.margin != this.paintMargin())
            jsx3.html.DOM.setStyles(objGUI, this.paintMargin() == "" ? "margin:;" : this.paintMargin());
        ...
        if (bor) {
          jsx3.html.DOM.setStyles(objGUI, this.paintBorder() == "" ? "border:;" : this.paintBorder());
          w=1;
          h=1;
        }
        if (pad) {
          jsx3.html.DOM.setStyles(objGUI, this.paintPadding() == "" ? "padding:;" : this.paintPadding());
          w=1;
          h=1;
        }
      }
 ...
 *     jsx3.gui.Template.compile(['&lt;transform version="1.0" ' ,
...
jsx3.Class.defineClass("jsx3.gui.Template.Block", jsx3.gui.Block, null, function(Block, Block_prototype) {
  ...
  Block_prototype.setText = function(strText, bRepaint) {
    ...
    this.syncProperty("jsxtext",bRepaint == true);
    return this;
  };
   ...
   * @jsxobf-clobber
   */
  Block_prototype._bindFragment = function(objChild, strNS, bDeep, objServer) {
    this.jsxsuper(objChild, strNS, bDeep, objServer);
    if(objChild.hasPaintProfile())
      objChild.syncProperty("jsxid");
  };
...
/* @JSC :: end */
   ...
   * @package
   */
  Block_prototype.forceSyncBoxProfile = function(objForUpdate,objGUI) {
/* @JSC :: begin BENCH */
    var t1 = new jsx3.util.Timer(jsx3.gui.Painted.jsxclass, this);
/* @JSC :: end */
    if(!objGUI) objGUI = this.getRendered();
    var objQueue = new jsx3.gui.Painted.Queue();
    this._updateBoxProfileImpl(objForUpdate,this.getPaintProfile(),objQueue,objGUI,true);
/* @JSC :: begin BENCH */
...
/* @JSC :: end */
    ...
    if(objBox) {
      ...
      if(objRoot)
        return !this.getFluid() ? objBox.getRendered(objRoot) :
                                 jsx3.html.findElements(objRoot, function(x) {
  ...
  Block_prototype.setDynamicProperty = function(strName, strValue, bUpdateView) {
    //declare object if it doesn't exists
    this.jsxsuper(strName,strValue);
      ...
      if(doBoxVerb) {
        ...
        if(VIEW) {
          if(doBoxVerb == 1)
            ...
            this.forceSyncBoxProfile(objForUpdate,objGUI,true);
        } else if(objGUI) {
  ...
  Block_prototype._synchronizeProfileGUI = function(objPaintProfile, objGUI, TYPE, ID, NAME, objBoxProfile,oldValue) {
    ...
    if(objNode) {
      if (TYPE == "css") {
        ...
        jsx3.html.DOM.setStyle(objNode, NAME, objPaintProfile[ID] || "");
        ...
        if(oldValue) jsx3.html.DOM.clearStyles(objNode,oldValue);
        jsx3.html.DOM.setStyles(objNode,objPaintProfile[ID]);
      } else if (TYPE == "event") {
        if(objPaintProfile[ID]) {
          ...
          jsx3.html.DOM.removeBridgedEvent(objNode,NAME);
          jsx3.html.DOM.addBridgedEvent(objNode,NAME,strFunction);
        } else {
