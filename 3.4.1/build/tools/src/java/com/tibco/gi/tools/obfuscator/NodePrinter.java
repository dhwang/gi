...
//				throw new AssertionError("unsupported child of Block " + node + ": " + firstChild);
  ...
  private static NodePrinter REGEX = new NodePrinter() {
    public void print(NodeWrapper node, StringBuilder buffer) {
      int regexNumber = node.getJsNode().getIntProp(Node.REGEXP_PROP, -1);
      ScriptOrFnNode fNode = (ScriptOrFnNode) node.getScriptParent().getJsNode();
      buffer.append("/");
      buffer.append(escapeRegex(fNode.getRegexpString(regexNumber)));
      buffer.append("/");
      buffer.append(fNode.getRegexpFlags(regexNumber));
    }
  };
    ...
    public void print(NodeWrapper node, StringBuilder buffer) {
      ...
      for (int i = 0; i < fields.length; i++) {
        ...
        if (field instanceof String) {
          ...
          if (NAME.matcher(s).find() || INTEGER.matcher(s).find())
            buffer.append(field);
          else
  ...
  private static final int PREC_CALL = 170; // 150 according to JavaScript guide but I get extraneous parens
