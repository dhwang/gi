...
jsx3.Class.defineClass("jsx3.gui.Column", jsx3.gui.Block, null, function(Column, Column_prototype) {
  ...
  /**
   * fires any custom or system event code necessary for object cleanup (such as clearing the VIEW) when the item is removed from the DOM. <p/>Note that this method is called after this object has been removed from the DOM tree; therefore this.getParent() and this.getServer() will return null. Use the objParent parameter for access to the DOM tree. <p/>This method should be overridden by classes with an on-screen view represented by a TD object.
   * @param objParent {jsx3.app.Model} reference to the former parent
   * @package
   */
  Column_prototype.onDestroy = function(objParent) {
    //call ondestroy event
    return this.doEvent(jsx3.gui.Interactive.DESTROY, {objPARENT:objParent});
  };
...
//        strAnchor = '<img ' + objParent.renderHandler(jsx3.gui.Event.MOUSEDOWN, "doResizeBegin") + ' align="absbottom" style="position:relative;left:0px;top:0px;' + objParent.paintHeaderHeight() + '" unselectable="on" src="' + jsx3.gui.Block.SPACE  + '" class="jsx30column_anchor"/>';
    ...
    if (bBufferColumn) {
      ...
      return '<td ' + ' unselectable="on" ' + ' class="jsx30column_cell" style="position:relative;padding:0px;" width="*" ' + '><div style="white-space:nowrap;position:relative;height:100%;width:100%;padding:0px;">' + strAnchor + '&#160;</div></td>';
    } else {
      ...
      return '<td cellindex="'+this.getChildIndex()+'" id="' + this.getId() + '" unselectable="on" ' + strReorder + this.paintWidth() +
          ...
          this.paintTip() + '><div style="' + strCSSVAlign + strCSSWrap +
  ...
  Column_prototype.paintXSLString = function(bBufferColumn) {
    ...
    if (bBufferColumn) {
      ...
      return '<td style="position:relative;" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" unselectable="on" width="*" class="' + this.paintClassName() + '">&#160;</td>';
      ...
      return '<td valign="top" ' + this.getParent().paintIndex() + strEditFlag + this.paintWidth() +
          ...
          this.paintTextAlign() + ';" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" unselectable="on" ' +
      ...
      return '<td' + this.paintWidth() + ' style="position:relative;' + this.paintTextAlign() +
          '" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" unselectable="on"' +
          ' class="' + this.paintClassName() + '">' + strN1 +  this.getXSLStringVal() + strN2 + '</td>';
    }
  };
