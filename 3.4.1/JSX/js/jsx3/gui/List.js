...
//    }
  ...
  List_prototype.doReorderBegin = function(objEvent, objGUI) {
    ...
    objGhost.innerHTML  = "";
    jsx3.html.setInnerText(objGhost, "");
    var objClone = objCell.childNodes[0].cloneNode(true);

    //create a box profile for the ghost cell to determine its browser-appropriate dimensions
    var trueoffset = parseInt(objCell.parentNode.parentNode.parentNode.style.left);
    var oI = {};
    oI.boxtype = "box";
    oI.tagname = "div";
    oI.left = objCell.offsetLeft + trueoffset;
    oI.top = 0;
    oI.parentheight = objCell.offsetHeight;
    oI.parentwidth = parseInt(objCell.offsetWidth);
    oI.width = "100%";
    oI.height = "100%";
    oI.border = "solid 1px #ffffff;solid 1px #9898a5;solid 1px #9898a5;solid 1px #ffffff";
    oI.padding = parseInt(objCell.childNodes[0].offsetTop) + " " + ((objCell.style.paddingRight) ? parseInt(objCell.style.paddingRight) : 0) + " " + ((objCell.style.paddingBottom) ? parseInt(objCell.style.paddingBottom) : 0) + " " + ((objCell.style.paddingLeft) ? parseInt(objCell.style.paddingLeft) : 0);
    ...
    objGhost.style.fontName = objCell.style.fontName;
    objGhost.style.fontSize = objCell.style.fontSize;
    objGhost.style.textAlign = objCell.style.textAlign;
    objGhost.style.fontWeight = objCell.style.fontWeight;
    ...
    //append the clone
    objGhost.appendChild(objClone);

    //state trackers, so we know which cell the user is planning on moving around
    this._jsxcurleft = objCell.offsetLeft + trueoffset;
    this._jsxcurindex = objCell.cellIndex;
  ...
  List.getDragIcon = function(objGUI,strJSXName,strDragType,strDragItemId) {
    return "<span id='JSX' style='font-family:Verdana;font-size:10px;padding:0px;height:22px;width:200px;overflow:hidden;text-overflow:ellipsis;filter:progid:DXImageTransform.Microsoft.Gradient(GradientType=1, StartColorStr=#eedfdfe8, EndColorStr=#00ffffff);'><table style='font-family:verdana;font-size:10px;color:#a8a8a8;' cellpadding='3' cellspacing='0'>" +
        jsx3.html.getOuterHTML(objGUI).replace(/id=/g,"tempid=").replace(/BACKGROUND/g,"bg").replace(/class=/g,"jsxc=") +
        "</table></span>";
  };
  ...
  List_prototype._ebKeyDown = function(objEvent, objGUI) {
    ...
    if (intKeyCode == Event.KEY_ARROW_UP) {
      ...
      bCaptured = true;
        ...
        this.getRendered().lastChild.focus();
        bCaptured = true;
    }
  ...
  List_prototype.repaintBody = function() {
    ...
    if (doc != null) {
      ...
      if (objGUI != null)
        jsx3.html.setOuterHTML(objGUI.childNodes[0], this.paintBody());
    }
  };
  ...
  List_prototype.repaintHead = function() {
    ...
    if (doc != null) {
      ...
      if (objGUI != null) {
        jsx3.html.setOuterHTML(objGUI.childNodes[0], this.paintHead());
        this.scrollHead();
      }
    }
  };
  ...
  List.onDelete = function(strId, objGUI) {
    ...
    var objJSX = jsx3.html.getJSXParent(objGUI);
   ...
   * @package
   */
  List_prototype.onDestroy = function(objParent) {
    this.jsxsuper(objParent);
    this.onDestroyCached(objParent);
    /* @jsxobf-clobber */
    this._jsxdestroyed = true; // for timeout cleanup
  };
