...
/* @JSC :: end */
  ...
  /**
   * Sets the format of this date picker. The format should conform to the syntax of <code>jsx3.util.DatePicker</code>.
   * The provided format may also be an integer, in which case it is intepreted as one of of the fields of
   * <code>DatePicker</code> - <code>SHORT</code>, <code>MEDIUM</code>, <code>LONG</code>, or <code>FULL</code> - and
  ...
  DatePicker_prototype.getDate = function() {
    if (this.jsxyear != null)
      return new Date(this.jsxyear, this.jsxmonth, this.jsxdate);
    else
      return null;
  };
  ...
  DatePicker_prototype.setDate = function(date) {
    ...
    if (input != null)
      input.value = this._getFormattedDate();
  };

  /** @private @jsxobf-clobber */
  DatePicker_prototype._incrementYear = function(nudge) {
    var date = new Date(this._jsxShownYear+nudge, this._jsxShownMonth, 1);
    this._jsxShownYear = date.getFullYear();
    this._jsxShownMonth = date.getMonth();
    this._repaintCalendar();
  ...
  DatePicker_prototype._incrementMonth = function(nudge) {
    var date = new Date(this._jsxShownYear, this._jsxShownMonth+nudge, 1);
    this._jsxShownYear = date.getFullYear();
    this._jsxShownMonth = date.getMonth();
    this._repaintCalendar();
  ...
  DatePicker_prototype._onDateSelected = function(objEvent, objGUI) {
    ...
    var newDate = new Date(ymd[0], ymd[1], ymd[2]);
    ...
    if (objGUI.value == "") {
      ...
      if (bParse) {
        try {
          ...
          objGUI.value = this._getFormattedDate();
          return;
        }
      }
  ...
  DatePicker_prototype._ebMouseWheel = function(objEvent, objGUI) {
    ...
    if (wd != 0) {
      ...
      if (d != null) {
        newD = new Date(d.getFullYear(), d.getMonth(), d.getDate() + (wd > 0 ? 1 : -1));
      } else {
        d = new Date();
  ...
  DatePicker_prototype._getInput = function(objGUI) {
    ...
    //LUKE: added 'return' statement and commented out 'focus' method
    if (objGUI)
      return objGUI.childNodes[0].childNodes[0];//.focus();
  };
  ...
  DatePicker_prototype.paint = function() {
    ...
    b1.setAttributes('id="' + this.getId() + '" class="jsx30datepicker" ' + 'label="' + this.getName() + '" ' + strAttributes);
    ...
    //paint the input box
    var b1a1 = b1a.getChildProfile(0);
    b1a1.setAttributes(' type="text" value="' + this._getFormattedDate() + '" ' + this.paintIndex() + this.paintTip() + this.paintEnabled() + strEvents);
    ...
    var b1a2 = b1a.getChildProfile(1);
    b1a2.setAttributes(' class="open" ' + this.renderHandler(Event.CLICK, "_doOpen", 2) + this.renderHandler(Event.KEYDOWN, "_ebKeyDownOpen", 2) + this.paintIndex());
    b1a2.setStyles('background-image:url(' + DatePicker.IMAGE_OPEN + ');');
  ...
  DatePicker._getLastDateOfMonth = function(objDate) {
    ...
    return DatePicker._LAST_DATE_ARRAY[month] ||
        ((new Date(objDate.getYear(), month, 29)).getMonth() == month ? 29 : 28);
  };
  ...
  DatePicker_prototype._paintMonth = function(year, month) {
    ...
    html.push('<table cellspacing="0" class="jsx3_dp_month">');
    // do day of week header
    html.push('<tr>');
    for (var i = firstWeekDay; i < firstWeekDay + 7; i++) {
      html.push('<th>' + this._getDayAbbreviation(i % 7, server) + '</th>');
    }
    html.push('</tr>');

    var firstShownDate = new Date(year, month, 1 - ((dayOfFirst-firstWeekDay+7)%7));
    ...
    while (i <= lastDate) {
      ...
      for (var j = 0; j < 7; j++) {
        ...
        html.push('<td id="' + id + '_' + tdYear + '-' + tdMonth + '-' + tdDate + '"' +
  ...
  DatePicker_prototype.emSetValue = function(strValue) {
    ...
    if (jsx3.util.strEmpty(strValue == null)) {
      es.datetype = "int";
