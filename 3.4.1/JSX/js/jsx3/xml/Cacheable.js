...
/* @JSC :: end */
  ...
  Cacheable_prototype._removeFxWrapper = function(strXML) {
    return strXML.replace(/<\/?JSX_FF_WELLFORMED_WRAPPER( [^>]*)?\/?>/g,"");
  };
...
/* @JSC :: end */
   ...
   * @protected
   */
  Cacheable_prototype.onDestroyCached = function(objParent) {
    ...
    if (this.getShareResources() == Cacheable.CLEANUPRESOURCES)
      this.resetCacheData(objParent.getServer());
  ...
  Cacheable_prototype.getXML = function() {
    ...
    var objXML = server.getCache().getDocument(this.getXMLId());
    if (objXML == null) {
      ...
      if (! jsx3.util.strEmpty(xmlString)) {
        ...
        if (! jsx3.util.strEmpty(xmlUrl)) {
          ...
          objXML = new Document();
          objXML.load(this.getUriResolver().resolveURI(xmlUrl));
        } else {
      ...
      objXML = this.setSourceXML(objXML);
    }
   ...
   * @see #setXMLTransformers()
   */
  Cacheable_prototype.setSourceXML = function(objDoc) {
    objDoc = this._runTransformers(objDoc);

    // persist the xml to the cache
    this.getServer().getCache().setDocument(this.getXMLId(), objDoc);
   ...
   * @package
   */
  Cacheable_prototype._getSharedXSL = function(strDefaultUrl) {
...
/* @JSC :: end */
        objXSL = jsx3.getSharedCache().getOrOpenDocument(strDefaultUrl, null, XslDocument.jsxclass);
/* @JSC :: begin DEP */
      }
/* @JSC :: end */
