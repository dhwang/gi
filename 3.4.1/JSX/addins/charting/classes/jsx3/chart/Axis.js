...
jsx3.Class.defineClass("jsx3.chart.Axis", jsx3.chart.ChartComponent, null, function(Axis, Axis_prototype) {
   ...
   * @jsxobf-clobber
   */
  Axis.TICK_ALIGNMENTS = {inside: 1, outside: 1, cross: 1};
   ...
   * @jsxobf-clobber
   */
  Axis.LABEL_PLACEMENTS = {axis: 1, high: 1, low: 1};
  ...
  Axis.LP_LEFT = 3;
  /** @private @jsxobf-clobber @jsxobf-final */
  Axis.LP_LEFTOFAXIS = 7;
  /** @private @jsxobf-clobber @jsxobf-final */
  Axis.LP_RIGHTOFAXIS = 8;
  /** @private @jsxobf-clobber @jsxobf-final */
  Axis.LP_ABOVEAXIS = 6;
  /** @private @jsxobf-clobber @jsxobf-final */
  Axis.LP_BELOWAXIS = 5;
  ...
  Axis.PLACEMENT_MATRIX = [
    Axis.LP_LEFT, Axis.LP_BOTTOM, Axis.LP_RIGHT, Axis.LP_TOP, 
    Axis.LP_RIGHT, Axis.LP_TOP, Axis.LP_LEFT, Axis.LP_BOTTOM, 
    Axis.LP_RIGHTOFAXIS, Axis.LP_ABOVEAXIS, Axis.LP_LEFTOFAXIS, Axis.LP_BELOWAXIS
  ];
  
  /** @private @jsxobf-clobber @jsxobf-final */
  Axis.LABEL_LINEHEIGHT = 10;
  /** @private @jsxobf-clobber @jsxobf-final */
  Axis.DEFAULT_VERTICAL_WIDTH = 50;
  /** @private @jsxobf-clobber @jsxobf-final */
  Axis.DEFAULT_HORIZONTAL_WIDTH = 12;
  ...
  Axis_prototype.getLabelFunction = function() {
    return this.getFunctionField("labelFunction");
  };
   ...
   * @param labelFunction {String} the new value for labelFunction
   */
  Axis_prototype.setLabelFunction = function( labelFunction ) {
    this.setReferenceField("labelFunction", labelFunction);
  };
  ...
  Axis_prototype.setLabelPlacement = function( labelPlacement ) {
    if (Axis.LABEL_PLACEMENTS[labelPlacement]) {
  ...
  Axis_prototype.setTickPlacement = function( tickPlacement ) {
    if (Axis.TICK_ALIGNMENTS[tickPlacement] || tickPlacement == Axis.TICK_NONE) {
  ...
  Axis_prototype.setMinorTickPlacement = function( minorTickPlacement ) {
    if (Axis.TICK_ALIGNMENTS[minorTickPlacement] || minorTickPlacement == Axis.TICK_NONE) {
  ...
  Axis_prototype.getDisplayWidth = function() {
    if (this.displayWidth != null) {
      ...
      return this.horizontal ? Axis.DEFAULT_HORIZONTAL_WIDTH : Axis.DEFAULT_VERTICAL_WIDTH;
    }
  };
  ...
  Axis_prototype.updateView = function() {
    ...
    var axisAt = this.getAxisCrossPoint(oAxis);
    ...
    if (Axis.TICK_ALIGNMENTS[this.tickPlacement] && this.tickLength > 0) {
      ...
      var metrics = this.getTickMetrics(this.tickPlacement, this.tickLength);
      var length = metrics[0];
      var start = axisAt + metrics[1];
      
      this.attachTicks(majorTicksView, ticks, start, length);
    }
    
    // draw minor ticks
    if (Axis.TICK_ALIGNMENTS[this.minorTickPlacement] && this.minorTickLength > 0) {
      // use a LineGroup to reduce VML tag and object count
      ...
      var metrics = this.getTickMetrics(this.minorTickPlacement, this.minorTickLength);
      var length = metrics[0];
      var start = axisAt + metrics[1];
      
      var last = 0;
      for (var i = 0; i < ticks.length; i++) {
        var minorTicks = this.getMinorTicks(ticks, i);
        this.attachTicks(minorTicksView, minorTicks, start, length);
        last = ticks[i];
      }
      ...
      if (last < this.length) {
        var minorTicks = this.getMinorTicks(ticks, ticks.length);
        this.attachTicks(minorTicksView, minorTicks, start, length);
      }
    }
    
    var labelMetrics = this.getLabelMetrics(axisAt);
    ...
    if (title != null && title.getDisplay() != jsx3.gui.Block.DISPLAYNONE) {
      ...
      title.updateView();
      root.appendChild(title.getRenderRoot());
    }

    // draw labels
    if (this.showLabels) {
      var labelTicks = this.getLabelTicks();
      ...
      for (var i = 0; i < labelTicks.length; i++) {
        ...
        if (this.horizontal) {
          ...
          width = this.getDisplayWidth() - this.getTickWidth() - this.labelGap;
        }
        
        if (this.horizontal) {
          var x1 = Math.round(midpoint-(width/2));
          var y = edge + transform * Math.round(Axis.LABEL_LINEHEIGHT/2);
          this.attachLabel(labelsView, x1, y, x1+width, y, this.getLabelForTick(i));
        } else {
          var x1 = (transform == 1) ? edge : edge - width;
          this.attachLabel(labelsView, x1, midpoint, x1+width, midpoint, this.getLabelForTick(i));
        }
      }
    }
  };
   ...
   * @jsxobf-clobber
   */
  Axis_prototype.attachLabel = function(group, x1, y1, x2, y2, label) {
...
//      text.setRotation(angle);
   ...
   * @jsxobf-clobber
   */
  Axis_prototype.getLabelForTick = function(index) {
    var value = this.getValueForTick(index);
    var funct = this.getLabelFunction();
    return funct != null ? funct.call(null, value) : (value != null ? value.toString() : "");
  };
   ...
   * @jsxobf-clobber
   */
  Axis_prototype.getTickWidth = function() {
   ...
   * @jsxobf-clobber
   */
  Axis_prototype.getAxisCrossPoint = function(oAxis) {
    if (oAxis == null) {
   ...
   * @jsxobf-clobber
   */
  Axis_prototype.getTickMetrics = function(placement, length) {
    var offset = 0;
   ...
   * @param axisAt {int} value of this.getAxisCrossPoint()
   * @return {Array} :
   *      placement {int} one of jsx3.chart.Axis.LP_... depending on where the labels should be placed
   ...
   * @jsxobf-clobber
   */
  Axis_prototype.getLabelMetrics = function(axisAt) {
    var oAxis = this.getOpposingAxis();
    if (axisAt == null) axisAt = this.getAxisCrossPoint(oAxis);
    ...
    var placement = Axis.PLACEMENT_MATRIX[placementIndex];
    ...
    switch (placement) {
      ...
      case Axis.LP_BOTTOM : 
        ...
        break;
      case Axis.LP_LEFTOFAXIS : 
      case Axis.LP_ABOVEAXIS : 
        transform = -1;
        edge = axisAt - this.labelGap - outsideTickWidth;
        break;
      case Axis.LP_RIGHTOFAXIS : 
      case Axis.LP_BELOWAXIS : 
        transform = 1;
        edge = axisAt + this.labelGap + outsideTickWidth;
        break;
      default :
    ...
    if (this.showLabels) {
      if (this.horizontal)
        titleEdge = edge + transform * Axis.LABEL_LINEHEIGHT;
      else
   ...
   * @private
   */
  Axis_prototype.getGutterWidths = function() {
    var low = 0, high = 0;
    var oAxis = this.getOpposingAxis();
    if (oAxis == null) return [0,0];
    var axisAt = this.getAxisCrossPoint(oAxis);
    var metrics = this.getLabelMetrics(axisAt);
    var title = this.getAxisTitle();
    ...
    if (this.showLabels) {
      if (this.horizontal)
        edge += (transform * Axis.LABEL_LINEHEIGHT);
      else
   ...
   * @jsxobf-clobber
   */
  Axis_prototype.attachTicks = function(group, ticks, start, length) {
    if (this.horizontal) {
      for (var i = 0; i < ticks.length; i++)
   ...
   * @private
   */
  Axis_prototype.getLabelTicks = function() {
   ...
   * @private
   */
  Axis_prototype.getMinorTicks = function(majorTicks, index) {
    var ticks = [];
    if (index == 0) {
  ...
  Axis_prototype.toString = function() {
    return "[Axis '" + this.getName() + "']";
  };
