...
jsx3.Class.defineClass("jsx3.chart.LinearAxis", jsx3.chart.Axis, null, function(LinearAxis, LinearAxis_prototype) {
   ...
   * @jsxobf-final
   */
  LinearAxis.PADDING_FACTOR = 1.1;

  // Use these values if there is no data range.
  /** @private @jsxobf-clobber @jsxobf-final */
  LinearAxis.DEFAULT_MIN = 0;
  /** @private @jsxobf-clobber @jsxobf-final */
  LinearAxis.DEFAULT_MAX = 100;
  /** @private @jsxobf-clobber @jsxobf-final */
  LinearAxis.DEFAULT_INTERVAL = 20;
  ...
  LinearAxis_prototype.init = function(name, horizontal, primary) {
    ...
    this.storeTransient("trans_shownMin", LinearAxis.DEFAULT_MIN);
    this.storeTransient("trans_shownMax", LinearAxis.DEFAULT_MAX);
    this.storeTransient("trans_shownInterval", LinearAxis.DEFAULT_INTERVAL);
  };
  ...
  LinearAxis_prototype.prePaintUpdate = function() {
    var success = false;
    if (this.autoAdjust)
      success = this.adjustForRange();
    
    if (! success) {
      this.storeTransient("trans_shownMin", 
          this.min != null ? this.min : LinearAxis.DEFAULT_MIN);
      this.storeTransient("trans_shownMax", 
          this.max != null ? this.max : LinearAxis.DEFAULT_MAX);
      this.storeTransient("trans_shownInterval", 
          this.interval != null ? this.interval : LinearAxis.DEFAULT_INTERVAL);
    }
  };
   ...
   * @jsxobf-clobber
   */
  LinearAxis_prototype.adjustForRange = function() {
    ...
    if (range == null) {
      ...
      if (this.min != null || this.max != null) {
        ...
        rangeMin = this.min || LinearAxis.DEFAULT_MIN;
        rangeMax = this.max || rangeMin + LinearAxis.DEFAULT_MAX; 
      } else {
        return false;
      }
    } else {
    ...
    if (itv == null) {
      ...
      if (min != null) {
        ...
        rmin = rangeMin;
        paddingFactor *= LinearAxis.PADDING_FACTOR;
      }
    ...
    if (min == null) {
      var minPadded = rangeMin - (LinearAxis.PADDING_FACTOR-1) *
        (rangeMax - rangeMin) / 2;
    ...
    if (max == null) {
      var maxPadded = rangeMax + (LinearAxis.PADDING_FACTOR-1) *
        (rangeMax - rangeMin) / 2;
  ...
  LinearAxis_prototype.toString = function() {
    return "[LinearAxis '" + this.getName() + "' hor:" + this.horizontal + " pri:" + this.primary + "]";
  };
