  ...
  /** @private @jsxobf-clobber */
    ...
    if (arrDocs.length == 0) {
		...
		if (typeof(doc) == "string") {
			...
			strPath = doc;
			objXML.load(jsx3.IDE.resolveURI(strPath));
		} else if (typeof(doc) == "object" && doc.isInstanceOf) {
		...
		if (objXML != null) {
			...
			if (error.code == "0") {
				...
				jsx3.ERROR.doLog("APIHELP01", "error in XML document " + doc + ": " + error.description, null, false);
			}
		} else {
			jsx3.ERROR.doLog("APIHELP02", "no XML document for " + doc, null, false);
		}
