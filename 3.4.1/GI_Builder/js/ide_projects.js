  ...
  if (bConfirmed || !bProjectOpen) {
    ...
    var uri = jsx3.net.URI.fromParts(loc.getScheme(), loc.getUserInfo(), loc.getHost(), loc.getPort(), loc.getPath(), 
		...
		if (bNewWindow) {
			var w = window.open(uri.toString());
      if (! w)
        jsx3.ide.LOG.error("The project did not open properly. Check that no popup blockers are running.");
    } else {
			// if user vetos location change an error will be thrown
			try {
        window.location.href = uri.toString();
			} catch (e) {
        jsx3.ide.LOG.error("Error opening project " + strPath + ".", jsx3.NativeError.wrap(e));
      }
		}
	} else {
...
jsx3.ide.showWelcomeDialog = function() {
  var dialog = jsx3.IDE.getRootBlock().load('components/project/welcome.xml');
  dialog.focus();
};
