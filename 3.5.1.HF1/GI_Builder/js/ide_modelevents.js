...
/**
 * ? onJsxModelEventsChange()   --called by dispatcher; notified of any published event to the jsx3.ide.events.OBJECT_EVENT_DID_CHANGE subject; for now makes current editor 'dirty'
 ...
 * ! returns        --(null)
 */
jsx3.ide.onJsxModelEventsChange = function(objEvent) {
 ...
 */
jsx3.ide.onModelEventEdit = function(objGrid, strRecordId, strValue) {
  ...
  jsx3.IDE.publish({subject:jsx3.ide.events.OBJECT_EVENT_DID_CHANGE, targets:objJSXs, prop:strRecordId});
};
 ...
 * ! returns        --(null)
 */
jsx3.ide.onModelEventMenuExecute = function(strRecordId) {
  ...
  jsx3.IDE.publish({subject:jsx3.ide.events.OBJECT_EVENT_DID_CHANGE, targets:objJSXs, prop:strRecordId});
  jsx3.ide.onModelEventChange();
};

jsx3.ide.onModelEventChangeSleep = function() {
  jsx3.sleep(jsx3.ide.onModelEventChange, "jsx3.ide.onModelEventChange");
};

/**
 * ? onModelEventChange()   --called by multiple functions to update the model event editor with a list of events appropriate to the selected item in the DOM;
 *            called directly and by the event dispatcher (e.g., jsx3.ide.events.SELECTED_DOM_DID_CHANGE)
 * ! returns        --(null)
 */
jsx3.ide.onModelEventChange = function() {
  ...
  for (var i = 0; i < objJSXs.length && bSuccess; i++) {
    ...
    if (! objXML) {
      bSuccess = false;
      jsx3.ide.LOG.error("Error loading events file for class " + objJSX.getClass() + ".");
  ...
  if (mePalette && bSuccess) {
    jsx3.IDE.getJSXByName("jsx_ide_me_contentblock").doActivate();
  } else if (mePalette) {
    jsx3.IDE.getJSXByName("jsx_ide_me_contentblock").doDeactivate();
  }
};
