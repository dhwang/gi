...
jsx3.ide.destroySystemOut = function(strLocation, out) {
  ...
  if (strLocation == "window") {
    ...
    window.setTimeout(function(){out.getParent().removeChild(out);}, 0);
  }
};
...
jsx3.ide.closeSystemOut = function(bSettingsOnly) {
  if (!bSettingsOnly) {
    ...
    if (current == "window")
      ...
      window.setTimeout(function(){out.getParent().removeChild(out);}, 1);
...
jsx3.ide.closeSystemOutBottom = function(objOut) {
  ...
  window.setTimeout(function(){objOut.getParent().removeChild(objOut);}, 1);  
};
...
jsx3.ide.scrollDownToLastDomChild = function(objJSX) {
  var objGUI = objJSX.getRendered();
  if (objGUI) {
    var childCount = objGUI.childNodes.length;
    if (childCount > 0)
      objGUI.scrollTop = objGUI.childNodes[childCount-1].offsetTop;
  }
};
