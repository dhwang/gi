...
// @jsxobf-clobber-shared  doLoad _jsxroots
...
jsx3.gui.Dialog.prototype.doClose = function() {
  if (this.getServer() != jsx3.IDE)
    jsx3.ide.doRecycle(this.getId(), true);
  else
    this._doClose();
};
...
jsx3.Class.defineClass("jsx3.ide.Server", jsx3.app.Server, null, function(Server, Server_prototype) {
  ...
  Server_prototype.init = function(strAppPath) {
    ...
    /* @jsxobf-clobber */
    this._relpath = jsx3.ide.getCurrentUserHome().toURI().relativize(
        jsx3.ide.getSystemDirFile().toURI().resolve(strAppPath)).toString();
    if (jsx3.util.strEndsWith(this._relpath, "/"))
      this._relpath = this._relpath.substring(0, this._relpath.length - 1);

    this.getCache().subscribe(jsx3.app.Cache.EVENT_CHANGE, jsx3.ide.updateCache);
  };

  Server_prototype.getRelPath = function() {
  ...
  Server_prototype.getDirectory = function() {
    if (this._dirfile == null)
      /* @jsxobf-clobber */
      this._dirfile = new jsx3.io.File(jsx3.ide.getSystemDirFile().toURI().resolve(this.getAppPath()));
  ...
  Server_prototype.getBaseDirectory = function() {
    if (this._basefile == null)
      /* @jsxobf-clobber */
      this._basefile = new jsx3.io.File(jsx3.ide.getSystemDirFile().toURI().resolve(this.getEnv("apppathuri")));
    return this._basefile;
  };

  Server_prototype.paint = function() {};
  Server_prototype.doLoad = function() {};
...
jsx3.Class.defineClass("jsx3.ide.ViewCache", jsx3.app.Cache, null, function(ViewCache, ViewCache_prototype) {
  ...
  ViewCache_prototype.isMyDocument = function(strId) {
    var ts = this._tsindex[strId];
    return ts != null && this._cache.getTimestamp(strId) == ts;
  };

  ViewCache_prototype.clearById = function(strId) {
    this._cache.clearById(strId);
    delete this._tsindex[strId];
  };

  ViewCache_prototype.clearByTimestamp = function(intTimestamp) {
    this._cache.clearByTimestamp(intTimestamp);
  };

  ViewCache_prototype.getDocument = function(strId) {
    return this._cache.getDocument(strId);
  };

  ViewCache_prototype.openDocument = function(strURL, strId) {
    return this._cache.openDocument(strURL, strId);
  };

  ViewCache_prototype.setDocument = function(strId, objDocument) {
    this._cache.setDocument(strId, objDocument, false);
    var ts = this._cache.getTimestamp(strId);
    this._tsindex[strId] = ts;
    this.onChange();
  };

  ViewCache_prototype.getTimestamp = function(strId) {
    return this._cache.getTimestamp(strId);
  };

  ViewCache_prototype.onChange = function() {
    this._cache.onChange();
  };

  ViewCache_prototype.keys = function() {
...
jsx3.Class.defineClass("jsx3.ide.ServerView", jsx3.app.Server, null, function(ServerView, ServerView_prototype) {
  ...
  ServerView_prototype.load = function() {
    ...
    var children = this.JSXBODY.load(this._compurl, false);
