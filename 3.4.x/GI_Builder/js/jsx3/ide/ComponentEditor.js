...
jsx3.Class.defineClass("jsx3.ide.ComponentEditor", jsx3.ide.Editor, null, function(ComponentEditor, ComponentEditor_prototype) {
  ...
  ComponentEditor_prototype._loadServer = function() {
    ...
    server.getCache().subscribe(jsx3.app.Cache.EVENT_CHANGE, jsx3.ide.updateCache);
  ...
  /** @private @jsxobf-clobber */
  ComponentEditor_prototype._redrawText = function() {
    //fixes text-area problem in IE. by repainting with a delay, text that doesn't show is displayed
    jsx3.sleep(function(){
      var view = this.getActiveView();
      if (view && view.getTextArea) {
        var textarea = view.getTextArea();
        if (textarea) textarea.repaint();
      }
    }, null, this);
  };
