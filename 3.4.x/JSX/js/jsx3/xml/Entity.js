...
// @jsxobf-clobber-shared  _createNSResolver _EP _PERM _jsxselectionns _jsxselectionnsobj serializeNamespacesMap
...
/* @JSC */ } else {
    if (intType == 2) {
      if (strNS) {
        ...
        objNode = objDoc.createAttribute(strNodeName);
      }
    } else if (intType == 3) {
      objNode = objDoc.createTextNode(strNodeName);
    } else if (intType == 4) {
      ...
      if (strNS) {
        ...
        objNode = objDoc.createElement(strNodeName);
      }
    }
/* @JSC */ }
...
/* @JSC */ }
   ...
   * @return {String} the attribute value or <code>undefined</code> if the attribute does not exist.
   */
...
/* @JSC */ if (jsx3.CLASS_LOADER.IE) {
  ...
  Entity_prototype._selectNodeOrNodes = function(strQuery, strNS, intMode) {
    ...
    if (intMode) {
      try {
        var objNodes = this._entity.selectNodes(strQuery);
      } catch (e) {;}
      if (! objNodes) objNodes = [];
...
/* @JSC */ } else {
  ...
  Entity._createNSResolver = function(ns) {
    ...
    return Entity._XPE.createNSResolver(nsDoc.documentElement);
  };
