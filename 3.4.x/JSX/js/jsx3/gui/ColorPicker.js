...
jsx3.Class.defineClass("jsx3.gui.ColorPicker", jsx3.gui.Block, [jsx3.gui.Form],
  ...
  var selectSingleElm = jsx3.html.selectSingleElm;
  ...
  for (var f in ColorPicker._IMAGES)
    jsx3.html.loadImages(ColorPicker._IMAGES[f]);
  ...
  ColorPicker_prototype.paint = function() {
    ...
    b1.setAttributes('id="' + this.getId() + '" class="jsx30colorpicker" ' + 'label="' + this.getName() + '" ' +
                      this.paintIndex() + this.paintTip() + strAttributes + ' unselectable="on"');
    ...
    b1a.setAttributes(this.renderHandler(Event.MOUSEDOWN, "_eb2DMouseDown", 2) + ' unselectable="on"');
    var b1b = b1i.getChildProfile(1);
    b1b.setAttributes(this.renderHandler(Event.MOUSEDOWN, "_eb1DMouseDown", 2) + ' unselectable="on"');
    ...
    var spanStart = '<span unselectable="on" class="gradient" style="width:';
    ...
    if (axis == ColorPicker.SATURATION) {
      d2html += spanStart + d2w + 'px;height:' + d2h + 'px;background-color:#FFFFFF;' +
              jsx3.html.getCSSOpacity(1 - hsb[1]) + '"></span>';
    ...
    if (axis == ColorPicker.BRIGHTNESS) {
      d2html += spanStart + d2w + 'px;height:' + d2h + 'px;background-color:#000000;' +
              jsx3.html.getCSSOpacity(1 - hsb[2]) + '"></span>';
  ...
  ColorPicker_prototype._paintGradientImage = function(strURL, intW, intH) {
    if (jsx3.CLASS_LOADER.IE6) {
      return '<span unselectable="on" class="gradient" style="width:' + intW + 'px;height:' + intH + 'px;' +
          'filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'' + strURL + '\', sizingMethod=\'scale\');"></span>';      
    } else {      
      return '<img src="' + strURL + '" unselectable="on" class="gradient" style="width:' + intW + 'px;height:' + intH + 'px;"/>';
    }    
  };
  ...
  ColorPicker_prototype._eb1DMouseDown = function(objEvent, objGUI) {
    ...
    var dim = jsx3.html.getRelativePosition(objGUI, objEvent.srcElement());
   ...
   * @param b1D {boolean} <code>true</code> is the hue component has changed.
   * @param b2D {boolean} <code>true</code> is the saturation/brightness component has changed.
  ...
  ColorPicker_prototype._updateDisplayedColor = function(b1D, b2D, hsb) {
    ...
    if (objGUI != null) {
      ...
      if (b1D) {
        switch (axis) {
          case ColorPicker.HUE:
            ...
            break;
          case ColorPicker.SATURATION:
            jsx3.html.updateCSSOpacity(selectSingleElm(objGUI, 0, 0, 1), 1 - hsb[1]);
            break;
          case ColorPicker.BRIGHTNESS:
            jsx3.html.updateCSSOpacity(selectSingleElm(objGUI, 0, 0, 2), 1 - hsb[2]);
            break;
          default:
            throw new jsx3.Exception();
        }
      ...
      if (b2D) {
        switch (axis) {
          case ColorPicker.HUE:
            jsx3.html.updateCSSOpacity(selectSingleElm(objGUI, 0, 1, 1), 1 - hsb[1]);
            jsx3.html.updateCSSOpacity(selectSingleElm(objGUI, 0, 1, 2), 1 - hsb[2]);
            break;
          case ColorPicker.SATURATION:
