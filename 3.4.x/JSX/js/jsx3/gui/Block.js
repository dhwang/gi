...
/* @JSC :: end */
 ...
 Block.DEFAULTFONTNAME = "Verdana";
   ...
   */
  Block_prototype.setBackgroundColor = function(strColor,bRepaint) {
    this.jsxbgcolor = strColor;

    //immediately upate VIEW if user passed true for repaint
    if (bRepaint) this.updateGUI("backgroundColor",(strColor==Block.NULLSTYLE) ? "" : strColor);
    return this;
  };
   ...
   * @param bRecalc {Boolean} if true, the VIEW will be updated with requiring a repaint
   * @return {jsx3.gui.Block} this object
   */
  Block_prototype.setBorder = function(strCSS,bRecalc) {
   ...
   * @return {jsx3.gui.Block} this object
   */
  Block_prototype.setColor = function(strColor,bRepaint) {
    this.jsxcolor = strColor;

    //immediately upate VIEW if user passed true for repaint
    if (bRepaint) this.updateGUI("color",(strColor==Block.NULLSTYLE) ? "" : strColor);
    return this;
  };
   ...
   * @param bRepaint {boolean} if <code>true</code>, the view of this object is immediately updated, obviating the need to call <code>repaint()</code>.
   */
  Block_prototype.setCursor = function(strCursor,bRepaint) {
    this.jsxcursor = strCursor;

    //immediately upate VIEW if user passed true for repaint
    if (bRepaint) this.updateGUI("cursor",strCursor);
    return this;
  };
   ...
   * @param strCSS {String} CSS text.  Fore example, <code>color:red;background-color:orange;</code>
   * @return {jsx3.gui.Block} this object
   */
   ...
   * @return {jsx3.gui.Block} this object
   */
  Block_prototype.setDisplay = function(DISPLAY, bRepaint) {
    //update the MODEL
    this.jsxdisplay = DISPLAY;

    //immediately update the VIEW if user passed true for bRepaint
    if(bRepaint) {
      ...
      if(DISPLAY != Block.DISPLAYNONE) {
        ...
        if(!(this.getRelativePosition() == Block.ABSOLUTE || (b1 && b1.getBoxType() != "relativebox"))) {
          var oX = jsx3.gui.Painted.Box.getCssFix().split(":");
          if(oX.length == 2) DISPLAY = oX[1];
        }
      }
   ...
   * @return {jsx3.gui.Block} this object.
   */
  Block_prototype.setHeight = function(vntHeight,bRepaint) {
    //update the model
    ...
    if (bRepaint) {
      ...
      this.syncBoxProfile({height:vntHeight/*,parentwidth:b1.implicit.parentwidth,parentheight:b1.implicit.parentheight*/}, true);
    } else {
      //destroy the box profile for the instance. since the profile is a one-to-one match with the VIEW, it must be destroyed if it doesn't match
      //pass true to destroy the descendant box profiles, since updates to the width affects descendant profiles
      this.setBoxDirty();
    }
   ...
   * @return {jsx3.gui.Block} this object
   */
  Block_prototype.setIndex = function(intIndex,bRepaint) {
...
/* @JSC :: end */
   ...
   * @param bRepaint {boolean} if @vntLeft is in integer (a number with no modifier) and @bRepaint is true, the object's on-screen VIEW is immediately updated to match its MODEL, obviating the need to call '[object].repaint()'
   * @return {jsx3.gui.Block} this object.
   */
  Block_prototype.setLeft = function(vntLeft,bRepaint) {
    //update the model
    this.jsxleft = vntLeft;

    //update the boxprofile
    if (bRepaint) {
      if(vntLeft == null) vntLeft = 0;
      var b1 = this.getBoxProfile(true);
      this.syncBoxProfile({left:vntLeft/*,parentwidth:b1.implicit.parentwidth,parentheight:b1.implicit.parentheight*/}, true);
    } else {
      //destroy the box profile for the instance. since the profile is a one-to-one match with the VIEW, it must be destroyed if it doesn't match
   ...
   */
  Block_prototype.setDimensions = function(left, top, width, height, bRepaint) {
    if (left instanceof Array) {
      this.setLeft(left[0], bRepaint);
      this.setTop(left[1], bRepaint);
      this.setWidth(left[2], bRepaint);
      this.setHeight(left[3], bRepaint);
    } else {
      this.setLeft(left, bRepaint);
      this.setTop(top, bRepaint);
      this.setWidth(width, bRepaint);
      this.setHeight(height, bRepaint);
    }
  };
   ...
   * @return {jsx3.gui.Block} this object
   */
  Block_prototype.setMargin = function(strCSS,bRecalc) {
  ...
  Block_prototype.doBeginDrag = function(objEvent, objGUI) {
    ...
    if (objEvent.leftButton())
      this.doDrag(objEvent, objGUI, null);
  };
   ...
   * @return {jsx3.gui.Block} this object
   */
  Block_prototype.setPadding = function(strCSS,bRecalc) {
...
/* @JSC :: end */
   ...
   *            returns reference to self to facilitate method chaining;
   * @param RELATIVE {int} jsx3.gui.Block.RELATIVE will be applied to the view if null. One of: jsx3.gui.Block.RELATIVE jsx3.gui.Block.ABSOLUTE
   * @param bRepaint {boolean} if <code>true</code>, the view of this object is immediately updated, obviating the need to call <code>repaint()</code>.
   * @return {jsx3.gui.Block} this object
   */
  Block_prototype.setRelativePosition = function(RELATIVE, bRepaint) {
    this.setBoxDirty();
    this.jsxrelativeposition = RELATIVE;
    if (bRepaint) {
      // SPEC: left/top only honored if absolute, margin only honored if relative
      if (RELATIVE == Block.ABSOLUTE) {
        this.setLeft(this.getLeft()?this.getLeft():0, true);
        this.setTop(this.getTop()?this.getTop():0, true);
      ...
      this.updateGUI("position", (RELATIVE == Block.RELATIVE) ? "relative" : "absolute");
   ...
   * @return {jsx3.gui.Block} this object.
   */
  Block_prototype.setTop = function(vntTop,bRepaint) {
    //update the model
    this.jsxtop = vntTop;

    //update the boxprofile
    if (bRepaint) {
      if(vntTop == null) vntTop = 0;
      var b1 = this.getBoxProfile(true);
      this.syncBoxProfile({top:vntTop/*,parentwidth:b1.implicit.parentwidth,parentheight:b1.implicit.parentheight*/}, true);
    } else {
      //destroy the box profile for the instance. since the profile is a one-to-one match with the VIEW, it must be destroyed if it doesn't match
      this.clearBoxProfile(false);
    }
   ...
   * @param bRepaint {boolean} if <code>true</code>, the view of this object is immediately updated, obviating the need to call <code>repaint()</code>.
   */
  Block_prototype.setVisibility = function(VISIBILITY,bRepaint) {
    //update the MODEL
    this.jsxvisibility = VISIBILITY;

    //immediately upate VIEW if user passed true for repaint
    if (bRepaint) this.updateGUI("visibility",VISIBILITY);
    return this;
  };
   ...
   * @return {jsx3.gui.Block} this object.
   */
  Block_prototype.setWidth = function(vntWidth, bRepaint) {
    //update the model
    ...
    if (bRepaint) {
      var b1 = this.getBoxProfile(true);
      this.syncBoxProfile({width:vntWidth/*,parentwidth:b1.implicit.parentwidth,parentheight:b1.implicit.parentheight*/}, true);
    } else {
      //destroy the box profile for the instance. since the profile is a one-to-one match with the VIEW, it must be destroyed if it doesn't match
      //pass true to destroy the descendant box profiles, since updates to the width affects descendant profiles
      this.setBoxDirty();
   ...
   * @param bRepaint {boolean} if <code>true</code>, the view of this object is immediately updated, obviating the need to call <code>repaint()</code>.
   */
  Block_prototype.setZIndex = function(intZIndex, bRepaint) {
    //update the MODEL
    this.jsxzindex = intZIndex;

    //immediately upate VIEW if user passed true for repaint
    if(bRepaint) this.updateGUI("zIndex",intZIndex);
    return this;
  };
...
//    this.updateBoxProfileImpl(objImplicit, objGUI, objQueue, 2);
  ...
  Block_prototype.createBoxProfile = function(objImplicit) {
    ...
    if(objImplicit.boxtype == null) objImplicit.boxtype = (bRelative) ? "relativebox" : "box";
...
//    return "color:" + ((this.getColor()) ? this.getColor() : Block.DEFAULTCOLOR)  + ";";
  ...
  Block_prototype.paintIndex = function(intIndex) {
    ...
    return (intIndex != null) ? ' tabindex="' + intIndex + '" jsxtabindex="' + intIndex + '"' : '';
  };
...
/* @JSC :: end */
  ...
  Block_prototype.onDestroy = function(objParent) {
    this.jsxsuper(objParent);
    this.doEvent(jsx3.gui.Interactive.DESTROY, {objPARENT:objParent});
  };
