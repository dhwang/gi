...
return;
    ...
    if (objGUI) {
      ...
      if (objCChild != null) {
        if(this.isFront()) {
          ...
          objCChild.setVisibility(Block.VISIBILITYVISIBLE, true);
        } else {
          //only make hidden--no need to do more
          objCChild.setVisibility(Block.VISIBILITYHIDDEN, true);
        }
      }
    }
  };
  ...
  Stack_prototype.paint = function() {
    ...
    b1b.setAttributes(' class="' + textClass + '" unselectable="on" ');
