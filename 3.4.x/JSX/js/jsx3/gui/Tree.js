...
/* @JSC :: end */
  ...
  jsx3.html.loadImages(Tree.ICONMINUS, Tree.ICONPLUS, Tree.ICON, Tree.SELECTEDIMAGE, "jsx:///images/tree/over.gif",
...
/* @JSC :: end */
  ...
  Tree_prototype.getXSL = function() {
    return this._getSharedXSL(Tree.DEFAULTXSLURL);
  };
...
/* @JSC :: end */
  ...
  Tree_prototype._focusRecord = function(strRecordId) {
    ...
    if (objGUI != null && objGUI.getAttribute) {
      ...
      if (strRecordId) {
        try {
          objGUI.childNodes[0].childNodes[2].focus();
  ...
  Tree_prototype._addItemSelection = function(strRecordId) {
    ...
    if (objGUI != null) {
      var elm = jsx3.html.selectSingleElm(objGUI, 0, 2);
      elm.style.backgroundImage = "url(" + Tree.SELECTEDIMAGE + ")";
      elm.style.borderRight = "solid 1px " + Tree.BORDERCOLOR;
    }
  };
  ...
  Tree_prototype.revealRecord = function(strRecordId, objJSX) {
    ...
    if (itemGUI) {
      ...
      if (baseGUI)
        jsx3.html.scrollIntoView(itemGUI, baseGUI, 0, 10);
    }
  };
...
/* @JSC :: begin DEP */
  ...
  Tree_prototype.redrawRecord = function(strRecordId, ACTION) {
    ...
    if (ACTION == jsx3.xml.CDF.DELETE) {
      if (objGUI) {
        if (objGUI.parentNode.childNodes.length > 1) {
          ...
          jsx3.html.setOuterHTML(parentGUI, this.doTransform(strParentId));
        }
      }
    ...
    if (objGUI == null) {
      ...
      jsx3.html.setOuterHTML(objGUI, this.doTransform(strRecordId));
    }
...
/* @JSC :: end */
  ...
  Tree.getDragIcon = function(objGUI, strJSXName, strDragType, strDragItemId) {
    return  "<div id='JSX' class='jsx30tree_dragicon' style='" +
            jsx3.html.getCSSOpacity(.75) + "'>" +
            jsx3.html.getOuterHTML(objGUI.parentNode.childNodes[1]) +
            jsx3.html.getOuterHTML(objGUI)  + "</div>";
  };
   ...
   * @param-private bBook {boolean} false if null; if true, the system wants to force the VIEW to make the item a book
   * @return {jsx3.gui.Tree} this object
   */
  Tree_prototype.toggleItem = function(strRecordId, bOpen, bBook) {
    var objRecord = this.getRecordNode(strRecordId);
    var objGUI = this._getFocusItem(strRecordId);
    if (objGUI != null) {
      this._showHideBranch(objRecord, objGUI, bOpen, bBook);
    }
    return this;
  };
  ...
  Tree_prototype._togItem = function(objEvent, strRecordId, bOpen, fctOnDone) {
    ...
    if (objGUI != null) {
      ...
      if (objRecord.getAttribute("jsxlazy") == "1" && objRecord.getAttribute("jsxopen") != "1" &&
        ...
        jsx3.html.updateCSSOpacity(objGUI.childNodes[0].childNodes[0], 0.50);
        ...
        newState = this._showHideBranch(objRecord, objGUI, bOpen, false);
      }

      // fire the jsxtoggle event for the tree; the user will have contextual handles to
      this.doEvent(Interactive.TOGGLE, {objEVENT:objEvent, strRECORDID:strRecordId, objRECORD:objRecord, bOPEN:newState});
    }
  };

  /** @private @jsxobf-clobber */
  Tree_prototype._showHideBranch = function(objRecord, objGUI, bOpen, bBook) {
    ...
    if (bBook) objGUI.childNodes[0].childNodes[0].setAttribute("jsxtype", "plusminus");
  ...
  Tree_prototype._ebMouseOver = function(objEvent, objTreeGUI) {
    ...
    if (strType == "text" && this.hasEvent(Interactive.SPYGLASS)) {
      ...
      Tree.SPYTIMEOUT = window.setTimeout(function(){
        Tree.SPYTIMEOUT = null;
        if (me.getParent() != null)
          me._doSpyDelay(objEvent, intLeft, intTop, strRecordId, objSource, objGUI);
...
//            " verb:" + objArrow.getAttribute("dropverb"));
   ...
   * @jsxobf-clobber
   */
  Tree_prototype._doSpyDelay = function(objEvent, intLeft, intTop, strRECORDID, objSource, objGUI) {
    ...
    if (objGUI != null && objGUI.getAttribute("jsxid") != null) {
      ...
      if (intKeyCode >= Event.KEY_ARROW_LEFT && intKeyCode <= Event.KEY_ARROW_DOWN) {
        ...
        if (objEvent.shiftKey()) {
          jsx3.html.focusPrevious(this.getRendered(objTreeGUI), objEvent);
        } else {
          jsx3.html.focusNext(this.getRendered(objTreeGUI), objEvent);
        }
...
/* @JSC :: end */
  ...
  Tree_prototype.paint = function() {
    ...
    var strContent = jsx3.util.strTrim(this.doTransform());
  ...
  Tree_prototype.doTransform = function(strFragmentId) {
    ...
    var objP = {};
    var bFrag = false;
    if (strFragmentId == null) {
      var root = this.getXML().getRootNode();
      if (root) {
    ...
    objP.jsxbordercolor = Tree.BORDERCOLOR;
   ...
   * @package
   */
  Tree_prototype.onDestroy = function(objParent) {
