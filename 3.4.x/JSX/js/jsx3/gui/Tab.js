...
// @jsxobf-clobber-shared  _revealTab _getContentPaneDimensions _updateAutoScrollDisplay
...
/* @JSC :: end */
  ...
  Tab_prototype._ebClick = function(objEvent, objGUI) {
    objGUI.focus(); // needed in IE because click probably originates in unselectable div
...
/* @JSC :: end */
   ...
   * @param strActiveColor {String} valid CSS property value, (e.g., red, #ffffff)
   * @return {jsx3.gui.Tab} this object
   */
   ...
   * @param strInactiveColor {String} valid CSS property value, (e.g., red, #ffffff)
   * @return {jsx3.gui.Tab} this object
   */
   ...
   * @param STATE {int} one of: jsx3.gui.Tab.STATEDISABLED, jsx3.gui.Tab.STATEENABLED
   * @return {jsx3.gui.Tab} this object
   */
  ...
  Tab_prototype._ebKeyDown = function(objEvent, objGUI) {
    ...
    if ((intKeyCode >= Event.KEY_ARROW_LEFT && intKeyCode <= Event.KEY_ARROW_DOWN)) {
      if (objEvent.leftArrow() || objEvent.upArrow()) {
        //navigate to previous tab
        var intIndex = myIndex > 0 ? myIndex - 1 : objParent.getChildren().length - 1;
      } else if (objEvent.rightArrow() || objEvent.downArrow()) {
        var intIndex = myIndex < objParent.getChildren().length - 1 ? myIndex + 1 : 0;
      }
      ...
      //send focus to the child pane when user hits the tabkey.
      //NOTE: this only works if the developer has assigned a tab index to the bound content pane. otherwise tabkey event is ignored
      this.getContentChild().focus();
      objEvent.cancelAll();
    } else if (intKeyCode == Event.KEY_TAB) {
      //assume want to escape tabbedpane...
  ...
  Tab_prototype.paint = function() {
    ...
    b1.setStyles(this.paintBevel() + this.paintCursor() + this.paintBackgroundColor() + this.paintColor() + this.paintFontName() + this.paintFontSize() + this.paintFontWeight() + this.paintTextAlign() + this.paintCSSOverride());

    //render the label box
    var b1a = b1.getChildProfile(0);
    b1a.setAttributes(' class="jsx30tab_text" unselectable="on" ');
  ...
  Tab_prototype.paintCursor = function() {
    return (this.getEnabled() == Tab.STATEENABLED) ? "cursor:pointer;" : "cursor:not-allowed;";
  };
