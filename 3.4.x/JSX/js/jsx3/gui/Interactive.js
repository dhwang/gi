...
jsx3.require("jsx3.gui.HotKey", "jsx3.gui.Heavyweight");
...
jsx3.Class.defineInterface("jsx3.gui.Interactive", null, function(Interactive, Interactive_prototype) {
  ...
  */
  ...
  Interactive_prototype.doEvent = function(strType, objContext) {
    ...
    return this.eval(script, objContext);
//    return this.eval("(function " + strType + "(){return " + script + "})();", objContext);
  ...
  Interactive_prototype.renderHandlers = function(objMap, useId) {
    ...
    for (var i = 0; i < Interactive.BRIDGE_EVENTS.length; i++) {
      ...
      var bridgeMethod = (objMap && objMap[eventType]) || inheritedMap[eventType];
      if (bridgeMethod) {
...
//      jsx3.log("cancelling move because button=" + objEvent.event().button);
  ...
  Interactive_prototype.doSpyOver = function(objEvent, objGUI, objContext) {
    ...
    this._jsxspytimeout = window.setTimeout(function() {
      ...
      // if not empty...
      if (strSPYHTML)
        me.showSpy(strSPYHTML, intX, intY);
  ...
  Interactive_prototype.doSpyOut = function(objEvent, objGUI) {
    ...
    if(objEvent.toElement() != null && objEvent.toElement().className == "jsx30spyglassbuffer") return;
   ...
   * @param intLeft {int} optional override to specify where the left position should be in relation to the left edge of the document BODY
   * @param intTop {int} optional override to specify where the top position should be in relation to the top edge of the document BODY
   */
  Interactive_prototype.showSpy = function(strHTML,intLeft,intTop) {
    if (strHTML != null) {
      ...
      strHTML = '<span onmouseout="if (event.toElement != this.childNodes[0]) jsx3.gui.Interactive.hideSpy();" class="jsx30spyglassbuffer"><div id="_jsxspychild" class="jsx30spyglass">' + strHTML + '</div></span>';
...
///////////////////////////// Methods for convenient handling of hot-key capturing /////////////////////////////////
   ...
   *    is called on this object. This parameter can also be an instance of <code>HotKey</code>, in which case all
...
//  EventHelp.objMyRange = null;
  ...
  /**
   * generic callback that can be used to create a less-generic drag item when dragging from a control; when adding a drag listener to an object, use this function (this.drag) as the name of the function to process the drag event and create a drag icon
   ...
   * @param strDragType {String} the type of element being dragged. For example, a given JSXTable may contain any number of 'dragabble' elements. Each of these may have a type as well as an id to uniquely distinguish it from other elements in the given JSX item
   * @param strDragItemId {String} the id for the html element about to be dragged. If this parameter isn't blank/null/empty, it reflects the unique node id for the element as defined by the source xml
   * @return {String} HTML content that goes in the draggable icon that follows the mouse
   */
  EventHelp.drag = function(objGUI, strJSXName, strDragType, strDragItemId) {
    return "<span style='background-color:#f6f6ff;border:solid 1px #a8a8af;font-family:Verdana;font-size:10px;padding:2px;height:11px;filter:progid:DXImageTransform.Microsoft.Alpha(Opacity=70, FinishOpacity=0, Style=0, StartX=0,  FinishX=100, StartY=0, FinishY=100);'><nobr>" + objGUI.innerText + "</nobr></span>";
  };
...
*/
      ...
      var strHTML =  '<div id="_jsxdrag" unselectable="on" style="position:absolute;left:' +
  ...
  EventHelp.reset = function() {
    ...
    if (EventHelp.curDragObject != null) {
      ...
      if (EventHelp.curDragObject.id == "_jsxdrag")
        jsx3.html.setOuterHTML(EventHelp.curDragObject, "");
