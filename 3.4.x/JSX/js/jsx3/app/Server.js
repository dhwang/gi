...
jsx3.Class.defineClass("jsx3.app.Server", null, [jsx3.util.EventDispatcher, jsx3.net.URIResolver/*, jsx3.gui.Alerts*/],
  ...
  Server.EVENT_SERVER_INITED = "serverInited";
  Server.EVENT_SERVER_CREATED = "serverCreated";
  Server.BODY_NAME = "JSXBODY";
   ...
   * @jsxobf-clobber
   */
  Server.INSTANCES = new jsx3.util.List();
...
/* @JSC :: end */
    ...
    Server.publish({subject: Server.EVENT_SERVER_INITED, target:this});
  ...
  /** @private @jsxobf-clobber */
  Server_prototype.createRootAndBody = function() {
    ...
    var root = this.JSXROOT = this.createRoot("JSXROOT");
...
/* @JSC */ if (jsx3.CLASS_LOADER.IE) {
    ...
    var my = this;
    this.resize_timeout_id = window.setTimeout(function() { my.onResizeDelay(); }, 250);
/* @JSC */ } else {
    this.onResizeDelay();
/* @JSC */ }
  };
   ...
   * @jsxobf-clobber
   */
  Server_prototype.onResizeDelay = function() {
  ...
  Server.allServers = function() {
    return Server.INSTANCES.toArray();
  };
   ...
   * @jsxobf-clobber
   */
  Server_prototype.createRoot = function(strName) {
  ...
  Server_prototype.getInvisibleRoot = function() {
    if (this.INVISIBLE == null)
      this.INVISIBLE = this.createRoot("JSXINVISIBLE");
  ...
  Server._showEnv = function(objEvent) {
    ...
    s.push('<b>Browser:</b> ', jsx3.CLASS_LOADER.getTypes().join(", "), '<br/>');
    ...
    for (var i = 0; i < servers.length; i++) {
      ...
      s.push('<b>GI Version:</b> ', server.getEnv("jsxversion"));
      s.push("</div>");
  ...
  Server_prototype.getSettings = function() {
    if (this._jsxsettings == null)
      /* @jsxobf-clobber */
      this._jsxsettings = new jsx3.app.Settings(jsx3.app.Settings.DOMAIN_PROJECT, this);
  ...
  Server_prototype.destroy = function() {
    ...
    if (unloadScript) {
      try {
        ...
        LOG.error(jsx3._msg("serv.err_unload", this), jsx3.NativeError.wrap(e));
      }
    }
    ...
    this.Cache.unsubscribeAll(jsx3.app.Cache.EVENT_CHANGE);
    this.Cache.destroy();
  ...
  Server_prototype.paint = function(objXML) {
    ...
    try {
      ...
      //call doLoad (regardless of whether or not there is a view component for this server instance, still try to initialize the environment
      this.doLoad(objXML);
    ...
    Server.publish({subject: Server.EVENT_SERVER_CREATED, target:this});
  };

  /**
   * called by [this].paint().  Deserializes any designated component and fires the onLoad script for the application
   *            At runtime, the body's onload event calls [this].paint() which then calls [this].doLoad() which then fires the
   *            the onLoad script for the application.
   * @private
   * @jsxobf-clobber-shared
   */
  Server_prototype.doLoad = function(objXML) {
    this.doLoadCaught(objXML);
  };

  /** @private @jsxobf-clobber */
  Server_prototype.doLoadCaught = function(objXML) {
    var child = null;
  ...
  Server_prototype.loadInclude = function(strSrc, strId, strType, bReload) {
    ...
    var strAppend = (bReload) ? ("?timestamp=" + (new Date()).valueOf()) : "";
    ...
    if (strType == "css") {
      ...
      this.LJSS.addParent(jsx3.app.PropsBundle.getProps(strSrc, this.getLocale(), this.getCache()));
  ...
  Server_prototype.loadResource = function(strId) {
    var objSettings = this.getSettings();
    var includes = objSettings.getMapsInArrayByField('includes', 'id', strId);
    var lastInclude;
  ...
  Server_prototype.getRootBlock = function() {
    if (this.JSXROOT == null) this.createRootAndBody();
    return this.JSXROOT;
  };
  ...
  Server_prototype.createAppWindow = function(strName) {
    ...
    if (this.WINDOWS == null)
      this.WINDOWS = this.createRoot("JSXWINDOWS");
   ...
   * @since 3.2
   */
  Server_prototype.loadAppWindow = function(strSource) {
    jsx3.require("jsx3.gui.Window");
    ...
    if (strSource instanceof jsx3.xml.Entity)
      component = this.WINDOWS.loadXML(strSource, false);
    else
      component = this.WINDOWS.load(strSource, false);
    ...
    if (!(component instanceof jsx3.gui.Window)) {
      ...
      throw new jsx3.Exceptionjsx3._msg("serv.win_notwin", strSource, component.getClass());
    }
...
/* @JSC :: end */
  ...
  Server_prototype.resolveURI = function(strURI) {
    ...
    if (this.isAtLeastVersion("3.2") && !URIResolver.isAbsoluteURI(uri)) {
      ...
      return URIResolver.DEFAULT.resolveURI(appPathUri.resolve(uri.toString()));
    } else {
  ...
  Server_prototype.reloadLocalizedResources = function() {
    ...
    var includes = this.getSettings().getMapsInArrayByField("includes", "type", "ljss");
    for (var i = 0; i < includes.length; i++) {
      var include = includes[i];
      this.loadInclude(this.resolveURI(include.src), include.id, include.type, false);
    }
  };
