...
// @jsxobf-clobber-shared  _jsxevents _jsxdynamic
/**
...
jsx3.Class.defineClass("jsx3.app.Model", null, [jsx3.util.EventDispatcher], function(Model, Model_prototype) {
  ...
  /**
   * {int} Persistance value for a child that is temporarily part of the DOM tree and will not be persisted.
   * @final @jsxobf-final
   */
  Model.PERSISTNONE = 0;
...
/* @JSC :: end */
  ...
  Model_prototype.setChild = function(objChild, intPersist, strSourceURL, strNS) {
    ...
    if (strNS == null && this._jsxns == null) {
      throw new jsx3.Exception(jsx3._msg("model.set_frag", objChild, this));
    } else if (strNS != null) {
      bFragment = true;
    } else {
      strNS = this._jsxns;
  ...
  Model_prototype._bindFragment = function(objChild, strNS, bDeep, objServer) {
    ...
    objChild._jsxid = jsx3.app.DOM.newId(strNS);
   ...
   * @package
   */
  Model_prototype.adoptChildrenFrom = function(objParent, bRepaint, bForce) {
    var arrChildren = objParent.getChildren().concat();
    if (arrChildren.length > 0) {
      for (var i = 0; i < arrChildren.length; i++)
        this._adoptChild(0, arrChildren[i], bRepaint, bForce, true);
  ...
  Model_prototype.insertBefore = function(objMoveChild,objPrecedeChild,bRepaint) {
    ...
    var bSuccess = true;
    if(!objMoveChild._jsxparent) {
      //user is inserting a fragment
      bSuccess = this.setChild(objMoveChild/*, intPersist, strSourceURL, strNS*/);
    } else if(objMoveChild._jsxparent != this) {
      //user is adopting
      bSuccess = this._adoptChild(objMoveChild.getChildIndex(),objMoveChild,false,true,false);
    }
    if(!bSuccess) return;
...
//    objChild.recalc();
  ...
  Model_prototype._adoptChildRecurse = function(objChild, objFormerParent, curServer, newServer) {
    ...
    //call destroy method on the child to clean up any remaining stuff on the old server; pass true, so the destroy method knows this is an adoption, not a true destroy
    if (objChild.getServer() != this.getServer() || objFormerParent == null) {
      // TODO: this code should be in Cacheable ...
      // shortcut for: objChild.instanceOf(jsx3.xml.Cacheable)
      if (typeof(objChild.resetCacheData) == "function") {
        //get the xml/xsl docs (if they exist) for the child
        var objXML = curServer.getCache().getDocument(objChild.getXMLId());
        var objXSL = curServer.getCache().getDocument(objChild.getXSLId());

        //remove the docs from the former server
        if (objChild.getShareResources() != jsx3.xml.Cacheable.SHARERESOURCES)
          objChild.resetCacheData(curServer);

        //move the docs to the new server if possible/applicable
        if (objXML) newServer.getCache().setDocument(objChild.getXMLId(), objXML);
        if (objXSL) newServer.getCache().setDocument(objChild.getXSLId(), objXSL);
      }
    }
    ...
    var maxLen = objChild.getChildren().length;
    for (var i = 0; i < maxLen; i++)
      objChild._adoptChildRecurse(objChild.getChild(i),null,curServer,newServer);
  };
   ...
   */
  Model_prototype.doClone = function(intPersist) {
    var objParent = this._jsxparent;
    var objJSXClone;

    if (objParent != null) {
      //clone by serializing to XML and then deserializing; ensures only valid structures get cloned (only serializeable structures)
      objJSXClone = objParent._loadObject(this.toXMLDoc(), true);
      if (objJSXClone)
        objJSXClone[0].setPersistence(intPersist);
    } else {
  ...
  Model_prototype.toXMLDoc = function(objProperties) {
    ...
    var objRoot = objXML.createDocumentElement("serialization", Model.CURRENT_VERSION);
    ...
    for (var f in Model._META_MAP) {
      ...
      var elem = objXML.createNode(Entity.TYPEELEMENT, nodeName, Model.CURRENT_VERSION);
      elem.appendChild(objXML.createNode(Entity.TYPECDATA, objProperties[f], Model.CURRENT_VERSION));
  ...
  Model_prototype._doSerialize = function(objXML, objProperties) {
    var objNode = objXML.createNode(Entity.TYPEELEMENT, "object", Model.CURRENT_VERSION);
    //declare variable to persist properties
    var objVars = objXML.createNode(Entity.TYPEELEMENT, "variants", Model.CURRENT_VERSION);
    var objStrings = objXML.createNode(Entity.TYPEELEMENT, "strings", Model.CURRENT_VERSION);
    objNode.appendChild(objVars);
    objNode.appendChild(objStrings);

    var dynaArrays = {_jsxdynamic:"dynamics", jsxcustom:"properties", _jsxevents:"events", jsxxslparams:"xslparameters"};
    for (var f in dynaArrays) {
      var a = this[f];
      if (a != null && typeof(a) == "object") {
        var objDynNode = Model._serializeObjectArray(objXML, dynaArrays[f], a);
        if (objDynNode != null) objNode.appendChild(objDynNode);
      }
    }

    //iterate through all properties in the object instance to generate xml
    for (var p in this) {
      //only act upon properties/attributes of the object, not its functions; only traverse down the DOM--NOT up
      if ((typeof(this[p]) == 'function') || (p.indexOf("_jsx") == 0 && p != "_jsxchildren"))
        continue;

      if (this[p] != null && typeof(this[p]) == 'object') {
        if (p.toString() == '_jsxchildren') {
          if (!(this[p] instanceof Array)) {
            jsx3.util.Logger.GLOBAL.error(jsx3._msg("model.child_notarr", this, this[p]));
            continue;
          }

          // persist object types that are children
          var children = this[p];
          ...
          if (numChildren > 0) {
            for (var i = 0; i < numChildren; i++) {
              ...
              if (intPersist == Model.PERSISTREF || intPersist == Model.PERSISTREFASYNC) {
                ...
                var objInclude = objXML.createNode(Entity.TYPEELEMENT, "include", Model.CURRENT_VERSION);
          ...
          var a = this[p];
          var asString = new Array(a.length);
          for (var i = 0; i < a.length; i++) {
            var val = a[i];
            asString[i] = typeof(val) == "boolean" || typeof(val) == "number" ? val : "'" + val + "'";
          }
          objVars.setAttribute(p, "[" + asString.join(",") + "]");
        }
      } else {
        if (this[p] != null && (this._jsxdynamic == null || this._jsxdynamic[p] == null)) {
          //ensure that this property gets evaluated during deserialization if it is a number or boolean
          if (typeof(this[p]) == 'boolean' || typeof(this[p]) == 'number') {
            objVars.setAttribute(p, this[p]);
          } else {
            objStrings.setAttribute(p, this[p]);
          }
        }
      }
    }
...
/* @JSC :: end */
  ...
  Model._serializeObjectArray = function(objXML, strTagName, objArray) {
    ...
    for (var f in objArray) {
      ...
      objNode.setAttribute(f, objArray[f]);
    }
    return objNode;
  };
  ...
  Model_prototype.getUriResolver = function() {
    var node = this;
    while (node != null) {
      if (node._jsxresolver != null) return node._jsxresolver;
  ...
  Model_prototype.loadXML = function(strXML, bRepaint, objResolver) {
    ...
    return this._loadObject(objXML, bRepaint, null, objXML.getSourceURL(), null, null, null, objResolver, null)[0];
  };
  ...
  Model_prototype._loadObject = function(objXML, bRepaint, intPersist, strSourceURL, strRawURL, strNS,
    ...
    if (objResolver == null) {
      ...
      var uriResolver = jsx3.net.URIResolver.getResolver(strRawURL);
      if (uriResolver != jsx3.net.URIResolver.DEFAULT)
        objResolver = uriResolver;
    } else if (strRawURL) {
    ...
    var i = objXML.selectNodeIterator(serPath + "jsx1:object | " + serPath + "jsx1:objects/jsx1:object");
    var objToInsert = [];

    while (i.hasNext()) {
      //get node descriptor in the XML serializatoin file and deserialize the node as a child of 'this' object
      var objNode = i.next();
      var objJSX = this._doLoad(objNode, strSourceURL, strNS, objServer, objCache, branchResolver, objResolver);
    ...
    if (objToInsert.length > 0 && attachedServer)
    ...
    if (bRepaint != false)
      for (var i = 0; i < objToInsert.length; i++)
        this.viewUpdateHook(objToInsert[i], i < objToInsert.length - 1);
   ...
   * @param myResolver {jsx3.net.URIResolver} the URI resolver to set at this node in the DOM.
   * @return {jsx3.app.Model} the loaded object with any descendant objects also loaded and attached.
   * @private
   * @jsxobf-clobber
   */
  Model_prototype._doLoad = function(objXML, strSourceURL, strNS, objServer, objCache, objResolver, myResolver) {
...
/* @JSC :: end */
    ...
    if (myResolver)
      /* @jsxobf-clobber */
      objInstance._jsxresolver = myResolver;
    ...
    Model._applyStringAttributes(objInstance, objXML);
    Model._applyVariantAttributes(objInstance, objXML);
    Model._bindObjectProperty(objInstance, objXML, "_jsxdynamic", "jsx1:dynamics");
    Model._bindObjectProperty(objInstance, objXML, "jsxcustom", "jsx1:properties");
    Model._bindObjectProperty(objInstance, objXML, "_jsxevents", "jsx1:events");
    Model._bindObjectProperty(objInstance, objXML, "jsxxslparams", "jsx1:xslparameters");
    ...
    while (i.hasNext()) {
      ...
      if (objItem.getBaseName() == "object") {
        ...
        var objNewChild = objInstance._doLoad(objItem, strSourceURL, strNS, objServer, objCache, objResolver, null);
        if (objNewChild != null) objInstance.setChild(objNewChild, Model.PERSISTEMBED, null, strNS);
  ...
  Model._applyStringAttributes = function(objInstance, objXML) {
    var objProp = objXML.selectSingleNode("jsx1:strings");
    if (objProp != null) {
      var names = objProp.getAttributeNames();
      for (var i = 0; i < names.length; i++) {
        var name = names[i];
        objInstance[name] = objProp.getAttribute(name);
      }
    }
  };
  ...
  Model._applyVariantAttributes = function(objInstance, objXML) {
    var objProp = objXML.selectSingleNode("jsx1:variants");
    if (objProp != null) {
      var names = objProp.getAttributeNames();
      for (var i = 0; i < names.length; i++) {
        var name = names[i];
        var strValue = objProp.getAttribute(name);
        // Checking isNaN is a 100x speedup on Fx and a 25x speedup on IE6 for all numbers
        objInstance[name] = isNaN(strValue) ? jsx3.eval(strValue) : Number(strValue);
      }
    }
  };

  /** @private @jsxobf-clobber */
  Model._bindObjectProperty = function(objInstance,objXML,strObjName,strNodeName) {
    //able to convert a given node in a serializattion file to a javascript object with name/value pairs
    var objProp = objXML.selectSingleNode(strNodeName);
    if (objProp != null) {
      ...
      for (var i = 0; i < names.length; i++) {
        var name = names[i];
        objTemp[name] = objProp.getAttribute(name);
      }
    }
  };
