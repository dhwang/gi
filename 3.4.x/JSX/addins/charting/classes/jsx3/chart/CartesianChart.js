...
jsx3.Class.defineClass("jsx3.chart.CartesianChart", jsx3.chart.Chart, null, function(CartesianChart, CartesianChart_prototype) {
   ...
   * @private
   */
  CartesianChart.XPRIMARY_FILTER = function(c) { 
   ...
   * @private
   */
  CartesianChart.XSECONDARY_FILTER = function(c) { 
    return (c instanceof Axis) && !c.getPrimary() && c.getHorizontal(); 
  };
   ...
   * @private
   */
  CartesianChart.YPRIMARY_FILTER = function(c) { 
    return (c instanceof Axis) && c.getPrimary() && !c.getHorizontal(); 
  };
   ...
   * @private
   */
  CartesianChart.YSECONDARY_FILTER = function(c) { 
    return (c instanceof Axis) && !c.getPrimary() && !c.getHorizontal(); 
  };
   ...
   * @private
   */
  CartesianChart.ZINDEX_GRIDLINES = Chart.ZINDEX_DATA + 1;
  CartesianChart.ZINDEX_GRIDLINES_FOREGROUND = Chart.ZINDEX_DATA + 90;
  CartesianChart.ZINDEX_AXIS = Chart.ZINDEX_DATA + 100;
  ...
  CartesianChart_prototype.getPrimaryXAxis = function() {
    return this.findDescendants(CartesianChart.XPRIMARY_FILTER, false, false, true);
  };
  ...
  CartesianChart_prototype.getSecondaryXAxis = function() {
    return this.findDescendants(CartesianChart.XSECONDARY_FILTER, false, false, true);
  };
  ...
  CartesianChart_prototype.getPrimaryYAxis = function() {
    return this.findDescendants(CartesianChart.YPRIMARY_FILTER, false, false, true);
  };
  ...
  CartesianChart_prototype.getSecondaryYAxis = function() {
    return this.findDescendants(CartesianChart.YSECONDARY_FILTER, false, false, true);
  };
   ...
   * @private
   */
  CartesianChart_prototype.updateView = function() {
    this.jsxsuper();
    ...
    for (var i = 1; i <= 2; i++) {
      ...
      if (i == 1) {
        ...
        axisWidth = this.getAxisMetrics(sxAxis, syAxis, pxAxis, pyAxis);
      }
      ...
      this.prepareAxis(pxAxis, dataDim[2]);
      this.prepareAxis(pyAxis, dataDim[3]);
      this.prepareAxis(sxAxis, dataDim[2]);
      this.prepareAxis(syAxis, dataDim[3]);
    }
    ...
    for (var i = 0; i < grids.length; i++) {
      ...
      gridLines.setZIndex(gridLines.getInForeground() ? 
          CartesianChart.ZINDEX_GRIDLINES_FOREGROUND : CartesianChart.ZINDEX_GRIDLINES);
          
      gridLines.updateView();
      root.appendChild(gridLines.getRenderRoot());
    }
    
    this.updateAxisView(pxAxis, dataDim[0], dataDim[1]);
    this.updateAxisView(pyAxis, dataDim[0], dataDim[1]);
    this.updateAxisView(sxAxis, dataDim[0], dataDim[1]);
    this.updateAxisView(syAxis, dataDim[0], dataDim[1]);
   ...
   * @private
   */
  CartesianChart_prototype.prepareAxis = function( axis, length ) {
    if (axis != null) {
   ...
   * @private
   */
  CartesianChart_prototype.updateAxisView = function( axis, offsetX, offsetY ) {
    var root = this.getDataCanvas();
    if (axis != null) {
      ...
      axis.setZIndex(CartesianChart.ZINDEX_AXIS);
      
      axis.updateView();
      root.appendChild(axis.getRenderRoot());
    }
  };
   ...
   * @private
   */
  CartesianChart_prototype.getAxisMetrics = function( sxAxis, syAxis, pxAxis, pyAxis ) {
    var l = 0, t = 0, r = 0, b = 0;
    
    if (sxAxis != null) {
      var gutter = sxAxis.getGutterWidths();
      t = gutter[1];
      b = gutter[0];
    }
    if (syAxis != null) {
      var gutter = syAxis.getGutterWidths();
      r += gutter[0];
      l += gutter[1];
    }
    if (pxAxis != null) {
      var gutter = pxAxis.getGutterWidths();
      b = Math.max(b, gutter[1]);
      t = Math.max(t, gutter[0]);
    }
    if (pyAxis != null) {
      var gutter = pyAxis.getGutterWidths();
  ...
  CartesianChart_prototype.toString = function() {
    return "[CartesianChart '" + this.getName() + "']";
  };
