...
//    jsunit.debug("Loaded " + strSrc + ". Still waiting for [" + jsunit._waiting + "]");
    ...
    if (strSrc == jsunit.JSX_JS_BASE + "JSX30.js") {
      ...
      var params = jsunit.getQueryParams(document.location.href);
      for (var p in params)
  ...
  jsunit._loadJsxIncludes = function() {
    for (var i = 0; i < jsx3.lang.ClassLoader.INCLUDES.length; i++) {
      var inc = jsx3.lang.ClassLoader.INCLUDES[i];
      if (inc.type == "ljss" && jsx3.app.PropsBundle) {
        var src = jsx3.CLASS_LOADER.resolvePath(jsunit.JSX_BASE + "JSX/" + inc.src);
        jsx3.app.PropsBundle.getProps(src, null, jsx3.getSystemCache());
      }
    }
  };
